...
<table class="table_interna text_center">
	<tr>
        <td><span class="titolo font18 text_center">Elenco <?php echo $cls_ruolo->a_docDetails['title']; ?></span></td>
    </tr>
    <tr>
...
for($i=0;$i<count($a_results);$i++){
    ...
    if(!empty($a_results[$i]['Totale_Dovuto']))
        $totDovuto = number_format($a_results[$i]['Totale_Dovuto'],2,",","");
    else
        $totDovuto = "0,00";
    $a_value[0] = array(
    ...
    if(!empty($a_results[$i]['Totale_Pagato']))
        $totPagato = number_format($a_results[$i]['Totale_Pagato'],2,",","");
    else
        $totPagato = "0,00";
    $a_value[1] = array(
    ...
    if($filter['fileType']=="pdf")
        ...
        $a_value[0][4] = $a_results[$i]['Totale_Dovuto'];
        $a_value[1][4] = $a_results[$i]['Totale_Pagato'];
        $a_params = array(
