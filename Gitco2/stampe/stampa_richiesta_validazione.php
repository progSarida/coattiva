...
$tempObj = $cls_partita->info_spedizione((array) $atto);
if($tempObj!=null) $spedizione = (object) $tempObj;
else $spedizione = $tempObj;
...
switch($tipo_richiesta)
{
	...
	default:	
		...
		echo json_encode([
			"error" => 1,
			"msg" => "Errore di selezione stampa!!"
		]);
		die;	
...
//$dump = "AAAA";
//var_dump($nome_file);die;


$path_temp_F = $cls_utils->crea_dir( ARCHIVIO ."/temp" );
$path_temp_W =  ARCHIVIO_WEB ."/temp";
$file_temp_F = $path_temp_F."/".$nome_file;
$file_temp_W = $path_temp_W."/".$nome_file;

if($stampa_select == "PROVVISORIA")
{
	//ob_end_clean();
	
	$pdf->Output( $file_temp_F , 'F');
	echo json_encode([
		"path" => $file_temp_W,
		"error" => 0,
	...
	if($cls_mail->send())
	{
		...
		if( $control_salva )
		{
			...
			else
			{
				...
				echo json_encode([
					"error" => 1,
					"msg" => "Aggiornamento su tabella atto fallito!"
				]);
				die;
			}
		...
		else
		{
			...
			echo json_encode([
				"error" => 1,
				"msg" => "Salvataggio tabella documento fallito!"
			]);
			die;
		}
		...
		else
		{
			...
			echo json_encode([
				"error" => 1,
				"msg" => "Inserimento su tabella email_inviate fallito!"
			]);
            die;
		}
...
else 
{
	...
	else
	{
		...
		echo json_encode([
			"error" => 1,
			"msg" => "Inserimento su tabella documento fallito!"
		]);
		die;
	}
