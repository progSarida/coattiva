...
$query = 'SELECT U.Comune_ID, U.CC_Comune, U.Cognome, U.Nome 
FROM `utente` AS U
LEFT JOIN indirizzo AS I ON I.Utente_ID = U.ID AND I.Tipo = "rec"
WHERE U.Data_Morte IS NOT NULL AND U.Genere != "D" AND (I.Presso IS NULL OR I.Presso = "" OR I.ID IS NULL) ';

if($CC != "")
    $query .= "AND U.CC_Comune = '".$CC."' ";
...
$a_headerPage[0] = array("Codice Catastale","Comune","Utente ID","Cognome","Nome");
$dataExcel[] = array("<b>Codice Catastale</b>","<b>Comune</b>","<b>Utente ID</b>","<b>Cognome</b>","<b>Nome</b>");


$pdf->setArray($a_headerPage,"a_headerPage");
$percent = 100/10*($pdf->getPageWidth()-20)/100;
$a_width = array( $percent , $percent*2 , $percent , $percent*3 , $percent*3 );
$a_align = array( "L" , "L" , "L" , "C" , "C");
...
for($i=0; $i < $count; $i++){
    ...
    $query = "SELECT Denominazione FROM enti_gestiti WHERE CC = '" . $result[$i]["CC_Comune"] . "'";
    ...
    if($fileType == 2) {
        ...
        $dataExcel[] = array($result[$i]["CC_Comune"],$city["Denominazione"],$result[$i]["Comune_ID"],$result[$i]["Cognome"],$result[$i]["Nome"]);
    }
}
