...
$filter["stato"] = $statoFlusso = $cls_help->getVar("stato");

$query = 'SELECT F.Year, F.Number, F.CityId, F.RecordsNumber ,IF(F.CancelDate IS NOT NULL, "ANNULLATO", IF(F.SendDate IS NOT NULL, "CONSEGNATO", IF(F.PostagePaymentDate IS NOT NULL, "PAGATO", IF(F.ProcessingDate IS NOT NULL, "LAVORATO", IF(F.UploadDate IS NOT NULL, "UPLOAD", "CREATO"))))) AS Stato,
       F.UploadDate, F.ProcessingDate, F.SendDate, F.PostagePaymentDate
        FROM flows as F
        WHERE 1 = 1  ';
...
if($statoFlusso != ""){
    $query .= ' AND (IF(F.CancelDate IS NOT NULL, "ANNULLATO", IF(F.SendDate IS NOT NULL, "CONSEGNATO", IF(F.PostagePaymentDate IS NOT NULL, "PAGATO", IF(F.ProcessingDate IS NOT NULL, "LAVORATO", IF(F.UploadDate IS NOT NULL, "UPLOAD", "CREATO")))))) = "'.$statoFlusso.'" ';
}
...
$a_headerPage[0] = array("Codice Catastale","Denominazione","Anno","Numero","Stato","Data Upload","Data Fine Lavorazione","Data Consegna","Data Pagamento");
$dataExcel[] = array("<b>Codice Catastale</b>","<b>Denominazione</b>","<b>Anno</b>","<b>Numero</b>","<b>Stato</b>","<b>Data Upload</b>","<b>Data Fine Lavorazione</b>","<b>Data Consegna</b>","<b>Data Pagamento</b>");
...
for($i=0; $i < $count; $i++){
    ...
    if($fileType == 2) {
        ...
        $dataExcel[] = array($result[$i]["CityId"],$city["Denominazione"],$result[$i]["Year"],$result[$i]["Number"],$result[$i]["Stato"],$result[$i]["UploadDate"],$result[$i]["ProcessingDate"],$result[$i]["SendDate"],$result[$i]["PostagePaymentDate"]);
    }
}
