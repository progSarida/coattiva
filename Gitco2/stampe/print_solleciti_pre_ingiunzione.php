...
$cls_ruolo->getTypeDetails($filter['docType']);
...
$a_fileToSave['webTempPath'] = $cls_file->getWebPath($a_fileToSave['rootTempPath']);
$a_fileToSave['rootFinalPath'] =  $cls_file->folderCreation($a_fileToSave['docDir']."/STAMPE DEFINITIVE");
$a_fileToSave['webFinalPath'] = $cls_file->getWebPath($a_fileToSave['rootFinalPath']);
$a_fileToSave['rootFlowPath'] =  $cls_file->folderCreation($a_fileToSave['docDir']."/FLUSSI");
$a_fileToSave['webFlowPath'] = $cls_file->getWebPath($a_fileToSave['rootFlowPath']);
...
if($filter['printType']=="flow"){
    ...
    $cls_flow->setMod23($a_generalParams);
...
for($i=0;$i<count($a_results);$i++){
    ...
    if($filter['printType']=="flow"){
        ...
        $pdf->setPrintFromParams($cls_text->a_textReplaced, $cls_text->a_textParams, $a_amounts, $cls_params->a_signature, 8.5);
        $pdf->setPostalBill($a_postal,1);
...
if(count($a_results) == 0)
    ...
    if($filter['printType']=="temp"){
        ...
        $mergeFile = "Solleciti_Pre_Ingiunzione_".$c."_".$filter['finalDate']."_".date('H-i-s').".pdf";
