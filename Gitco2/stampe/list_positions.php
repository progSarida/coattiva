...
$a_enteAdmin = $cls_db->getArrayLine( $cls_db->SelectQuery("SELECT * FROM enti_gestiti WHERE CC = '".$city."'") );							 
...
$query = "SELECT ( SELECT SUM(TRIB.Imposta) FROM `tributo` as TRIB where TRIB.Partita_ID = v_positions.Partita_ID ) as Importo_Preso_In_Carico, v_positions.* FROM v_positions ";
$query.= "WHERE 1=1 ";
...
//    $a_headerPage[2] = array("Pignoramento","Rateiz. pigno.","Stato rateiz. pigno.","Note blocco coazione","");
    ...
    $a_headerPage[2] = array("ID Utente - Nominativo", "Modalità Notifica Atto", "Tot. Pr.Car.", "Modalità Notifica Pigno.", "Tot.3");
    ...
    $a_width = array( $percent*4 , $percent*3, $percent, $percent*3, $percent );
    $a_align = array( "L" , "L" , "R", "L" , "R"  );
    $a_totalsVar = array(0=>array(2,4),1=>array(2,4),2=>array(2,4),4=>array(4));
    $a_totalsHeader = array(
        0=>array("{TOTALE}","Totale 1 < 60gg","{0}","Totale 1","{1}"),
        1=>array("","Totale 2 > 60gg","{2}","Totale 3","{3}"),
        2=>array("","Totale Preso in Carico","{4}","Totale 3","{5}"),
        3=>array("","","","Totale pagato","{6}")
    );
...
for($i=0;$i<$count;$i++){
    ...
    if($a_results[$i]['Atto_ID']>0){
        ...
        if($a_results[$i]['Importo_Preso_In_Carico']>0)
            $a_actTotals[3] = number_format($a_results[$i]['Importo_Preso_In_Carico'],2,",","");
    }
