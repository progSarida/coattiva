...
$selectedDaStampare = "";
$nascondi_flag = "<script>$('.flag').hide();</script>";
$flag_ristampa = "";
...
switch($tipo_atto)
{
	case "Ingiunzione":			$action_page = "stampa_ingiunzioni.php";
								$tipo_atto = "Ingiunzione";
								$next_page = "sollecito";
								$prev_page = "preavviso_ing";
								$nascondi_flag = "";
								$printTypeWhere = "WHERE Disabled=0 AND (Id=1 OR Id=6)";
                                $selectedPrintType = 1;
								break;
								
	case "sollecito":			$action_page = "stampa_solleciti_ingiunzione.php";
								$tipo_atto = "Sollecito di pagamento";
								$next_page = "avv_intimazione";
								$prev_page = "Ingiunzione";
                                $printTypeWhere = "WHERE Disabled=0 AND Id!=1";
                                $selectedPrintType = 3;
								break;

	case "avv_intimazione":		$action_page = "stampa_avvisi.php";
...
$a_printType = $cls_db->getResults($cls_db->ExecuteQuery("SELECT * FROM print_type ".$printTypeWhere));
$a_selection = array("value"=>"Id","firstOpt"=>0,"selected"=>$selectedPrintType, "text"=>array("[Description]"));
$optPrintType = $cls_html->getOptions($a_printType,$a_selection);
...
function annulla()
{
	location.href="stampa_atto.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>&tipo_atto="+atto_val;
}
...
function stampa_change()
{
	
	tipo_stampa = $("#stampa_select").val();
	
	if(tipo_stampa=="Definitiva"){
		$('#p_avviso').text("L'esecuzione della stampa definitiva aggiornera' lo stato di stampa degli atti e la corrispondenza.");
...
function CheckSelectStampa()
{
	tipo_stampa = $("#stampa_select").val();
	stato_stampa = $("#stato_stampa").val();
	
	if (stato_stampa != "Da stampare")
	{
		var TipoAtto = "<?php echo $tipo_atto ?>";
		if ( TipoAtto == "Preavviso di Ingiunzione" )
		{
			alert ("Per i preavvisi e' selezionabile solo la scelta STATO DI STAMPA: 'Da stampare'");
			$("#stato_stampa").val("Da stampare");
		}
	}
...
<table class="table_interna text_center" border="0">
	...
	<tr>
		...
		<td class="text_left width40">
			<input name=tipo_atto readonly class="text_left sfondo_ricerca" value="<?php echo $tipo_atto; ?>" size=35 tabindex=1>
		</td>
		<td class="width20 text_left">Tipo di stampa</td>
		<td class="width25 text_left">
			<select name="stampa_select" id=stampa_select onchange="stampa_change();" tabindex=2>
				<option>Provvisoria</option>
				<option value=cronologici>Assegnamento Cronologici</option>
				<option>Definitiva</option>
                <?php if($tipo_atto!="Sollecito di pagamento"){ ?>
				<option>Flusso</option>
                <?php } ?>
			</select>
		</td>
	</tr>
	<tr class="display_element">
		<td class="text_left" colspan=2>
            ...
            if($flag_ristampa=="y"){
                ...
                <input type="checkbox" name="flag_ristampa" value="y">
...
<table class="table_interna text_center" border="0">
    ...
    <tr>
        <td class="text_left width25"><font class="color_titolo font_bold">Modalita spedizione</font></td>
        <td class="width40 text_center" colspan=4>
            <select name="PrintTypeId" id="PrintTypeId" class="width100">
                <?=$optPrintType;?>
            </select>
        </td>
    ...
    <tr>
        <td class="text_left width25"><font class="color_titolo font_bold">Tipo di riscossione</font></td>
        <td class="width40 text_center" colspan=4>
            <select name="TipoUfficiale" id="TipoUfficiale" class="width100">
    ...
    <tr>
        <td class="text_left width25"><font class="color_titolo font_bold">Tipo entrata</font></td>
        <td class="width40 text_center" colspan=4>
            <select name="tipo_partita" id="tipo_partita" class="width100">
                <?=$optionsRiscossione;?>
            </select>
        </td>
    </tr>
	...
	<tr>
		...
		<td class="width40 text_center" colspan=4>
			<select name=ordinamento class="width100">
	...
	<tr class="flag">
		<td colspan=9 class="text_left"><input type="checkbox" id="atto_precedente" name="atto_precedente" value="si"> Aggiungi i dati dell'atto uscito precedentemente sulla stampa</td>
	</tr>
	<tr>
		<td colspan=9><hr></td>
	</tr>
