...
$query = "SELECT A.*, T.Info_Cartella, PT.Tipo AS Tipo_Entrata, PT.Comune_ID AS Partita_Comune_ID, PT.Anno_Riferimento, T.Tipo_Info , U.Ditta, U.Cognome, U.Nome, SA.ID AS ID_Sospensione, SA.Motivo_Sospensione_ID, SA.Note_Sospensione, SA.Data_Sospensione 
            FROM v_atti as A 
            ...
            JOIN sospensione_atto as SA on SA.Partita_ID = A.Partita_ID AND SA.ID_Atto_Pigno = A.Atto_ID
...
$query_ing =   "SELECT * 
                FROM atto 
                WHERE Partita_ID = ".$p." AND (DocumentTypeId = 2 OR DocumentTypeId = 4)
                AND Data_Notifica IS NOT NULL 
                ORDER BY Data_Notifica DESC
                LIMIT 1";

$resultIng = $cls_db->getArrayLine($cls_db->ExecuteQuery($query_ing));

if($resultIng == null){
    echo json_encode([
        "error" => 2,
        "msg" => "Impossibile stampare sospensione atto: non è stata inserita la data di notifica!"
    ]);
    die;
}
...
$actNot = strtoupper($resultIng["Atto"]." N. ".$resultIng["ID_Cronologico"]." DEL ".$resultIng["Anno_Cronologico"]." NOTIFICATO IL ".$cls_date->Get_DateNewFormat($resultIng["Data_Notifica"],"DB"))."<br>";
...
$a_text_annull = $cls_db->getArrayLine($cls_db->SelectQuery($cls_text_annull->getParametersQuery($c,45)));
...
$identificativo_file = "Sospensione_Atto_".$c."_".$atto->ID_Cronologico."_".$atto->Anno_Cronologico."_".date("Y-m-d_H-i");
