...
if ($genere_da != "D" && $genere_a != "D") {
    ...
    if(($genere_da == "D" || $genere_da == "M" || $genere_da == "F") && $genere_a == null){
        ...
        echo json_encode([
            "error" => 2,
            "msg" => "Non selezionare ditte e utenti insieme nella selezione utenti!! (Da Cognome/Nome - A Cognome/Nome). Selezionare entrambe imprese o entrambi utenti!"
        ]);
        die;
    }
}
...
if($contoTerzi=="Y")
    ...
    $query.= "AND (pagamento.Conto_Terzi != 'Y' OR pagamento.Conto_Terzi IS NULL)";
...
*/
	...
	$changeParams = "";
	$ctrl_linea = "no";
    $count = count($arrayPagamenti);
	for( $l=0; $l < $count; $l++ )//FOR ATTI
	{

        set_time_limit(300);

        if(session_status() == PHP_SESSION_NONE)session_start();
        $_SESSION['progress'] = number_format(($l*100)/$count ,2);
        session_write_close();
	...
	if ($tipofile == "PDF")
	{
		//echo "<br>" . $file_elenco;
		//return;
        $file = SUPER_WEB_ROOT.$cls_utils->mostra_file_path($file_elenco);
	...
	{
        $file = SUPER_WEB_ROOT.$cls_utils->mostra_file_path($file_csv);
		fclose($myCsv);
        /*
		if ($cont_pag_result == 0 && $cont_pagdaass_result == 0) 
		{
