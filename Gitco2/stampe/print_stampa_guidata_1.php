...
$query = 'SELECT F.Year, F.Number, F.CityId, F.UploadDate, IF(F.CancelDate IS NOT NULL, "ANNULLATO", IF(F.SendDate IS NOT NULL, "CONSEGNATO", IF(F.PostagePaymentDate IS NOT NULL, "PAGATO", IF(F.ProcessingDate IS NOT NULL, "LAVORATO", IF(F.UploadDate IS NOT NULL, "UPLOAD", "CREATO"))))) AS Stato,
       (SELECT COUNT(ID) FROM atto AS A WHERE F.Id = A.FlowId) AS NumeroAtti, F.ProcessingDate, F.SendDate, F.PostagePaymentDate
        FROM flows as F
        ...
        WHERE (
          ...
          IF(F.CancelDate IS NOT NULL, "ANNULLATO", IF(F.SendDate IS NOT NULL, "CONSEGNATO", IF(F.PostagePaymentDate IS NOT NULL, "PAGATO", IF(F.ProcessingDate IS NOT NULL, "LAVORATO", IF(F.UploadDate IS NOT NULL, "UPLOAD", "CREATO"))))) != "ANNULLATO" ';
...
$a_headerPage[0] = array("Codice Catastale","Comune","Anno flusso","Numero flusso","Stato","Data Upload","Data Fine Lavorazione","Data Consegna","Data Pagamento","N° Atti Flusso");
$dataExcel[] = array("<b>Codice Catastale</b>","<b>Comune</b>","<b>Anno flusso</b>","<b>Numero flusso</b>","<b>Stato</b>","<b>Data Upload</b>","<b>Data Fine Lavorazione</b>","<b>Data Consegna</b>","<b>Data Pagamento</b>","<b>N° Atti Flusso</b>");


$pdf->setArray($a_headerPage,"a_headerPage");
$percent = 100/10*($pdf->getPageWidth()-20)/100;
$a_width = array( $percent , $percent , $percent , $percent , $percent , $percent , $percent , $percent , $percent , $percent );
$a_align = array( "L" , "L" , "L" , "L" ,"L" ,"L" ,"L" ,"L" ,"L" ,"L");
...
for($i=0; $i < $count; $i++){
    ...
    if($fileType == 2) {
        ...
        $a_value[0] = array(
            ...
            $result[$i]["Stato"],
            $result[$i]["UploadDate"],
            $result[$i]["ProcessingDate"],
            $result[$i]["SendDate"],
            $result[$i]["PostagePaymentDate"],
            $result[$i]["NumeroAtti"]
        );
        ...
        $dataExcel[] = array($result[$i]["CityId"],$city["Denominazione"],$result[$i]["Year"],$result[$i]["Number"],$result[$i]["Stato"],$result[$i]["UploadDate"],$result[$i]["ProcessingDate"],$result[$i]["SendDate"],$result[$i]["PostagePaymentDate"],$result[$i]["NumeroAtti"]);
    }
}
