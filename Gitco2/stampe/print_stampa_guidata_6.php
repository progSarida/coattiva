...
$query = 'SELECT PT.Comune_ID, PT.CC, DT.Description, A.Totale_Dovuto, ( 
    SELECT SUM(P.Importo) 
    FROM pagamento AS P 
    WHERE P.Partita_ID = PT.ID AND P.DocumentTableTypeId = 1) AS Totale_Pagato, (
        SELECT IF(A2.Data_Richiesta_Rate IS NULL OR (A2.Rate_Previste IS NULL OR A2.Rate_Previste = 0),"NO","SI") 
        FROM atto AS A2 
        WHERE A2.ID = A.ID) AS Rateizzazione
FROM partita_tributi AS PT
LEFT JOIN atto AS A ON A.Partita_ID = PT.ID AND A.ID = (SELECT ID FROM atto AS A2 WHERE A2.Partita_ID = PT.ID AND A2.CC = PT.CC ORDER BY A2.Data_Elaborazione DESC LIMIT 1)
LEFT JOIN document_type AS DT ON A.DocumentTypeId = DT.Id
WHERE (A.Totale_Dovuto - ( SELECT SUM(P.Importo) FROM pagamento AS P WHERE P.Partita_ID = PT.ID AND P.DocumentTableTypeId = 1)) > 0 ';

if($CC!="")
    $query .= ' AND PT.CC = "'.$CC.'" ';

if($range_giorni != ""){
    $query .= " AND (NOW() > (SELECT P.Data_Pagamento FROM pagamento AS P WHERE P.Atto_ID = A.ID ORDER BY P.Data_Pagamento DESC LIMIT 1) + INTERVAL ".$range_giorni." DAY) ";
}
...
$a_headerPage[0] = array("Partita ID","Codice catastale","Comune","Tipo atto","Totale Dovuto","Totale Pagato","Rateizzazione SI/NO");
$dataExcel[] = array("<b>Partita ID</b>","<b>Codice Catastale</b>","<b>Comune</b>","<b>Tipo atto</b>","<b>Totale Dovuto</b>","<b>Totale Pagato</b>","<b>Rateizzazione SI/NO</b>");


$pdf->setArray($a_headerPage,"a_headerPage");
$percent = 100/10*($pdf->getPageWidth()-20)/100;
$a_width = array( $percent , $percent , $percent*2 , $percent*3 , $percent , $percent , $percent );
...
for($i=0; $i < $count; $i++){
    ...
    if($fileType == 2) {
        ...
        $a_value[0] = array(
            ...
            $result[$i]["Totale_Dovuto"],
            $result[$i]["Totale_Pagato"],
            $result[$i]["Rateizzazione"]
        );
        ...
        $dataExcel[] = array($result[$i]["Comune_ID"],$result[$i]["CC"],$city["Denominazione"],$result[$i]["Description"],$result[$i]["Totale_Dovuto"],$result[$i]["Totale_Pagato"],$result[$i]["Rateizzazione"]);
    }
}
