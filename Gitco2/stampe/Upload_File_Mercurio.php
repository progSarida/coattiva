...
if($a_enteAdmin['Ufficio_ID']>0)
    $enteEmail = $a_enteAdmin['Ufficio_Mail'];
else if($a_enteAdmin['Gestore_ID']>0)
    $enteEmail = $a_enteAdmin['Gestore_Mail'];
else if($a_enteAdmin['Info_ID']>0)
    $enteEmail = $a_enteAdmin['Info_Mail'];
if(empty($enteEmail)){
    $msg = "EMAIL UFFICIO/GESTORE/ENTE ASSENTE. INSERIRE I DATI NEI PARAMETRI.";
    echo "<script>window.location = document.referrer + '&error=1&msg={$msg}';</script>";
}
...
try{
    ...
    $file = ATTI . "/" . $resultF["CityId"] . "/" . $cartella . "/FLUSSI/" . $FileName;
