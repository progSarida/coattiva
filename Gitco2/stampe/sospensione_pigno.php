...
{
    $query = "SELECT * FROM pignoramento_generale WHERE ID = ".$ID_Atto." AND CC = '".$c."'";
	$pigno = $cls_db->getObjectLineNull($cls_db->ExecuteQuery($query),"atto");//new atto($ID_Atto, $c);
    $query = "SELECT * FROM partita_tributi WHERE ID = '".$pigno->Partita_ID."' AND CC = '".$c."'";
	...
	$cronologico = $pigno->ID_Cronologico;
	$anno = $pigno->Anno_Cronologico;
}
else 
{
	$partita_ID = "";
...
switchMenuImg("F5");
F5_button = function(){
    location.href="sospensione_atto.php?richiesta_singola=<?= $richiesta_singola; ?>&c=<?php echo $c; ?>&a=<?php echo $a; ?>&ID_Atto=<?php echo $ID_Atto; ?>";
}
...
function ajaxCall() {
        ...
        $.ajax({
            ...
            url: $("#sospensione_form").attr('action'),
            //data: new FormData(document.getElementById("storico_form")),
            data: $("#sospensione_form").serialize(),
            dataType : 'json',
            type: 'POST',
            success: function (resp) {
                ...
                if(resp.error == 0)
                    showFileOnModal(resp.path,"Stampa sospensione",resp.path.split('.').pop());
            },
            error:function(resp)
            {
                spinner.closeSpinner();
...
function ritorno_atto()
{
	...
	{
		link = "<?= WEB_ROOT; ?>/coattiva/coazione.php?partita=<?php echo $partita_ID; ?>&c=<?php echo $c; ?>&a=<?php echo $a; ?>";
		location.href= link;
	}
}
...
<div class="row justify-content-md-center ">
    <div class="col col-md-auto text_center">
        <p class="titolo font16 under_decor">Stampa sospensione pignoramenti</p>
    </div>
</div>
	
<form id="sospensione_form" name="sospensione_form" action="stampa_sospensione_pigno.php">
    ...
    <div class="row">
        <div class="col col-lg-6 col-lg-offset-1">
            <div class="form-group">
                <label class="col-lg-4 control-label resize" style="text-align: left;"><span >Nominativo utente/ditta</span></label>
                <div class="col-lg-8">
