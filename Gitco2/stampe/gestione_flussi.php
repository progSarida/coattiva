...
$control_pigno = get_var('control_pigno');

set_time_limit(200);
$array_atti = get_var('array_flussi');
...
for($i=0;$i<count($array_atti);$i++)
{
	if($control_pigno!="si") {
        ...
        $infoCart[$i] = $atto[$i]->Info_Cartella;
    }
	else {
        $atto[$i] = new pignoramento($array_atti[$i], $c);
        $infoCart[$i] = "";
    }
...
<tr class="text_left riga_dispari" style="height:30px;" >
	...
	<td class="width1"><br></td>
	<td class="width12"><b><?php echo count($atto);?> ATTI</b></td>
</tr>
...
{		
	...
	if($control_pigno!="si"){
		...
		$testo_atto = $atto[$i]->Atto;
	}
	else{
		$link = $atto[$i]->pignoramento_stampato($atto[$i]->Tipo, $tipo_stampa, $atto[$i]->Tipo_Terzi);
		$testo_atto = "Pignoramento ".$atto[$i]->Tipo." ".$atto[$i]->Tipo_Terzi;
	}
...
?>
	<tr <?php echo $stile_riga; ?>>
		...
		<td class="width1"><br></td>
		<td class="text_left width15"><?php echo $testo_atto; ?></td>
		<td class="width1"><br></td>
	...
	<tr <?php echo $stile_riga; ?>>
		...
		<td class="width1"><br></td>
		<td class="text_left" colspan=9><?php echo $infoCart[$i]; ?><br><span class="color_red">PARTITA N.<?php echo $partita->Comune_ID;?> DEL <?php echo $partita->Anno_Riferimento;?></span><br><span class="color_red">FLUSSO N.<?php echo $atto[$i]->Numero_Flusso; ?> DEL <?php echo $atto[$i]->Anno_Flusso; ?> CREATO IL <?php echo from_mysql_date($atto[$i]->Data_Flusso); ?></span></td>
		<td class="width1"><br></td>
