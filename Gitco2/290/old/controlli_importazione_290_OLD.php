    ...
    <script>
        ...
        function fine() {
            if (solo_rate == 0) {
                ...
                $("div#importazione").append("<div class='row' style='margin-top: 3%;'><div class='col-lg-10 col-lg-offset-1'><input type=button name=riepilogo class='btn btn-danger' value='Scarti' onclick='lista_scarti();'><input type=button name=avanti class='btn btn-primary' value='Importazione' onclick='importa();'></div></div>");
            } else
...
for ($i = 0; $i < $duenovanta["Record_N1"]; $i++) {
    ...
    for ($x = 0; $x < $duenovanta["n1"][$i]["Record_N2"]; $x++) {
        ...
        flush();
        ob_flush();
        echo "<script>$( \"#progressbar_caricamento\" ).progressbar({value: " . intval($count * 100 / $Tot100) . " })$( \\" . intval($count * 100 / $Tot100) . "+'%')</script>";
        flush();
        ob_flush();
        flush();
        ob_flush();
...
for ($i = 0; $i < $duenovanta["Record_N1"]; $i++) {
    ...
    for ($y = 0; $y < $duenovanta["Record_N2"]; $y++) {
        ...
        echo "<script>$( \"#progressbar\" ).progressbar({value: " . intval($y * 100 / $duenovanta["Record_N2"]) . " })$( \\" . intval($y * 100 / $duenovanta["Record_N2"]) . "+'%')</script>";
