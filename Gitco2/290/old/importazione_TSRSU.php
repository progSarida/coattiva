...
<table class="table_interna text_center" border="0" cellspacing="10" cellpadding="0">
	<tr>
		<td><font class="titolo font16 under_decor">Importazione TSRSU</font></td>
	</tr>
</table>
...
for($k=0;$k<(count($array_importazione));$k++)
{
	...
	$array_utente['C/O'] = $array_importazione[$k]['C/O'];
	$array_utente['EREDE'] = $array_importazione[$k]['EREDE'];
	
	
	$array_residenza['STATO_RESIDENZA'] = strtoupper($array_importazione[$k]['STATO_RESIDENZA']);
	$array_residenza['COMUNE_RESIDENZA'] = strtoupper($array_importazione[$k]['COMUNE_RESIDENZA']);
	...
	if($array_residenza['STATO_RESIDENZA']=="ITALIA" || $array_residenza['STATO_RESIDENZA']=="")
	{
		$trova_cc = new comune(null);
		$array_ritorno = $trova_cc->trovaCC($array_residenza['COMUNE_RESIDENZA'],$array_residenza['CAP_RESIDENZA']);

		if($array_residenza['CC_RESIDENZA']!="")
		{
			$array_residenza['COMUNE_RESIDENZA'] = strtoupper($array_ritorno['Com_Nome']);
	...
	if( $array_tributo['TIPO_TRIBUTO'] == "TSRSU" )
	{
				
		$info_cartella = "Sollecito di pagamento TSRSU anni ";
		$conta_anni = 0;
		$array_tributo['IMPOSTA'] = array();
		for($i=0;$i<count($intestazione);$i++) {
			
			if(strpos($intestazione[$i],"IMPOSTA")!==false && $array_importazione[$k][$intestazione[$i]]!="")
			{
				$explode = explode(" ", $intestazione[$i]);
				if($conta_anni>0)
					$info_cartella.= "/";
				
				$info_cartella.= $explode[count($explode)-1];
				$conta_anni++;
				
				$array_tributo['IMPOSTA'][] = $array_importazione[$k][$intestazione[$i]];
			}

		}
		
		$info_cartella.= " not. il ".from_mysql_date($array_importazione[$k]['DATA_NOTIFICA'])." (Progr. ".$array_importazione[$k]['PROGRESSIVO'].")";
		
		if($array_importazione[$k]['INFO_CARTELLA']=="")
			$array_tributo['INFO_CARTELLA'] = $info_cartella;
		
		$array_tributo['TOTALE_IMPOSTE'] = $array_importazione[$k]['TOTALE_IMPOSTE'];
		...
		$array_tributo['ANNO_RIFERIMENTO'] = substr($array_importazione[$k]['DATA_NOTIFICA'],0,4);
	...
	$query.= "AND Info_Cartella = '".$info_cartella."' LIMIT 1";
	...
	if(count($array_importazione)>0 && $control_salva_ruolo == 0)
	{
		...
		$ruolo->Descrizione = "TSRSU ".$comune_id_ruolo." - ".date('d/m/Y');
	...
	if($id_utente==null)
	{
		...
		if($array_utente['EREDE']=="SI")
		{
			$utente->Data_Morte = date('Y-m-d');
		}
		...
		$toponimo->Comune = $array_residenza['COMUNE_RESIDENZA'];
		$toponimo->Nome = $array_residenza['INDIRIZZO_RESIDENZA'];
		$toponimo->Paese = $array_residenza['STATO_RESIDENZA'];
		...
		$indirizzo->Comune = $array_residenza['COMUNE_RESIDENZA'];
		...
		$indirizzo->Paese = $array_residenza['STATO_RESIDENZA'];
		...
		if($array_utente['C/O'] != "")
		{
			$indirizzo = new indirizzo(null, "rec", $array_residenza['CC_RESIDENZA']);
		
			$indirizzo->Presso = $array_utente['C/O'];
			$indirizzo->CC_Indirizzo = $array_residenza['CC_RESIDENZA'];
			$indirizzo->Cap = $array_residenza['CAP_RESIDENZA'];
			$indirizzo->Comune = $array_residenza['COMUNE_RESIDENZA'];
			$indirizzo->Civico = $array_residenza['CIVICO_RESIDENZA'];
	...
	if( $array_tributo['TIPO_TRIBUTO'] == "TSRSU" )
	{
		...
		$partita->Tipo = "RIFIUTI";
		$partita->Sottotipo = "TSRSU";
		...
		$salta_posizione = 0;
		for($j=0;$j<count($array_tributo['IMPOSTA']);$j++)
		{
			...
			$tributo->Codice_Tributo = "3920";
			$tributo->Imposta = $array_tributo['IMPOSTA'][$j];
			$tributo->Data_Decorrenza_Interessi = date("Y-m-d" ,strtotime( $array_tributo['DATA_NOTIFICA']."+1 month" ));
			$tributo->Tipo_Info = "E";
		...
		if($array_tributo['SPESE_NOTIFICA']!="")
		{
			$tributo = new tributo(null, $CC_importazione);
			...
			$tributo->Info_Cartella = $array_tributo['INFO_CARTELLA'];
			$tributo->Anno_Tributo = $array_tributo['ANNO_RIFERIMENTO'];
			$tributo->Codice_Tributo = "9999";
			$tributo->Imposta = $array_tributo['SPESE_NOTIFICA'];
			$tributo->Data_Decorrenza_Interessi = date("Y-m-d" ,strtotime( $array_tributo['DATA_NOTIFICA']."+1 month" ));
			$tributo->Tipo_Info = "E";
			$tributo->Titolo_Entrata = "";
