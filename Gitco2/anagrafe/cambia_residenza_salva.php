...
$cls_anagr = new cls_anagr();
	...
	$data_ultima_mod = $cls_date->GetDateDB($cls_help->getVar("data_conf"),"IT");
	$error = 0;
	$msg = "";
	...
	if($indirizzo_res != null) {
		...
		//if($data_ultima_mod==null) $data_ultima_mod = date("Y-m-d");
		...
		$a_paramsStorico = array(
			...
			'fields' => array(
				...
				array('name' => 'Data_Fine', 'type' => 'date', 'value' => $cls_date->GetDateDB($data_inizio, "IT")),//date("Y-m-d")
				array('name' => 'Data_Ultima_Modifica', 'type' => 'date', 'value' => $data_ultima_mod)
			)
		);
		...
		if (!empty($cls_help->getVar('ID_via_cap'))) {
			array_push($a_paramsStorico["fields"], array('name' => 'Via_Cap_ID', 'type' => 'int', 'value' => $cls_help->getVar('ID_via_cap')));
		}
		/** GV 20/06/2022 END */
...
//echo "<h1>ID: ".$ID_via."</h1>";
	...
	$a_paramsIndirizzo = array(
			...
			'fields'=> array(
					...
					array(  'name' => 'Data_Inizio_Residenza',  'type' => 'date',   'value' => $cls_date->GetDateDB($cls_help->getVar('data_res'),"IT")),//date("Y-m-d"))
					array(  'name' => 'Utente_ID',              'type' => 'int',    'value' => $ID),
					array(  'name' => 'Data_Conferma_Indirizzo','type' => 'date',    'value' => null)
			),
 ...
 */
	if(!empty($cls_help->getVar('ID_via_cap')))
	{
		array_push($a_paramsIndirizzo["fields"], array(  'name' => 'Via_Cap_ID', 'type' => 'int', 'value' => $cls_help->getVar('ID_via_cap')));
	}
/** GV 20/06/2022 END */
	//echo "<h1>First ".$civico_residenza." - ".$interno_residenza."</h1>";
	...
	switch($invia)
	{
		case "Update":
			...
			if(!$cls_db->DbSave($a_paramsIndirizzo))
			{
				...
				$storico->insRow('U',"Modificata residenza per l'utente ".$msg_name." (".$comune_id.") per ".$ente['Denominazione']."[".$c."]");
