...
if ($invia == "Delete") {
    ...
    $cognome = $cls_help->getVar('cognome_utente');
    $nome = $cls_help->getVar('nome_utente');
	...
	if (!$cls_db->Delete("utente", "ID = '" . $ID . "' AND CC_Comune ='" . $c . "' AND Comune_ID ='" . $comune_id . "'")) {
        ...
        if($genere == 'D')
            $storico->insRow('D',"Eliminato utente ".$ditta." (".$comune_id.") per ".$ente['Denominazione']."[".$c."]");
        else
            $storico->insRow('D',"Eliminato utente ".$cognome." ".$nome." (".$comune_id.") per ".$ente['Denominazione']."[".$c."]");
...
if ($invia == "Salva") {
	...
	if ($genere == 'D') {
		$ditta = $cls_help->getVar('ditta');
        $msg_name = $cls_help->getVar('ditta');
        ...
        $msg_name = $cognome_utente." ".$nome_utente;
	...
	switch ($invia) {
		case "Insert":
            ...
            if($flag1 && $flag2){
				$comune_id_ = $comune_id+1;
				$storico->insRow('I',"Inserito utente ".$msg_name." (".$comune_id_.") per ".$ente['Denominazione']."[".$c."]");
			}
			break;
		...
		case "Update":
            ...
            if($flag1 && $flag2)
                $storico->insRow('U',"Modificato utente ".$msg_name." (".$comune_id.") per ".$ente['Denominazione']."[".$c."]");
			break;
	}
}
