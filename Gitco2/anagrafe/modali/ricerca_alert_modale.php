...
switch ($richiesta)
{
	...
	case ("ricCap"):
		...
		$comune_cap = get_var('pc');
		$via_cap = get_var('via_ric');
		$CC = get_var('pCC');
		$tipoRicInd = get_var('tipoRicInd');
...
if( $posted == true )
{
	$varie_cercate = get_var('varie_cercate');
	$nome_col = get_var('nome_col');
...
if (mysql_num_rows($resultContr)==0)
...
while($contr = mysql_fetch_array($resultContr, MYSQL_ASSOC))
      {
      	$add_tag = "";
      	if($contr['Genere']=='D')
      	{
         ...
         if($j==0)
         	$add_tag = "tabindex=1";
         else if($j == mysql_num_rows($resultContr) - 1)
         	$add_tag = "onblur=\"blurLast();\"";
?>
	<tr <?php echo $stile_riga ?>>
    	<td align=center>
    		<input type=image src="/gitco2/immagini/select.png" id=tab_<?php echo $j+1 ?> <?php echo $add_tag; ?> style="width:25px; height:25px; border:0;"
...
while($CDF_trovato = mysql_fetch_array($resultCF, MYSQL_ASSOC))
	{
		$add_tag = "";
		if($CDF_trovato['Genere']=='D')
         ...
         if($j==0)
         	$add_tag = "tabindex=1";
         else if($j == mysql_num_rows($resultCF) - 1)
         	$add_tag = "onblur=\"blurLast();\"";
?>
	<tr <?php echo $stile_riga ?>>
    	<td width=5% align=center>
    	<input type=image src="/gitco2/immagini/select.png" id=tab_<?php echo $j+1 ?> <?php echo $add_tag; ?> style="width:25px; height:25px; border:0;" title="Clicca qui per inserire l'utente"
...
<?php while($paese_trovato = mysql_fetch_array($resultPaese, MYSQL_ASSOC))
{
		$add_tag = "";
...
?>
	...
	<tr <?php echo $stile_riga; ?>>
		<td width=5% align=center>
		<input type=image src="/gitco2/immagini/select.png" id=tab_<?php echo $j+1 ?> <?php echo $add_tag; ?> style="width:25px; height:25px; border:0;" title="Clicca qui per inserire il paese"
...
<?php
        while($cap_trovato = mysql_fetch_array($resultCap, MYSQL_ASSOC))
        {
        	$add_tag = "";
...
<?php
        while($via_trovata = mysql_fetch_array($resultVia, MYSQL_ASSOC))
        {
        	$add_tag = "";
...
case ('ricGruppo'):
	...
	if( $posted == true )
	{
if ($num_varie==0){	echo"<script>alert('Non � stato trovato alcun risultato.');self.close();</script>"; }
else
{
	$j = 0;
	$i = 0; // contatore : serve per identificare righe pari e righe dispari
...
<?php
        while( $dettagli_trovati = mysql_fetch_array($resultGruppo, MYSQL_ASSOC) )
        {
        	$add_tag = "";
...
?>
	...
	<tr <?php echo $stile_riga; ?>>
		...
		<input type=image src="/gitco2/immagini/select.png" id=tab_<?php echo $j+1 ?> <?php echo $add_tag; ?> style="width:25px; height:25px; border:0;" title="Clicca qui per inserire il valore"
