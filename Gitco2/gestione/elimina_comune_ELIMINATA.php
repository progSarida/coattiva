...
//$layout = "<script>$('.ente_5').hide();focusIndex();</script>";
...
F5_button = function()
{
	location.href="elimina_comune.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>&servizio=<?php echo $servizio; ?>";
}
...
//$(document).ready(function(){
	...
	function callParent(value)
	{
		...
		{
			$('#comune').val(value.comune);
			$('#CC').val(value.CC);
		}
	}

	/*window.callParent= function (value) {
...
function ricerca_comune()
{
	//strDim = Dim_Alert(600, 300);
	var stringa = "<?=WEB_ROOT; ?>/search/ente/ricerca_ente.php?richiesta=ricComune";
...
/*$(document).ready(function(){
	...
	$('#form_crea').ajaxForm(
		...
		{
			alert('Ente eliminato correttamente!');
			annulla();

		}
		else if(array_ritorno[0]=='ERROR')
		{
			alert('Eliminazione ente fallita!');
		}
...
<table class="table_interna text_center" border="0" cellspacing="10" cellpadding="0">
	<tr>
		<td ><font class="titolo font16 under_decor">Cancellazione ente</font></td>
	</tr>
</table>
<br>

<form name=form_crea id=form_crea method=post action="crea_comune_salva.php">
...
<table class="table_interna text_center" border="0" cellspacing="4" cellpadding="0">
	<tr>
		...
		<td class="text_left width25">Ente</td>
		<td class="text_left width25">
			<input class=sfondo_azzurro type=text name=comune readonly id=comune size=25 value="<?php echo $nome_com; ?>" ondblclick="ricerca_comune();" tabindex=1>
		</td>
		<td class="text_left width25"></td>
	</tr>
</table>
