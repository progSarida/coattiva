...
else if( $num_enti > 1 )
{
	...
	if(strlen($num_unione)==1) 			$newCC = "U00".$num_unione;
	else if(strlen($num_unione)==2)		$newCC = "U0".$num_unione;
	else if(strlen($num_unione)==3)		$newCC = "U".$num_unione;
...
header("Location: crea_anno.php?c={$c}&a={$a}&servizio={$servizio}&newCC={$newCC}");
//if($control_parametri==1)
//{
	/*$query = "SELECT * FROM parametri_annuali WHERE CC = '".$nuovoCodice."' AND Anno = '".date('Y')."' AND Tipo_Riscossione = '*****'";
	$par_annuali = $cls_db->getArrayLine($cls_db->ExecuteQuery($query));
	...
	{
		$error = 1;
		$msg = "Errore inserimento ati non riuscito";
	}*/
...
//		mysql_query('ROLLBACK');
	...
	mysql_query('BEGIN');
	$control = $tariffe_coazione->crea_tariffe_base( $nuovoCodice );
