...
if( $num_enti == 1 )
{
	...
	if( $control_ente > 0 )
	{
		...
		{
			$storico->insRow('I',"Inserito ente comune di ".$comune_1." [".$CC_1."]");
			$control_parametri = 1;
	...
	{
		$storico->insRow('I','Inserito ente comune di '.$comune_1);
		$control_parametri = 1;
		$msg = "Inserimento dati riuscito";
	}
...
else if( $num_enti > 1 )
{
	...
	$value_crea = array();
	$comuni = array();
	$elenco = " per i comuni di";
	$comuni[] = $comune_1." [".$CC_1."]";
	$elenco.=" ".$comune_1;
	...
	if( $num_enti > 1 )
	{
		$comuni[] = $comune_2." [".$CC_2."]";
		$elenco.=", ".$comune_2;
		$cod_unione = $CC_1 ."/". $CC_2;
	}
	if( $num_enti > 2 )
	{
		$comuni[] = $comune_3." [".$CC_3."]";
		$elenco.=", ".$comune_3;
		$cod_unione .= "/" . $CC_3;
	}
	if( $num_enti > 3 )
	{
		$comuni[] = $comune_4." [".$CC_4."]";
		$elenco.=", ".$comune_4;
		$cod_unione .= "/" . $CC_4;
	}
	if( $num_enti > 4 )
	{
		$comuni[] = $comune_5." [".$CC_5."]";
		$elenco.=", ".$comune_5;
		$cod_unione .= "/" . $CC_5;
	}
	...
	{
		for($i=0;$i<$num_enti;$i++){
			$storico->insRow('I',"Inserito comune ".$comuni[$i]." sotto l'ente ".$denominazione." [".$new_cod."]");
		}
		$storico->insRow('I','Inserito ente '.$denominazione." [".$new_cod."]".$elenco);
		$control_parametri = 1;
		$msg = "Inserimento dati riuscito";
	}
...
if($control_parametri==1)
{
	$a_paramsAnno = array(
			'table' => 'anni_gestiti',
			'fields'=> array(
					array(  'name' => 'CC_Anno',        'type' => 'string', 'value' =>  $nuovoCodice),
...
else if( $invia == "Delete" )
{
	...
	if(!$cls_db->Delete("enti_gestiti","CC = '".$CC."'"))
	{
		...
		$storico->insRow('D','Eliminato ente '.$denominazione);
		$msg = "Dati eliminati con sucesso";
	}
...
//		mysql_query('ROLLBACK');
	...
	$query = "INSERT INTO coefficiente_coazione (CC, Percentuale, Credito_Minimo, Credito_Massimo) SELECT '".$nuovoCodice."',Percentuale, Credito_Minimo, Credito_Massimo FROM coefficiente_coazione WHERE CC = '*****';";
	...
	{
		$error = 1;
		$msg = "Errore! Inserimento coefficienti non riuscito";
	}
