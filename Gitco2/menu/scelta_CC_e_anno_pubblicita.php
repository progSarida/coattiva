	...
	if ($tiporicerca == "comune")
	{
		if ($autorizzazione == 1)
		{
			$query = "select DISTINCT CC, Denominazione from enti_gestiti, anni_gestiti
					WHERE CC_Anno = CC AND Gestione_Pubblicita = 'Y'
					order by Denominazione ASC";
		}
		else
		{
			$query = "select DISTINCT CC, Denominazione from enti_gestiti, anni_gestiti
					where CC!='XXXX' and CC!='ZZZZ' AND CC_Anno = CC AND Gestione_Pubblicita = 'Y'
					order by Denominazione ASC";
		}
...
function ElencoQuiAnni ($tiporicerca, $comuneselected, $annoselected)
{
	...
	if ($comuneselected == "ZZZZ" || $comuneselected == 'YYYY' || $comuneselected == 'XXXX')
	{
	...
	else
	{
		...
		$query ="select Anno from anni_gestiti where CC_Anno ='$comuneselected' and Gestione_Pubblicita='Y' order by Anno DESC";
		...
		if ($res == NULL)
		{
		    ...
		    //echo "<script>location.href('../../gestione/crea_anno.php?c=$comuneselected&servizio=PUBBLICITA');<script>";
		}
		else
		{
			echo <<< LISTAANNI
...
function cambiocomune ()
{
	...
	var cerco = "anno";
	strLink = "scelta_CC_e_anno_pubblicita.php?";
	strLink += "c=" + c;
	strLink += "&a=" + a;
	strLink += "&cerco=" + cerco;
...
function confermascelte()
{
	var c = $("#selectcomune").val();
	var a = $("#selectanno").val();
	var autorizz = <?php echo $autorizzazione; ?>;
	strLink = "home_pubblicita.php?";
	strLink += "c=" + c;
	strLink += "&a=" + a;
	strLink1 = "../gestione/crea_anno.php?";
	strLink1 += "c=" + c;
	strLink1 += "&servizio=PUBBLICITA";
	if(a==undefined && autorizz==1)
	{
		location.href = strLink1;
	}
	else if(a==undefined && autorizz==2)
	{
		alert("Non è possibile creare anni da gestire con un utenza limitata!")
		return;
	}
	if(a.length!=4)
	{
...
{
    document.getElementById("selectcomune").focus();
}

</script>
</head>
<body class="sfondo_new_gitco" onload='setFocusToTextBox()'>
...
<table class="table_azzurra text_center" style="height:92%;">
	<tr height=6%>
		...
		<td align=right width=15%>
			<a onMouseover="title='Torna al login'" href="../../index_pubblicita.php?c=<?php echo $c?>&a=<?php echo $a?>" target="_top">
