...
<script>
    ...
    function getSubmenuPagename(number){
        switch(number){
            ...
            case 9: pagename = "annulamento_sgravi"; break;
        }
        return pagename;
    }
    ...
    F7_button = function(){
        if( checkModify() )
            openLocation(pageName, "partita="+prevID+"&p="+taxPayer);
    }
    ...
    F8_button = function(){
        if( checkModify() )
            openLocation(pageName, "partita="+nextID+"&p="+taxPayer);
    }
    ...
    function processJson(a_backValue) {
        ...
        if (a_backValue['response'] === true) {
            top.location.href = thisPage+"?partita=" + a_backValue['ID'] + "&c="+ a_backValue['c'] +"&a=" + a_backValue['a'];
        }
        else{
            alert('Codice partita non trovato!');
            openLocation(pageName,"partita="+partitaID);
        }
    }
...
<table class="table_interna text_center" border=0 style="border:2px solid #6D95D5;">
    <tr>
        ...
        <td class="text_center width15"><span class="titolo font18">PARTITA</span><span class="titolo font14"><br> Pag <?php echo $submenuPageNo; ?>/9</span></td>
        ...
        <td class="text_center width15">
            ...
            <a href="#" onclick="openSubLink(getSubmenuPagename(9));" style="text-decoration: none;">
                <span id="spanPage9" class="titolo font15">Annullamento/Sgravi</span>
            </a>
            <?php } ?>
        </td>
        <td class="text_center width40"><em style="background-color:rgb(251,255,208);font-style : normal ;"><?php echo $denominazioneUtente; ?></em></td>
        <td class="text_left">
            <a onMouseover="title='Gestione Ruolo'" href="#" style="text-decoration:none;" onClick="openRuolo();" >
