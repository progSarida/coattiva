...
if (!isset($_SESSION['Popup']))
{
     ...
     if($_SESSION['aut_tipo']==20)
            include_once INC."/menu_div_ridotto.php";
        else
            include_once INC."/menu_div.php";
...
<script>
    ...
    //F12
    function F12_button(){
        top.location.href = "<?php echo WEB_ROOT; ?>/home.php?&c=<?php echo $c; ?>&a=<?php echo $a; ?>";
        return false;
    }
    ...
    function switchMenuImg(button){
        ...
        if(a_src[a_src.length-1]=="on.png")
            src = button+"_off.png";
        else if(a_src[a_src.length-1]=="off.png")
            src = button+"_on.png";

        selector.attr("src","<?php echo IMG; ?>"+"/"+src);
    }

    function checkMenuImg(button){
        selector = $('#'+button+'_img');
        var src = selector.attr("src");
        var a_src = src.split("_");
        if(a_src[a_src.length-1]=="on.png"){
...
<table class=" text_center" style="margin: 0;padding: 0;">
    <tr>
...
<div id='alertErrSucc' class="alertMsg text_center"><p id='textErrSucc' style="font-weight: bold; height:22px;margin: 0;padding: 0;"></p></div>

<script type='text/javascript'>ShowAlert('<?=$cls_help->getVar('error');?>','<?=$cls_help->getVar('msg');?>');</script>
