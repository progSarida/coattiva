...
class cls_elaborationTest
{
    ...
    public function __construct($a_params = null, $test=false)
    {
        if (!is_null($a_params))
        ...
        $this->test = $test;
        if($this->test===true){
            $this->allPositionStutus();
        }

    }

    public function allPositionStutus(){
        $query = "SELECT Id, Name FROM position_status";
        $this->a_positionParams = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query),'array','Id');
    }

    public function showControlOutcome($controlStr, $a_position){
        if($a_position['position_status']==1)
            echo $controlStr.": OK<br><br>";
        else
            echo $controlStr.": ".$this->a_positionParams[$a_position['position_status']]['Id']." - ".$this->a_positionParams[$a_position['position_status']]['Name']."<br><br>";
    }
    ...
    public function checkPagamentoConPignoramento()
    {
        ...
        if(!$this->checkPignoramentoAnnullato() && !empty($this->a_checkPartita['Totale_Dovuto_PG'])){
            ...
            var_dump($tot_pagamenti);
            var_dump($this->a_checkPartita['TOTALE_PAGAMENTI']);
            var_dump($this->a_checkPartita['TOTALE_PAGAMENTI_PG']);
    ...
    public function getResidual($a_checkPartita){

        if(is_null($a_checkPartita['Totale_Dovuto_ATTO']))
            return false;

        if (is_null($a_checkPartita['TOTALE_PAGAMENTI']))
            $a_checkPartita['TOTALE_PAGAMENTI'] = 0;

        if(!is_null($a_checkPartita['Data_Notifica_Atto'])){
            $data_not = new cls_DateTime($a_checkPartita['Data_Notifica_Atto'],"DB");
            if($data_not->CompareDate("DB",">",$a_checkPartita['Data_Sgravio']))
                return (float)$a_checkPartita['Totale_Dovuto_Atto_Iniziale'] + (float)$a_checkPartita['Diritto_Riscossione_Minimo_ATTO'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];
        }
        else return (float)$a_checkPartita['Totale_Dovuto_Atto_Iniziale'] + (float)$a_checkPartita['Diritto_Riscossione_Minimo_ATTO'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];

        

        if(!is_null($a_checkPartita['ID_CRONOLOGICO_PG']) && $a_checkPartita['Stato_Pignoramento']!="Annullato" && $a_checkPartita['Stato_Pignoramento']!="Archiviato"){
            
            $date = new cls_DateTime($a_checkPartita['Data_Elaborazione_Pignoramento'],"DB");
            
            if($date->CompareDate("DB","<",$a_checkPartita['Data_Elaborazione_ATTO']) /*$a_checkPartita['Data_Elaborazione_Pignoramento']<$a_checkPartita['Data_Elaborazione_ATTO']*/)
            {
                if(is_null($a_checkPartita['Rate_Previste_Atto']) || ((int)$a_checkPartita['Rate_Previste_Atto'] === 0 && $a_checkPartita['Rate_Previste_Atto'] !== null) )
                    $totale = (float)$a_checkPartita['Totale_Dovuto_ATTO'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];
                else{
                    $data_not = new cls_DateTime($a_checkPartita['Data_Notifica_Atto'],"DB");
                    $data_not->AddDay(60);
                    if($data_not->CompareDate("DB","<",$a_checkPartita['Data_Richiesta_Rate_Atto']))
                        $totale = (float)$a_checkPartita['Totale_Dovuto_Atto_Iniziale'] + (float)$a_checkPartita['Diritto_Riscossione_Massimo_ATTO'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];
                    else 
                    {
                        $num_rate = $a_checkPartita['Rate_Previste_Atto'];
                        ...
                        for($i = 0,$a=1; $i < $num_rate; $i++,$a++){
                            ...
                            if($data_rate->CompareDate("DB","<",$a_checkPartita['Data_Sgravio']))
                            {
                        ...
                        if($flagAllPayment){
                            $totale = (float)$a_checkPartita['Totale_Dovuto_Atto_Iniziale'] + (float)$a_checkPartita['Diritto_Riscossione_Minimo_ATTO'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];
                        }
                    }
                }
            }
            ...
            {
                $totale = (float)$a_checkPartita['Totale_Dovuto_PG'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];
            }
        }
        else{


            //var_dump((int)null);die;

            if(is_null($a_checkPartita['Rate_Previste_Atto']) || ((int)$a_checkPartita['Rate_Previste_Atto'] === 0 && $a_checkPartita['Rate_Previste_Atto'] !== null) ){
                $totale = (float)$a_checkPartita['Totale_Dovuto_ATTO'] - (float)$a_checkPartita['TOTALE_PAGAMENTI'];
                //if($a_checkPartita['Partita_ID'] == 21){
                    //var_dump($a_checkPartita['Totale_Dovuto_ATTO']." --- ".$a_checkPartita['TOTALE_PAGAMENTI']);die;
                //}
            }       
            else{
    ...
    public function getPositionStatus($a_checkPartita)
    {
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkPagamentoConPignoramento", $a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkDeath",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkBloccoCoazione",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkFlagSospensione",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkFlagCoobbligati",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkAppeal",$a_return);
        }
         ...
         if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkPartitaExpiration",$a_return);
         }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkAttoDaElaborareConPignoramentoAttivo",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkRateizzazioneConPignoramento",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkRettifica",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkRielabora",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkDataNotificaConPignoramento",$a_return);
        }
         ...
         if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkPresenzaIngiunzione",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkActExpiration",$a_return);
        }
        ...
        if ($a_return['position_status'] == 1) {
            ...
            if($this->test)
                $this->showControlOutcome("checkSollecitoExpiration",$a_return);
        }
