<?php
	...
	$salva->Importo_Riscontro = conv_num(get_var('importo_riscontro'));
	
	$salva->Data_Deposito = to_mysql_date(get_var('data_deposito'));
	$salva->Stato_Deposito = get_var('stato_deposito');
	$salva->Data_Vendita = to_mysql_date(get_var('data_vendita'));
	$salva->Stato_Vendita = get_var('stato_vendita');
	$salva->Prezzo_Vendita = conv_num(get_var('prezzo_vendita'));
