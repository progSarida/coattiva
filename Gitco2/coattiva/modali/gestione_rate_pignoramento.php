...
for( $i=0; $i<count($terzi_id); $i++ )
{
    ...
    $where_notifica_terzo = "CC = '".$c."' AND Atto_Notificato_ID = '".$pignoramento["Presso_Terzi"][$i]["Pignoramento_ID"]."' AND ID_Collegamento = '".$pignoramento["Presso_Terzi"][$i]["ID"]."'";
    ...
    $result_2 = $cls_db->getResults($cls_db->ExecuteQuery($query));

    $pignoramento["Presso_Terzi"][$i]["Notifiche_Terzo"] = $result_2;
}

/*$query = "SELECT * FROM pignoramento_presso_terzi WHERE ID = '".$pignoramento["Presso_Terzi"][$i]["ID"]."' AND CC = '".$c."'";
$resultPPT = $cls_db->getArrayLineNull($cls_db->ExecuteQuery($query),"pignoramento_presso_terzi");*/
...
$importo = explode("*",$pignoramento->Importi_Rate);
...
{
	$tot_rate += number_format($importo[$i],2);
}
...
$checkTot1 = number_format($totali["Totali_Array"][1]-$importo_bloccato,2,".","");
$checkTot2 = number_format($totali["Totali_Array"][2]-$importo_bloccato,2,".","");
$checkTot3 = number_format($totali["Totali_Array"][3]-$importo_bloccato,2,".","");
$checkTotRate = number_format($tot_rate,2,".","");
