...
if(isset($partita["Atto"][count($partita["Atto"])-1]["data_start_archived_act"]))
    if($partita["Atto"][count($partita["Atto"])-1]["data_start_archived_act"] != null) $data_single_att_blocco = $cls_date->Get_DateNewFormat($partita["Atto"][count($partita["Atto"])-1]["data_start_archived_act"],"DB");
    else $data_single_att_blocco = "";
else $data_single_att_blocco = "";

$FileSingolaArchiviazione = "";
if(isset($partita["Atto"][count($partita["Atto"])-1]["File"])) 
    $FileSingolaArchiviazione = $partita["Atto"][count($partita["Atto"])-1]["File"] != null ? $partita["Atto"][count($partita["Atto"])-1]["File"] : "";
else $FileSingolaArchiviazione = "";
...
$text_motivation_archived = isset($partita["Atto"][count($partita["Atto"])-1]["text_motivation_archived"])?$partita["Atto"][count($partita["Atto"])-1]["text_motivation_archived"]:"";
...
if(isset($partita["Atto"][count($partita["Atto"])-1]["archived"]))
    if($partita["Atto"][count($partita["Atto"])-1]["archived"]=="si")
    {
        //var_dump("qui");
        $layout.= "<script>$('#archived').prop('checked',true);</script>";
        $layout.= "<script>$('#archived_parametri_notifica_id').val('".$partita["Atto"][count($partita["Atto"])-1]["archived_parametri_notifica_id"]."');cambia_title('archived_parametri_notifica_id');</script>";
        //var_dump($layout);

        $noteArchiveDrop = $partita["Atto"][count($partita["Atto"])-1]["archived_parametri_notifica_id"];
    }

    //var_dump($partita["Atto"][count($partita["Atto"])-1]["archived"]=="si");die;
...
if(count($atto)!=0)
{
    ...
    $pathcompletoCAD = IMMAGINI_CAD."/".$c."/";
    //ECHO $pathcompletoAR;
    ...
    if($info_spedizione!=null)
    {
        ...
        if(is_file($pathcompletoAR.$info_spedizione["Immagine_Fronte"]))
        {
        ...
        if(is_file($pathcompletoAR.$info_spedizione["Immagine_Retro"]))
        {
        ...
        if(is_file($pathcompletoCAD.$info_spedizione["CAD_Fronte"]))
        {
            $layout.="<script>$('#CAD_fronte').show();</script>";
            $layout.="<script>$('#CAD_fronte').attr('onclick','apri_notifica(\"".$pathcompletoCAD.$info_spedizione["CAD_Fronte"]."\")')</script>";
        }

        if(is_file($pathcompletoCAD.$info_spedizione["CAD_Retro"]))
        {
            $layout.="<script>$('#CAD_retro').show();</script>";
            $layout.="<script>$('#CAD_retro').attr('onclick','apri_notifica(\"".$pathcompletoCAD.$info_spedizione["CAD_Retro"]."\")')</script>";
        }
    }
    else
        $control_spedizione = "no";
  ...
  */
    ...
    $spese_not_pigno = $ing["Spese_Notifica_Pignoramento"];
    $spese_acc_pigno = $ing["Spese_Accessorie_Pignoramento"];
    ...
    if(!empty($ing["Data_Notifica"]))
        $data_check_pagamenti = $ing["Data_Notifica"];
    else if(!empty($ing["Data_Stampa"]))
        $data_check_pagamenti = $ing["Data_Stampa"];
    else
        $data_check_pagamenti = $ing["Data_Elaborazione"];
    $pagamenti_precedenti = $cls_partita->pagamenti_precedenti($ing["ID"],$ing["Partita_ID"], $data_check_pagamenti);
...
<script>
    ...
    function initialId(type,val){
        switch (type){
            ...
            case "entry":
            case "fore":
                top.location.href="<?= WEB_ROOT; ?>/coattiva/ingiunzione.php?mode=consulta&partita="+val['ID']+"&c=<?php echo $c; ?>&a="+val['Anno_Riferimento'];
                break;
    ...
    F3_button = function(){
        ...
        if(elaboration_id==""){
            if( ultimo_id == atto_corrente )
            {
                ...
                if(($('#flag_blocco').is(":checked") || $('#archived').is(":checked") ) && $('#rielabora').is(":checked"))
                {
                ...
                /*if("<?= $flagRateizzazione; ?>" == "true" && $("#note_annull_rateizzazione").val() == "" && $('#rate_id').not(":checked"))
            {
                ...
                {
                    $("#archived_parametri_notifica_id").addClass("validateCustom vld_Custom_r");
                    $("#text_motivation_archived").addClass("validateCustom vld_Custom_r");
                }
                else {
                    $("#archived_parametri_notifica_id").removeClass("validateCustom vld_Custom_r");
                    $("#text_motivation_archived").removeClass("validateCustom vld_Custom_r");
                }*/
...
<script>
    //alert("qui");
    var tipo_atto = "<?= isset($ing["Atto"])?$ing["Atto"]:""; ?>";
    ...
    var spese_not_pigno = new Array();
    var spese_acc_pigno = new Array();
    var can = new Array();
    var cad = new Array();
    ...
    var CAD_fronte = new Array();
    var CAD_retro = new Array();
    ...
    var archived = new Array();
    var archived_parametri_notifica_id = new Array();
    var text_motivation_archived = new Array();
    var data_start_archived_act = new Array();
    var data_start_archived_act_ita = new Array();
 
    var archived_parametri_notifica_id_old = "<?= $noteArchiveDrop; ?>";
    var text_motivation_archived_old = "<?= $text_motivation_archived; ?>";
    ...
    {
        $js_spedizione = "si";
        if(is_file($pathcompletoAR.$spedizione["Immagine_Fronte"]))
            $js_AR_fronte = $pathcompletoAR.$spedizione["Immagine_Fronte"];
        else
            $js_AR_fronte = "";
        if(is_file($pathcompletoAR.$spedizione["Immagine_Retro"]))
            $js_AR_retro = $pathcompletoAR.$spedizione["Immagine_Retro"];
        else
            $js_AR_retro = "";
        if(is_file($pathcompletoCAD.$spedizione["CAD_Fronte"]))
            $js_CAD_fronte = $pathcompletoCAD.$spedizione["CAD_Fronte"];
        else
            $js_CAD_fronte = "";
        if(is_file($pathcompletoCAD.$spedizione["CAD_Retro"]))
            $js_CAD_retro = $pathcompletoCAD.$spedizione["CAD_Retro"];
        else
            $js_CAD_retro = "";
    }
    else
    ...
    file[<?php echo $y; ?>] = "<?php echo $atto[$y]["File"]; ?>";

    archived[<?php echo $y; ?>] = "<?php echo $atto[$y]["archived"]; ?>";
    archived_parametri_notifica_id[<?php echo $y; ?>] = "<?php echo $atto[$y]["archived_parametri_notifica_id"]; ?>";
    text_motivation_archived[<?php echo $y; ?>] = "<?php echo $atto[$y]["text_motivation_archived"]; ?>";
    data_start_archived_act[<?php echo $y; ?>] = "<?php echo $cls_date->Get_DateNewFormat($atto[$y]["data_start_archived_act"],"DB"); ?>";
    ...
    CAD_fronte[<?php echo $y; ?>] = "<?php echo $js_CAD_fronte; ?>";
    CAD_retro[<?php echo $y; ?>] = "<?php echo $js_CAD_retro; ?>";
    ...
    spese_not_pigno[<?php echo $y; ?>] = "<?php echo number_format($atto[$y]["Spese_Notifica_Pignoramento"],2,",",""); ?>";
    spese_acc_pigno[<?php echo $y; ?>] = "<?php echo number_format($atto[$y]["Spese_Accessorie_Pignoramento"],2,",",""); ?>";
    ...
    <?php
    if(!empty($atto[$y]["Data_Notifica"]))
        $data_check_pagamenti = $atto[$y]["Data_Notifica"];
    else if(!empty($atto[$y]["Data_Stampa"]))
        $data_check_pagamenti = $atto[$y]["Data_Stampa"];
    else
        $data_check_pagamenti = $atto[$y]["Data_Elaborazione"];
    $pag_prec = $cls_partita->pagamenti_precedenti($atto[$y]["ID"], $atto[$y]["Partita_ID"],$data_check_pagamenti);
    ?>

    pagamenti_precedenti[<?php echo $y; ?>] = "<?php echo number_format($pag_prec,2,",",""); ?>";
    if(pagamenti_precedenti[<?php echo $y; ?>]=="")	pagamenti_precedenti[<?php echo $y; ?>] = "0,00";

    tot_dovuto_meno_pag[<?php echo $y; ?>] = "<?php echo number_format($atto[$y]["Totale_Dovuto"]-$pag_prec,2,',',''); ?>";
    ...
    tot_1[<?php echo $y; ?>] = "<?php echo number_format($atto[$y]["Totale_Dovuto"]-$pag_prec+$atto[$y]["Diritto_Riscossione_Minimo"],2, ",",""); ?>";
    tot_2[<?php echo $y; ?>] = "<?php echo number_format($atto[$y]["Totale_Dovuto"]-$pag_prec+$atto[$y]["Diritto_Riscossione_Massimo"],2, ",",""); ?>";
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#blockModal').on('shown.bs.modal', function (e) {
            addClass("archived","vld_Custom_r");
            addClass("archived_parametri_notifica_id","vld_Custom_r");
            addClass("text_motivation_archived","vld_Custom_r");
        });

        $('#blockModal').on('hidden.bs.modal', function (e) { 
            removeClass("archived","vld_Custom_r");
            removeClass("archived_parametri_notifica_id","vld_Custom_r");
            removeClass("text_motivation_archived","vld_Custom_r");
        });
    ...
    switch(type){
      case "0":
      case 0: $('#blockModalLabel').empty().append('<b>Rateizzazione pignoramenti</b> - <b style="color:green !important;">'+msg+'</b>'); break;
      case "1":
      case 1: $('#blockModalLabel').empty().append('<b>Rateizzazione pignoramenti</b> - <b style="color:red !important;">'+msg+'</b>'); break;
      case "2":
      case 2: $('#blockModalLabel').empty().append('<b>Rateizzazione pignoramenti</b> - <b style="color:#F6BE00 !important;">'+msg+'</b>'); break;
      default: $('#blockModalLabel').empty().append('<b>Rateizzazione pignoramenti</b> - <b>'+msg+'</b>'); break;
    }
  ...
  function reset(){
    $('#blockModalLabel').empty().append('<b>Rateizzazione pignoramenti</b>');
  }
...
//            $('#addizionale').val(addizionale[value]);
        ...
        $('#spese_not_pigno').val(spese_not_pigno[value]);
        $('#spese_acc_pigno').val(spese_acc_pigno[value]);
...
//            $('#sanzione_ing').text(sanzione[value]);
        ...
        {
            $('#CAD_fronte').show();
            $('#CAD_fronte').attr('onclick',"apri_notifica('"+CAD_fronte[value]+"')");
        }
        else
        {
            $('#CAD_fronte').attr('onclick','');
            $('#CAD_fronte').hide();
        }
        ...
        {
            $('#CAD_retro').show();
            $('#CAD_retro').attr('onclick',"apri_notifica('"+CAD_retro[value]+"')");
        }
        else
        {
            $('#CAD_retro').attr('onclick','');
            $('#CAD_retro').hide();
        }

        if(archived[value] == "si"){
        ...
        $("#archived_parametri_notifica_id").val(archived_parametri_notifica_id[value]);
        $("#text_motivation_archived").val(text_motivation_archived[value]);
        $("#data_start_archived_act").val(data_start_archived_act[value]);
        $("#printImg").attr("onclick","archivia('"+id_atti[value]+"',2);");
        $("#printShow").attr("onclick","apriPdfArchiviazioneSingola('"+file[value]+"');");

        $("#block_data_single").text("ARCHIVIAZIONE Atto ( "+data_start_archived_act[value]+" )");
        archived_parametri_notifica_id_old = archived_parametri_notifica_id[value];
        text_motivation_archived_old = text_motivation_archived[value];
    }
    ...
    function spese_manuali()
    {
        ...
        spese_not_pigno = $('#spese_not_pigno').val();
        if(spese_not_pigno=="")
            spese_not_pigno = "0,00";
        $('#spese_not_pigno').val(spese_not_pigno);
        spese_not_pigno = parseFloat( spese_not_pigno.replace(",",".") );

        spese_acc_pigno = $('#spese_acc_pigno').val();
        if(spese_acc_pigno=="")
            spese_acc_pigno = "0,00";
        $('#spese_acc_pigno').val(spese_acc_pigno);
        spese_acc_pigno = parseFloat( spese_acc_pigno.replace(",",".") );
        ...
        tot_parziale = spese_can_cad + spese_not + tot_inter_importo +spese_prec + spese_not_pigno + spese_acc_pigno;
...
<script>
    ...
    function carica_notifica(type)
    {
        if(control_notifica_presente == true)
        {
            var tipo_atto = "<?= isset($ing["Atto"])?$ing["Atto"]:""; ?>";
            var stringa = "<?= WEB_ROOT; ?>/coattiva/modali/carica_notifica.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>&partita=<?php echo $partita_ID; ?>&atto_corrente="+atto_corrente+"&stato=0&tipo_atto="+tipo_atto+"&type_file="+type;
            //valorediritorno = window.showModalDialog(stringa,"", strDim);
    ...
    function IsPdf(str)
    {
        return right(str,4)==".pdf";
    }

    function apri_notifica(link)
    {
        if (IsPdf(link))
        {   link = link.replaceAll("/var/www","");
            apri(link);
        }
        else
        {
...
<script>
    ...
    function archivia(value,type)
    {
        if(!$('#flag_blocco').is(":checked"))
        {
            alert("Spuntare il flag di archiviazione dell'atto!");
            return false;
        }
        switch(type){
            case 1:
            ...
            case 2:
            case "2":
                if(text_motivation_archived_old != $("#text_motivation_archived").val() || archived_parametri_notifica_id_old != $("#archived_parametri_notifica_id").val())
                {
                    alert("<?= $text_motivation_archived; ?> --- "+$("#text_motivation_archived").val()+" --- <?= $noteArchiveDrop; ?> --- "+$("#archived_parametri_notifica_id").val());
                    showMessage(2,"Prima di procedere salvare le motivazioni!");
                    return false;
                }
                if($("#text_motivation_archived").val() == "" && $("#archived_parametri_notifica_id").val() == "")
                {
                    showMessage(2,"Prima di procedere specificare la motivazione del blocco!");
                    return false;
                }
                break;
    ...
    function sospendi(value)
    {
        ...
        link = "<?= SUPER_WEB_ROOT; ?>/Gitco2/stampe/sospensione_atto.php?richiesta_singola=si&c=<?php echo $c; ?>&a=<?php echo $a?>&ID_Atto="+value;
        location.href= link;
    }
    ...
    function checkStatus(){
        if($('#flag_blocco').is(":checked"))
        {
            alert("Impossibile sospendere una partita già archiviata!");
            $('#flag_sosp').prop('checked',false);
            return false;
        }
        if(!$('#flag_sosp').is(":checked"))
        {
            if(!confirm("ATTENZIONE! Si sta togliendo la sospensione della partita, proseguire?"))
                $('#flag_sosp').prop('checked',true);
        }
    }
...
<form id=form_ingiunzione name=form_ingiunzione action="ingiunzione_salva.php" method=post style="z-index: 1;" >
    ...
    {?>
        ...
        <table class="text_center table_interna" cellspacing=0 border=0 style="border:1px solid black;">
            ...
            for($i=0; $i<count($atto); $i++)
            {
                ...
                if(!empty($atto[$i]["Data_Notifica"]))
                    $data_check_pagamenti = $atto[$i]["Data_Notifica"];
                else if(!empty($atto[$i]["Data_Stampa"]))
                    $data_check_pagamenti = $atto[$i]["Data_Stampa"];
                else
                    $data_check_pagamenti = $atto[$i]["Data_Elaborazione"];
                $pag_prec = $cls_partita->pagamenti_precedenti($atto[$i]["ID"], $atto[$i]["Partita_ID"],$data_check_pagamenti);

                $dovuto_table = $atto[$i]["Totale_Dovuto"] - $pag_prec;
                $pagato_table = $cls_partita->totale_pagamenti($atto[$i]["ID"], $atto[$i]["Partita_ID"],$c);
                $residuo_table = $atto[$i]["Totale_Dovuto"] - $pag_prec - $pagato_table;
    ...
    //$cls_help->alert($partita['Elaboration_Id']);
        if(!is_null($partita['Elaboration_Id'])){
            ...
            switch($partita['Elaboration_DocumentTypeId'])
            {
                case 7:
                case 8:
                    $elabLink = ELAB_PIGNORAMENTI_LAVORO_WEB."/mgmt_pignoramenti_lavoro.php";
                    break;
                case 22:
                    $elabLink = ELAB_PIGNORAMENTI_WEB."/mgmt_pignoramenti.php";
                    break; 
                case 43:
                    $elabLink = ELAB_PIGNORAMENTI_WEB."/mgmt_estrazioni.php";
                    break; 
              ...
              //nel caso mi trovassi nella elaborazione di tipo estrazione per comuni
              if ($partita['Elaboration_Status_Id']==9999) $elabLink = ELAB_PIGNORAMENTI_WEB."/mgmt_estrazioni.php";
              ?>
              <br/>
            ...
            <div class="form-group">
                ...
                <div class="col-lg-12">
                    ...
                    <tr>
                        ...
                        <th>
                            <input type=button class="btn-primary" id=caricanotifica name=caricanotifica value="Carica Notifica" onclick="carica_notifica('notif');" style="/*display:none;*/">
                        </th>
                        <th>
                            <input type=button class="btn-primary" id=caricanotifica name=caricanotifica value="Carica CAD" onclick="carica_notifica('cad');" style="/*display:none;*/">
                        </th>
                        <th>
    ...
    <div class="row">
        <div class="col col-lg-5 col-lg-offset-1">
            <div class="form-group">
                <label class="col-lg-9 control-label resize" style="text-align: left;"> <p style="float: left;">+ &nbsp;&nbsp;&nbsp;&nbsp; Spese Notifica Pignoramenti Prec.</p><p style="float: right;">&euro;</p></label>
                <div class="col-lg-3">
                    <input class="form-control resize corrige_numero" readonly style="background-color: rgb(153, 204, 255); border: 2px solid black;text-align: right;" id=spese_not_pigno name=spese_not_pigno onchange="spese_manuali();" size=7 value="<?php echo number_format($spese_not_pigno,2,",",""); ?>">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col col-lg-5 col-lg-offset-1">
            <div class="form-group">
                <label class="col-lg-9 control-label resize" style="text-align: left;"> <p style="float: left;">+ &nbsp;&nbsp;&nbsp;&nbsp; Spese Accessorie Pignoramenti Prec.</p><p style="float: right;">&euro;</p></label>
                <div class="col-lg-3">
                    <input class="form-control resize corrige_numero" readonly style="background-color: rgb(153, 204, 255); border: 2px solid black;text-align: right;" id=spese_acc_pigno name=spese_acc_pigno onchange="spese_manuali();" size=7 value="<?php echo number_format($spese_acc_pigno,2,",",""); ?>">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row" style="margin-top: 2%;">
        ...
        <div class="row" style="margin-top: 1%;">
            <div class="col-lg-2 col-lg-offset-1">
                <p style="font-weight: bold;float: left;">Stampa archiviazione partita</p>
                <a onMouseover="title='Stampa archiviazione'" href="#" style="text-decoration:none;float:right;" onClick="archivia('<?php echo $ing["ID"]; ?>',1)" >
            ...
            <div class=" col-lg-4">
                <div class="form-group">
                    <label class="col-lg-4 control-label resize" style="text-align: left;">Motivi archiviazione partita</label>
                    <div class="col-lg-8">
            ...
            <div class="col-lg-1">
                <i title="Per generare l'atto di archiviazione del cliente compilare i campi qui a fianco, salvare, e quindi cliccare sul pulsante dell'archiviazione sempre qui affianco!" style="cursor: pointer;" class="fa fa-info-circle fa-2x text_info" aria-hidden="true"></i>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col col-lg-10 col-lg-offset-1">
                <div class="form-group">
                    <label class="col-lg-2 control-label resize" style="text-align: left;">Motivazione archiviazione partita</label>
                    <div class="col-lg-10">
        ...
        <div class="row" style="margin-top: 1%;">
            <div class="col-lg-2 col-lg-offset-1">
                <p style="font-weight: bold;float: left;">Stampa sospensione partita</p>
                <a onMouseover="title='Stampa sospensione'" href="#" style="text-decoration:none;float:right;" onClick="sospendi('<?php echo $ing["ID"]; ?>')" >
                    <img src="<?= IMMAGINIWEB; ?>/sospensione.png" width=20 height=20 border=0 >
                </a>
            </div>
            <div class=" col-lg-4 ">
                <div class="form-group">
                    <label class="control-label resize col-lg-12">
                        <input type="checkbox" name=flag_sosp id=flag_sosp value="si" onclick="checkStatus();">
                        <b>SOSPENSIONE/Blocco coazione</b><?php echo " ( ".$data_att_sosp." )"; ?>
                    </label>
                </div>
            </div>
            <div class=" col-lg-4">
                <div class="form-group">
                    <label class="col-lg-4 control-label resize" style="text-align: left;">Motivi sospensione partita</label>
                    <div class="col-lg-8">
                        <select id=motivo_sosp name=motivo_sosp class="form-control resize" onchange="cambia_title('motivo_sosp');">
                            <option value=""></option>
            ...
            <div class="col-lg-1">
                <i title="Per generare l'atto di sospensione del cliente compilare i campi qui a fianco, salvare, e quindi cliccare sul pulsante della sospensione sempre qui affianco!" style="cursor: pointer;" class="fa fa-info-circle fa-2x text_info" aria-hidden="true"></i>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col col-lg-10 col-lg-offset-1">
                <div class="form-group">
                    <label class="col-lg-2 control-label resize" style="text-align: left;">Motivazione sospensione partita</label>
                    <div class="col-lg-10">
                        <input class="form-control resize" name="note_sosp" id="note_sosp" value="<?php echo $note_sosp; ?>" >
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-lg-10 col-lg-offset-1">
                <div type="button" style="width:100%;" class="btn btn-primary" onclick="$('#blockModal').modal('show');">Blocco Atto Singolo</div>
            </div>
        </div>
    ...
    <div class="modal-dialog" style="width: 65% !important;height: 85vh !important;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="blockModalLabel" style="color: blue;">Blocco singolo atto</h5>
            ...
            <div class="modal-body" style="height: 72vh !important;">
                <div style="overflow-x:hidden;overflow-y: auto;max-height:70vh;">
                    <form id="blockForm" method="POST" action="">
                        ...
                        <input type="hidden" id="Atto_ID" name="Atto_ID" value="<?php echo $atto_ID; ?>">
                        <input type="hidden" id="data_start_archived_act" name="data_start_archived_act" value="<?= $data_single_att_blocco; ?>" />

                        <div class="row" style="margin-top: 2%;">
                            <div class="col-lg-1 col-lg-offset-10">
                                <i title="Per generare l'atto di archiviazione dell'atto compilare i campi qui a fianco, salvare, e quindi cliccare sul pulsante dell'archiviazione sempre qui affianco!" style="cursor: pointer;" class="fa fa-info-circle fa-2x text_info" aria-hidden="true"></i>
                            </div>
                        </div>
                        ...
                        <div class="row" >
                            <div class="col-lg-1 col-lg-offset-1">
                                <a class="col-lg-12" onMouseover="title='Stampa archiviazione'" style="cursor:pointer;" id="printImg" onclick="archivia('<?php echo $atto_ID; ?>',2);" >
                                    <img src="<?= IMMAGINIWEB; ?>/archiviazione.png" width=20 height=20 >
                                </a>
                            </div>
                            <div class="col-lg-8"><p style="font-weight: bold;">Stampa archiviazione atto</p></div>
                            <div class="col-lg-1">
                        ...
                        <div class="row">
                            <div class="col-lg-4 col-lg-offset-1">
                                <label class="control-label col-lg-12">
                                    <input type="checkbox" name=archived id=archived value="si">
                                    <b id="block_data_single">ARCHIVIAZIONE Atto <?php echo " ( ".$data_single_att_blocco." )"; ?></b>
                                </label>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label class="col-lg-5 control-label" style="text-align: left;">Motivi archiviazione atto</label>
                                    <div class="col-lg-7">
                                        <select id=archived_parametri_notifica_id name=archived_parametri_notifica_id class="form-control" onchange="cambia_title('archived_parametri_notifica_id');">
                                            <option value=""></option>
                                            <?php echo $options_blocco; ?>
                                        </select>
                                    </div>
                        ...
                        <div class="row" style="margin-top: 2%;">
                            <!--<div class="col col-lg-10 col-lg-offset-1">
                                <div class="form-group">
                                    <label class="col-lg-2 control-label" style="text-align: left;">Motivazione archiviazione atto</label>
                                    <div class="col-lg-10">
                                        <input class="form-control" name="text_motivation_archived" id="text_motivation_archived" value="<?php echo $text_motivation_archived; ?>" >
                                    </div>
                                </div>
                            </div>-->
                            ...
                            <div class="col col-lg-10 col-lg-offset-1">
                                <div class="form-group">
                                    <label class="col-lg-3 control-label" style="text-align: left;">Motivazione archiviazione atto</label>
                                    <div class="col-lg-9">
                                        <textarea style="max-width: 100%;" class="text_left form-control" id=text_motivation_archived name=text_motivation_archived rows="6" ><?php echo $text_motivation_archived; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function apriPdfArchiviazioneSingola(fileName){
        if(fileName == "")
            showMessage(2,"File non presente!");
        else
            showFileOnModal("<?= ATTI_WEB.'/'.$c.'/Documenti/'?>"+fileName,"Stampa archiviazione atto singolo");
    }
    ...
    function saveForm(mode){
        ...
        var data_value = $("#blockForm").serialize();

        $.ajax({
            type: "POST",
            async: true,
            url: "ajax/ajax_save_single_block.php",
            dataType: "json",
            data: data_value+"&mode="+mode,
            success: function(response) {
                if(response.error == 0){
                    location.href = "<?= WEB_ROOT ?>/coattiva/ingiunzione.php?c=<?= $c; ?>&a=<?= $a; ?>&partita=<?= $cls_help->getVar('partita'); ?>&p=<?= $cls_help->getVar('p'); ?>&msg="+response.msg+"&error="+response.error;
                }
                else{
