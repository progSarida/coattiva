...
$a_params = $cls_db->getArrayLineNull( $cls_db->SelectQuery( $cls_split->getParametersFromIdQuery($partita["Split_Parameters_ID"]) ),"split_payment_parameters" );
if(!$a_params['id']>0)
...
for($y=0;$y<3;$y++){
    if($countHeader<count($a_order)){
        ...
        if($y==0){
            ...
            $prevPaymentHtml.= '<input style="text-align: right;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" style="" id="importo_pagato_'.$count.'" name="importo_pagato['.$count.']" value="'.number_format($a_prevPayments['Importo'],2,",",".").'"> ';
            $prevPaymentHtml.= '</div>';
        ...
        for($i=0;$i<6;$i++){
            ...
            if(isset($a_order[$countHeader])) {
                $prevPaymentHtml.= '<div class="form-group">';
                $prevPaymentHtml.= '<div style="float: left; width: 70%;">';
                $prevPaymentHtml.= '<input style="text-align: right;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" id="split'.$a_order[$countHeader]['split_number'].'_'.$count.'" name="split'.$a_order[$countHeader]['split_number'].'['.$count.']" ';
            ...
            for($z=0;$z<3;$z++){
                if($countHeader<count($a_order)){
                    ...
                    if($z==0){
                        ...
                        $attiHtml.= '<input readonly id="importo_pagato_'.$count.'" name="importo_pagato['.$count.']" style="text-align: right;background-color: #FFC7C7;border: 1px solid black;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" value="'.number_format($pagamento["Importo"],2,",",".").'"> ';
                        $attiHtml.= '</div>';
                    ...
                    for($k=0;$k<6;$k++){
                        ...
                        if(isset($a_order[$countHeader])) {
                            ...
                            $attiHtml.= '<div class="form-group">';
                            $attiHtml.= '<div style="float: left; width: 70%;">';
                            $attiHtml.= '<input style="text-align: right;" id="split'.$a_order[$countHeader]['split_number'].'_'.$count.'" name="split'.$a_order[$countHeader]['split_number'].'['.$count.']" ';
            ...
            for($z=0;$z<3;$z++){
                if($countHeader<count($a_order)){
                    ...
                    if($z==0){
                        ...
                        $pignoHtml.= '<input readonly id="importo_pagato_'.$count.'" name="importo_pagato['.$count.']" style="text-align: right;background-color: #FFC7C7;border: 1px solid black;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" value="'.number_format($pagamento["Importo"],2,",",".").'">';
                        $pignoHtml.= '</div>';
                    ...
                    for($k=0;$k<6;$k++){
                        ...
                        if(isset($a_order[$countHeader])) {
                            ...
                            $pignoHtml.= '<div class="form-group">';
                            $pignoHtml.= '<div style="float: left; width: 70%;">';
                            $pignoHtml.= '<input style="text-align: right;" id="split'.$a_order[$countHeader]['split_number'].'_'.$count.'" name="split'.$a_order[$countHeader]['split_number'].'['.$count.']" ';
...
for($z=0;$z<3;$z++){
    if($countHeader<count($a_order)){
        ...
        if($z==0){
            ...
            $totaliHtml.= '<input style="text-align: right;background-color: #FFC7C7;border: 1px solid black;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" name="importo_pagato_totali" id="importo_pagato_totali" value="'.number_format($a_totalAmounts['Total_Payments'],2,",",".").'" />';
            $totaliHtml.= '</div>';
        ...
        for($k=0;$k<6;$k++){
            ...
            if(isset($a_order[$countHeader])) {
                ...
                $totaliHtml.= 'style="text-align: right;background-color: #FFC7C7;border: 1px solid black;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" value="'.number_format($a_totalAmounts['Total_Split'.$a_order[$countHeader]['split_number']],2,",",".").'">';
                $totaliHtml.= '</div>';
                $totaliHtml.= '<label class="resize control-label" style="text-align: left; float: right; width: 30%;">&nbsp;&euro;</label>';
                $totaliHtml.= '</div>';
                $countHeader++;
...
$totaleDovutoHtml.= '<input style="text-align: right;background-color: #FFC7C7;border: 1px solid black;" class="form-control resize validateCustom vld_Custom_d vld_Custom_r" readonly value="'.number_format((double)$tot_dovuto,2,",",".").'">';
    ...
    function initialId(type,val){
        switch (type){
            ...
            */
            ...
            case "entry":
            case "fore":
                top.location.href="<?= WEB_ROOT; ?>/coattiva/scorporo_pagamento.php?mode=consulta&partita="+val['ID']+"&c=<?php echo $c; ?>&a="+val['Anno_Riferimento'];
                break;
