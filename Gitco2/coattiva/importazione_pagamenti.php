...
$dir['DaBonificare'] = IMP_PAGAMENTI . "/DaBonificare";
$dir['BackUp'] = IMP_PAGAMENTI . "/BackUp";
$dir['Temp'] = IMP_PAGAMENTI . "/Temp";
...
F5_button = function()
{
    location.href="importazione_pagamenti.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>";
}
...
<div class="row pheight40">
	...
	$handle = opendir(IMP_PAGAMENTI);
	while (($file = readdir($handle)) != false)
	{
		if ($file != "." && $file != ".." && $file != "Thumbs.db")  //  queste sono cartelle
		{
			if (!is_dir(IMP_PAGAMENTI ."/". $file))
			{
	...
	if ($importatutto != "SI")
	{
		...
		for ($k = 0; $k < $contatoreAFZ; $k++)
		{
            ...
            $checkExtraction = $cls_zip->extractZip( IMP_PAGAMENTI ."/". $arrayFileZip[$k],$dir['Temp']);
            if(!$checkExtraction){
	...
	$handle = opendir($dir['Temp']);
	while (($file = readdir($handle)) != false)
	{
		if ($file != "." && $file != ".." && $file != "Thumbs.db")  //  queste sono cartelle
		{
			if (!is_dir($dir['Temp'] ."/". $file))
			{
				$esplEstensione = explode (".", $file);
				$posizEstens = count($esplEstensione) - 1;
				$estensione = strtoupper($esplEstensione[$posizEstens]);
	...
	for ($k = 0; $k < $countContenuto; $k++)
	{
		...
		$imgFronte = trim($contenuto[$k][$avanti++]);
		$imgRetro = trim($contenuto[$k][$avanti++]);
		...
		$tipologiaPagamento = "";
		$myProgrNot = "0";
		$dir['Pagamenti'] = ATTI . "/" . $quintoCcComune . "/Pagamenti";
		$breveTipo = "?";
		$attoCompleto = "";
		$tipoContoCorrente = "";
...
//				break;
			...
			default:
				...
				$dir['Pagamenti'] = $dir['DaBonificare'];
				break;
		}
		...
		if (1)
		{
			if ($imgFronte != "")
			{
				...
				$nuovoNomeFileFronte = $dir['Pagamenti']  . "/". $nomeBreveJpgFronte;
			}
			else 
			{
			...
			if ($imgRetro != "")
			{
				...
				$nuovoNomeFileRetro = $dir['Pagamenti'] . "/". $nomeBreveJpgRetro;
			}
			else 
			{
			...
			if ($imgFronte != "" && file_exists($dir['Temp']."/". $imgFronte))
			{
			...
			if ($imgRetro != "" && file_exists($dir['Temp']."/". $imgRetro))
			{
			...
			if ($importatutto == "SI" && $problemaImg == 0)
			{
				...
				//echo "<br><br>" . $dir['Temp']."/". $imgFronte . " ---> " . $nuovoNomeFileFronte . "<br><br>";
				//echo "<br>" . $dir['Temp']."/". $imgRetro . " ---> " . $nuovoNomeFileRetro;
				
				//da rimettere
				if ($imgFronte != "" && file_exists($dir['Temp']."/". $imgFronte))
				{
					$percorsoImg = $cls_utils->crea_dir($dir['Pagamenti']);
					
					$im = new imagick( $dir['Temp']."/". $imgFronte );
					...
					unlink ($dir['Temp']."/". $imgFronte);
				}
				if ($imgRetro != "" && file_exists($dir['Temp']."/". $imgRetro))
				{
					$percorsoImg = $cls_utils->crea_dir($dir['Pagamenti']);
					
					$im = new imagick( $dir['Temp']."/". $imgRetro );
					
					$im->setImageCompression(Imagick::COMPRESSION_JPEG);
					$im->setImageCompressionQuality(5);
					$im->writeImage( $nuovoNomeFileRetro );
					
					unlink ($dir['Temp']."/". $imgRetro);
				}
...
if ($importatutto == "SI")
{
	...
	$backupCsv = $dir['BackUp'] ."/". $backupCsv;
	
	$backupTxt = date("Y-m-d") . "_" . $myFileTxt;
	$backupTxt = $dir['BackUp'] ."/". $backupTxt;
	
	//da rimettere
	if($myFileCsv!="")
		rename ($dir['Temp']."/". $myFileCsv, $backupCsv);
	if($myFileTxt!="")
		rename ($dir['Temp']."/". $myFileTxt, $backupTxt);

	
	
	for ($p = 0; $p < count($arrayFileZip); $p++)
	{
		unlink (IMP_PAGAMENTI ."/". $arrayFileZip[$p]);
	}
