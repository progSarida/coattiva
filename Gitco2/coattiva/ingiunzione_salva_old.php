<?php
	...
	switch($invia)
	{
      ...
      $data_protocollo = $cls_date->GetDateDB($cls_help->getVar('data_protocollo'),"IT");
			...
			if($data_notifica=="")	$data_notifica = "0000-00-00";
			...
			if($rateizza == "rateizza")
			{
				...
				if( $num_rate!=null && $control_rate == 0 )
				{
					...
					if($cls_date->Get_DateNewFormat($data_notifica,"DB")=="")
					{
						...
						$salva["A_Mani"] = utf8_decode($result['A_Mani']);
						if( $data >= $result['A_Mani_Data'] && $result['A_Mani_Data'] != null )
						{
							$salva["A_Mani"] = utf8_decode($result['A_Mani_New']);
						}
			...
			else
			{
				...
				$salva["Importi_Rate"] = "";
				$salva["Scadenze_Rate"] = "";
				$salva["Data_Richiesta_Rate"] = "0000-00-00";
			...
			{
				if($data_stampa == null || $data_stampa == '0000-00-00')
				{
		...
		case "rate":
			...
			case "Interessi":
				...
				$salva->Totale_Dovuto = $salva->Totale_Dovuto - $salva->Interessi - $salva->Interessi_Precedenti;
				$salva->Interessi = 0;
			...
			case "Sanzione":
				...
				$salva->Totale_Dovuto = $salva->Totale_Dovuto - $salva->Sanzione;
				$salva->Sanzione = 0;
