...
$crisis_tools_id = $cls_help->getVar("Crisis_ID");

if($crisis_tools_id==null)
    $crisis_tools_id = 0;
$cls_crisis_tools = new cls_crisis_tools();
$result = $cls_db->getArrayLine($cls_db->SelectQuery($cls_crisis_tools->getCrisis_query($crisis_tools_id)));

if($result!=null) $a_crisis_tools = $result;
else $a_crisis_tools = array('');



$changeAuthority = $cls_help->getVar("changeAuthority");
$lastCrisis = $cls_help->getVar("lastCrisis");
if($lastCrisis==null)
    $lastCrisis = 0;
$a_lastCrisis = null;
//$countCrisis = count($a_crisis_tools);
if($result==null){
    $a_lastCrisis = $cls_db->getArrayLine($cls_db->SelectQuery($cls_crisis_tools->getCrisis_query($lastCrisis)));

    $a_crisis_tools = $cls_db->getColumnsArray("crisis_tools");
    $a_crisis_tools['ID'] = $crisis_tools_id;
    $a_crisis_tools['Start_Date'] = date("d/m/Y");
    $a_crisis_tools['Authority_Description'] = "";
    $a_crisis_tools['Body_Part'] = 2;
    $a_crisis_tools['Trespassers_Part'] = 1;

    $a_crisis_toolsPart = array();
    $a_courtHearing = array();

    $a_proceedings[0] = $cls_db->getColumnsArray("crisis_tools_proceedings_status");
    $a_proceedings[1] = $cls_db->getColumnsArray("crisis_tools_proceedings_status");

    $a_lawyerBill[0] = $cls_db->getColumnsArray("crisis_tools_lawyer_bill");
    $a_lawyerBill[1] = $cls_db->getColumnsArray("crisis_tools_lawyer_bill");

    $a_crisis_tools['Challenge_Description'] = "";

    if($a_lastCrisis!=null && $changeAuthority=="y"){
        $a_crisis_tools['Type'] = $a_lastCrisis['Type'];
        $a_crisis_tools['Amendment_Date'] = $cls_help->toItalianDate($a_lastCrisis['Amendment_Date']);
        $a_crisis_tools['Notification_Date'] = $cls_help->toItalianDate($a_lastCrisis['Notification_Date']);
        $a_crisis_tools['Registration_Date'] = $cls_help->toItalianDate($a_lastCrisis['Registration_Date']);
        $a_crisis_tools['Dossier_Submission_Date'] = $cls_help->toItalianDate($a_lastCrisis['Dossier_Submission_Date']);
        $a_crisis_tools['Trespassers_Part'] = $a_lastCrisis['Trespassers_Part'];
        $a_crisis_tools['Trespassers_Lawyer'] = $a_lastCrisis['Trespassers_Lawyer'];
        $a_crisis_tools['Trespassers_Lawyer_Bar'] = $a_lastCrisis['Trespassers_Lawyer_Bar'];
        $a_crisis_tools['Body_Type'] = $a_lastCrisis['Body_Type'];
        $a_crisis_tools['Body_Part'] = $a_lastCrisis['Body_Part'];
        $a_crisis_tools['Body_Lawyer'] = $a_lastCrisis['Body_Lawyer'];
        $a_crisis_tools['Body_Lawyer_Bar'] = $a_lastCrisis['Body_Lawyer_Bar'];
        switch($a_lastCrisis['Authority_Type']){
            case "giudice":         $a_lastCrisis['Authority_Type'] = "del Giudice di Pace"; break;
            case "tribunale":       $a_lastCrisis['Authority_Type'] = "del Tribunale";   break;
            case "comm_trib_prov":  $a_lastCrisis['Authority_Type'] = "della Commissione Tributaria Provinciale";  break;
            case "comm_trib_reg":   $a_lastCrisis['Authority_Type'] = "della Commissione Tributaria Regionale";    break;
            case "appello":         $a_lastCrisis['Authority_Type'] = "della Corte d'Appello"; break;
            case "cassazione":      $a_lastCrisis['Authority_Type'] = "della Corte di Cassazione"; break;
        }
        $a_crisis_tools['Notes'] = "Rimessione nei termini del ricorso a causa di incompetenza ".$a_lastCrisis['Authority_Type']." di ".$a_lastCrisis['Authority_Place']." ".$a_lastCrisis['Judge'];
        $a_crisis_toolsPart = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCrisisPart_query($lastCrisis)));
    }
}
else{
    $a_crisis_toolsPart = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCrisisPart_query($crisis_tools_id)));
    $a_courtHearing = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCourtHearing_query($crisis_tools_id)));
    $a_proceedings = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getProceedings_query($crisis_tools_id)));
    $a_lawyerBill = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getLawyerBill_query($crisis_tools_id)));

    $a_crisis_tools['Start_Date'] = $cls_help->toItalianDate($a_crisis_tools['Start_Date']);
    $a_crisis_tools['End_Date'] = $cls_help->toItalianDate($a_crisis_tools['End_Date']);
    switch($a_crisis_tools['Authority_Type']){
        case "giudice":         $a_crisis_tools['Authority_Type'] = "Giudice di Pace"; break;
        case "tribunale":       $a_crisis_tools['Authority_Type'] = "Tribunale";   break;
        case "comm_trib_prov":  $a_crisis_tools['Authority_Type'] = "Commissione Tributaria Provinciale";  break;
        case "comm_trib_reg":   $a_crisis_tools['Authority_Type'] = "Commissione Tributaria Regionale";    break;
        case "appello":         $a_crisis_tools['Authority_Type'] = "Corte d'Appello"; break;
        case "cassazione":      $a_crisis_tools['Authority_Type'] = "Corte di Cassazione"; break;
    }
    $a_crisis_tools['Authority_Description'] = $a_crisis_tools['Authority_Type']." - ".$a_crisis_tools['Authority_Place'];
    if($a_crisis_tools['Authority_Section']!="")
        $a_crisis_tools['Authority_Description'].= " sez. ".$a_crisis_tools['Authority_Section'];

    $a_crisis_tools['Amendment_Date'] = $cls_help->toItalianDate($a_crisis_tools['Amendment_Date']);
    $a_crisis_tools['Notification_Date'] = $cls_help->toItalianDate($a_crisis_tools['Notification_Date']);
    $a_crisis_tools['Registration_Date'] = $cls_help->toItalianDate($a_crisis_tools['Registration_Date']);
    $a_crisis_tools['Dossier_Submission_Date'] = $cls_help->toItalianDate($a_crisis_tools['Dossier_Submission_Date']);
}
...
if($partita_ID!=null){
    ...
    if($a_crisis_tools['ID']>0){
        $courtLevelInput = "<input type=hidden name='Court_Level' value='".$a_crisis_tools['Court_Level']."'><span class='titolo'>".$a_crisis_tools['Court_Level']."&deg;</span>";
    }
    else{
        if($changeAuthority=="y")
            $courtLevel = $a_crisis_tools['Court_Level'];
        else
            $courtLevel = $a_crisis_tools['Court_Level']+1;

        if($a_crisis_tools==null && $courtLevel>1 && $changeAuthority!="y"){
            $a_lastProceedings = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getProceedings_query($lastCrisis)));
            if($a_lastProceedings[1]['Number']>0){
                $a_crisis_tools['Notes'] = "Sentenza n. ".$a_lastProceedings[1]['Number']." del ".$cls_help->toItalianDate($a_lastProceedings[1]['Date']);
                if($cls_help->toItalianDate($a_lastProceedings[1]['Sentence_Request_Date'])!=null)
                    $a_crisis_tools['Notes'].= " richiesta il ".$cls_help->toItalianDate($a_lastProceedings[1]['Sentence_Request_Date']);
                if($cls_help->toItalianDate($a_lastProceedings[1]['Sentence_Challenge_Date'])!=null)
                    $a_crisis_tools['Notes'].= " impugnata il ".$cls_help->toItalianDate($a_lastProceedings[1]['Sentence_Challenge_Date']);
                if($a_lastProceedings[1]['Sentence_Challenger']!=null)
                    $a_crisis_tools['Notes'].= " da ".$a_lastProceedings[1]['Sentence_Challenger'];
            }
        }
...
$a_crisis_toolsType = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCrisisType_query()));
$a_selection = array("value"=>"ID","firstOpt"=>1,"selected"=>$a_crisis_tools['Type'], "text"=>array("[Description]"));
$opt_crisis_toolsType = $cls_html->getOptions($a_crisis_toolsType,$a_selection);

$a_proceedingsType = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCrisisProceedingsType_query()));
...
$a_courtType = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCourtHearingType_query()));
$opt_courtType = $cls_html->getOptions($a_courtType,$a_selection);

$a_courtDoctype = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCourtHearingDoctype_query()));
$opt_courtDoctype = $cls_html->getOptions($a_courtDoctype,$a_selection);

$a_crisis_toolsPartType = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCrisisPartType_query()));
$a_selection["selected"] = $a_crisis_tools['Trespassers_Part'];
$opt_trespassersPart = $cls_html->getOptions($a_crisis_toolsPartType,$a_selection);
$a_selection["selected"] = $a_crisis_tools['Body_Part'];
$opt_bodyPart = $cls_html->getOptions($a_crisis_toolsPartType,$a_selection);

// $a_crisis_toolsBodyType = $cls_db->getResults($cls_db->SelectQuery($cls_crisis_tools->getCrisisBodyType_query()));
// $a_selection["selected"] = $a_crisis_tools['Body_Type'];
// $opt_bodyType = $cls_html->getOptions($a_crisis_toolsBodyType,$a_selection);
...
$opt_lawyerPart[0] = $cls_html->getOptions($a_crisis_toolsPartType,$a_selection);
$a_selection["selected"] = $a_lawyerBill[1]['Part'];
$opt_lawyerPart[1] = $cls_html->getOptions($a_crisis_toolsPartType,$a_selection);

$a_proceedingsPath = $cls_crisis_tools->getProceedingStatusPath($c,$a_crisis_tools['ID']);
...
<script>
    ...
    function openOfcanvas(type,rif){
        ...
        switch (type){
            case 'authoritySearchModal':                                                     // ricerca autorità
                ...
                $('#crisis_tools').prop('checked', false);
    ...
    <script src="<?= JS ?>/crisis_tools.js"></script>
    ...
    <script>
        ...
        switchMenuImg("F4");
        F4_button = function(){
            $("form#crisis_tools_form").append('<input type="hidden" name="delete" value=1>');
            $("#crisis_tools_form").submit();
        }

        Crisis.setParams("<?=$c;?>","<?=$a;?>");
...
?>
    ...
    <form id="crisis_tools_form" name="crisis_tools_form" action="crisis_tools_save.php" method=post enctype="multipart/form-data">
        <input type="hidden" name="c" value="<?=$c;?>">
        <input type="hidden" name="a" value="<?=$a;?>">
        <input type="hidden" name="Crisis_ID" value="<?=$crisis_tools_id;?>">
        ...
        <div class="row">
            <div class="col col-lg-10 col-lg-offset-1">
                <div class="form-group">
                    <label class="col-lg-3 control-label resize"><span class="titolo">Crisi relativa a</span></label>
                    <div class="col-lg-6">
                        <select name="Crisis_Type" class="form-control resize">
                            <?=$opt_crisis_toolsType;?>
                        </select>
                    </div>
        ...
        <div class="row">
            <div class="col col-lg-10 col-lg-offset-1">
                <div class="form-group">
                    <div class="col-lg-12">
                        <textarea class="form-control resize" style="max-width: 100%;" name="Notes"><?php echo $a_crisis_tools['Notes'];?></textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_datiGenerali">
            <div class="col col-lg-3 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                        <input name=Start_Date style="background-color: #97CFDD; border: 2px solid black;" class="form-control resize readonly text_center" readonly value="<?=$a_crisis_tools['Start_Date']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-3">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                        <input name=End_Date class="form-control resize text_center picker" value="<?=$a_crisis_tools['End_Date']?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_datiGenerali">
            <div class="col col-lg-5 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                        <input name="Authority_Name" id="authorityName" style="background-color: rgb(153, 204, 255); border: 2px solid black;" class="form-control resize readonly text_left" ondblclick="/*Crisis.searchAuthority();*/openOfcanvas('authoritySearchModal',0);" readonly value="<?=$a_crisis_tools['Authority_Description'];?>">
                        <input type=hidden id="authorityId" name="Authority_ID" value="<?=$a_crisis_tools['Authority_ID']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-5">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                        <input name="Judge" class="form-control resize text_left" value="<?=$a_crisis_tools['Judge']?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_datiGenerali">
            <div class="col col-lg-3 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Amendment_Date" class="form-control resize text_center picker" value="<?=$a_crisis_tools['Amendment_Date']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-3">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Notification_Date" class="form-control resize text_center picker" value="<?=$a_crisis_tools['Notification_Date']?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_datiGenerali">
            <div class="col col-lg-3 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Registration_Date" class="form-control resize text_center picker" value="<?=$a_crisis_tools['Registration_Date']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-3">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Dossier_Submission_Date" class="form-control resize text_center picker" value="<?=$a_crisis_tools['Dossier_Submission_Date']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-4">
                <div class="form-group">
                    ...
                    <div class="col-lg-10">
                        <input name="RG" class="form-control resize text_left" value="<?=$a_crisis_tools['RG']?>">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col col-lg-5 col-lg-offset-1">
                <div class="form-group">
                    <div class="col-lg-12">
                        ...
                        <a onMouseover="title='Aggiungi coloro che sono rappresentati dall\'avvocato'" href="#" onClick="Crisis.addUserPart('trespassers')" style="text-decoration: none">
        ...
        <div class="row">
            <div class="col col-lg-5 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                        <input name="Trespassers_Lawyer" class="form-control resize text_left" value="<?=$a_crisis_tools['Trespassers_Lawyer']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-5">
                <div class="form-group">
                    ...
                    <div class="col-lg-8">
                        <input name="Trespassers_Lawyer_Bar" class="form-control resize text_left" value="<?=$a_crisis_tools['Trespassers_Lawyer_Bar']?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="tr_trespassers last_trespassers row" id="td_trespassers_begin">
            <?php
            for($i=0;$i<count($a_crisis_toolsPart);$i++){
            if($i%2==0 && $i>0){
        ...
        <div class="tr_trespassers last_trespassers row">
            ...
            <div class="col col-lg-4 <?=$offset;?>"  id='td_trespassers_<?=($i+1)?>'>
                <input type=hidden name='id_trespassers[<?=($i+1)?>]'  id='id_trespassers_<?=($i+1)?>' value="<?=$a_crisis_toolsPart[$i]['Part_ID'];?>">
                <input class='form-control resize readonly' style="background-color: rgb(153, 204, 255); border: 2px solid black;" type="text" id='trespassers_<?=($i+1)?>' name='trespassers[<?=($i+1)?>]'
                       readonly onclick='Crisis.searchUserPart("<?=($i+1)?>","trespassers");' value="<?=$a_crisis_toolsPart[$i]['Part_Name'];?>"></div>
            <div class="col-lg-1 resize" id='post_td_trespassers_<?=($i+1)?>'>
                <a onMouseover="title='Elimina utente'" href='#' style='text-decoration:none;' onClick='Crisis.removeUserPart("<?=($i+1)?>","trespassers")' >
        ...
        if((count($a_crisis_toolsPart)%2!=0 && count($a_crisis_toolsPart)>0) || count($a_crisis_toolsPart)==0){
            ?>
            </div>
            <?php
        }
        ...
        <div class="row">
            <div class="col col-lg-3 col-lg-offset-1">
                <div class="form-group">
                    <div class="col-lg-12">
                        <span class="titolo font14">Controparte</span>
                    </div>
                </div>
            </div>
            <div class="col col-lg-3">
            ...
            <div class="col col-lg-4">
                <div class="form-group">
                    <div class="col-lg-8">
                        <input name="Body_Type" class="form-control resize text_left" value="<?=$a_crisis_tools['Body_Type']?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col col-lg-5 col-lg-offset-1">
                <div class="form-group">
                    <label class="col-lg-4 control-label resize">Avvocato</label>
                    <div class="col-lg-8">
                        <input name="Body_Lawyer" class="form-control resize text_left" value="<?=$a_crisis_tools['Body_Lawyer']?>">
                    </div>
                </div>
            </div>
            <div class="col col-lg-5">
                <div class="form-group">
                    <label class="col-lg-4 control-label resize">Foro</label>
                    <div class="col-lg-8">
                        <input name="Body_Lawyer_Bar" class="form-control resize text_left" value="<?=$a_crisis_tools['Body_Lawyer_Bar']?>">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col col-lg-5 col-lg-offset-1">
                <div class="form-group">
                    <div class="col-lg-12">
                        ...
                        <a onMouseover="title='Aggiungi udienza'" href="#" onClick="$('.tr_courtHearing').show();Crisis.addCourtHearing(opt_court,opt_court_doc);" style="text-decoration: none">
                            <img src="<?= IMMAGINIWEB; ?>/plus.png" width=15 height=15 border=0>
                        </a>
                    </div>
                </div>
        ...
        for($i=count($a_courtHearing)-1;$i>=0;$i--){
            ...
            $courtHearingPath = $cls_crisis_tools->getCourtHearingPath($c,$a_crisis_tools['ID'],$a_courtHearing[$i]['ID']);
            ...
            <div class="tr_courtHearing row" >
                ...
                <div class="col col-lg-5 ">
                    <div class="form-group">
                        <div class="col-lg-12">
                            <?=$cls_crisis_tools->getFilesRow($a_plaintiffFiles,$_SERVER['REQUEST_URI']);?>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="tr_courtHearing row" >
                ...
                <div class="col col-lg-5 ">
                    <div class="form-group">
                        <div class="col-lg-12">
                            <?=$cls_crisis_tools->getFilesRow($a_respondentFiles,$_SERVER['REQUEST_URI']);?>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row tr_proceedingStatus" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <?=$cls_crisis_tools->getFilesRow($a_proceedingsFiles[1][0],$_SERVER['REQUEST_URI']);?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_proceedingStatus" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <?=$cls_crisis_tools->getFilesRow($a_proceedingsFiles[2][0],$_SERVER['REQUEST_URI']);?>
                    </div>
                </div>
            </div>
            <div class="col col-lg-2">
                    ...
                    <div class="col-lg-12">
                        ...
                        if(isset($a_proceedings[1])){
                            if($a_proceedings[1]['Outcome']==6){//INCOMPETENTE
                                ?>
                                <input class="btn btn-primary" type="button" onclick='Crisis.changeAuthority("<?=$partitaID;?>","<?=$a_crisis_tools['ID'];?>")' value="Cambia autorità">
                                <?php
                            }
                        }
        ...
        <div class="row tr_proceedingStatus" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <?=$cls_crisis_tools->getFilesRow($a_proceedingsFiles[2][1],$_SERVER['REQUEST_URI']);?>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_lawyerBill" style="margin-top: 2%;" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    ...
                    <div class="col-lg-7">
                        <input name="Fee[1]" id="fee1" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(1);" value="<?=$cls_help->floatToString($a_lawyerBill[0]['Fee'])?>">
                    </div>
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    ...
                    <div class="col-lg-7">
                        <input name="Rights[1]" id="rights1" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(1);" value="<?=$cls_help->floatToString($a_lawyerBill[0]['Rights'])?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_lawyerBill" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <select id="VAT_exemption1" class="form-control resize" name="VAT_Exemption[1]" onchange="Crisis.lawyerAmounts(1);"><?=$opt_VAT[0];?></select>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_lawyerBill">
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    ...
                    <div class="col-lg-7">
                        <input name="CU[1]" id="cu1" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(1);" value="<?=$cls_help->floatToString($a_lawyerBill[0]['CU'])?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
            <div class="col col-lg-2">
                <div class="form-group">
                    ...
                    <div class="col-lg-7">
                        <input name="Stamp_Duty[1]" id="stamp_duty1" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(1);" value="<?=$cls_help->floatToString($a_lawyerBill[0]['Stamp_Duty'])?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
            <div class="col col-lg-2">
                <div class="form-group">
                    ...
                    <div class="col-lg-7">
                        <input name="Other_Costs[1]" id="other_costs1" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(1);" value="<?=$cls_help->floatToString($a_lawyerBill[0]['Other_Costs'])?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_lawyerBill" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <select id="withholding_tax_exemption1" class="form-control resize" name="Withholding_Tax_Exemption[1]" onchange="Crisis.lawyerAmounts(1);"><?=$opt_Withholding_Tax[0];?></select>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_lawyerLosing" style="margin-top: 2%;" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <label class="col-lg-3 control-label resize">Onorari</label>
                    <div class="col-lg-7">
                        <input name="Fee[2]" id="fee2" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(2);" value="<?=$cls_help->floatToString($a_lawyerBill[1]['Fee']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
            <div class="col col-lg-2">
                <div class="form-group">
                    <label class="col-lg-3 control-label resize">Diritti</label>
                    <div class="col-lg-7">
                        <input name="Rights[2]" id="rights2" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(2);" value="<?=$cls_help->floatToString($a_lawyerBill[1]['Fee']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_lawyerLosing" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <select id="VAT_exemption2" class="form-control resize" name="VAT_Exemption[2]" onchange="Crisis.lawyerAmounts(2);"><?=$opt_VAT[1];?></select>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_lawyerLosing">
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <label class="col-lg-3 control-label resize">C.U.</label>
                    <div class="col-lg-7">
                        <input name="CU[2]" id="cu2" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(2);" value="<?=$cls_help->floatToString($a_lawyerBill[1]['CU']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
            <div class="col col-lg-2">
                <div class="form-group">
                    <label class="col-lg-3 control-label resize">Marche</label>
                    <div class="col-lg-7">
                        <input name="Stamp_Duty[2]" id="stamp_duty2" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(2);" value="<?=$cls_help->floatToString($a_lawyerBill[1]['Stamp_Duty']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
            <div class="col col-lg-2">
                <div class="form-group">
                    <label class="col-lg-3 control-label resize">Altro</label>
                    <div class="col-lg-7">
                        <input name="Other_Costs[2]" id="other_costs2" class="form-control resize corrige_numero text_right" onchange="Crisis.lawyerAmounts(2);" value="<?=$cls_help->floatToString($a_lawyerBill[1]['Other_Costs']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_lawyerLosing" >
            ...
            <div class="col col-lg-2">
                <div class="form-group">
                    <div class="col-lg-12">
                        <select id="withholding_tax_exemption2" class="form-control resize" name="Withholding_Tax_Exemption[2]" onchange="Crisis.lawyerAmounts(2);"><?=$opt_Withholding_Tax[1];?></select>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row tr_judgeAmounts" style="margin-top: 2%;">
            <div class="col col-lg-6 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Judge_Total" id="judge_total" class="form-control resize text_right corrige_numero" value="<?=$cls_help->floatToString($a_crisis_tools['Total']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_judgeAmounts">
            <div class="col col-lg-6 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Judge_Act_Amount" id="judge_act_amount" class="form-control resize text_right corrige_numero" value="<?=$cls_help->floatToString($a_crisis_tools['Act_Amount']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_judgeAmounts">
            <div class="col col-lg-6 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Judge_Legal_Costs" id="judge_legal_costs" class="form-control resize text_right corrige_numero" value="<?=$cls_help->floatToString($a_crisis_tools['Legal_Costs']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
        ...
        <div class="row tr_judgeAmounts">
            <div class="col col-lg-6 col-lg-offset-1">
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input name="Judge_Actual_Costs" id="judge_actual_costs" class="form-control resize text_right corrige_numero" value="<?=$cls_help->floatToString($a_crisis_tools['Actual_Costs']);?>">
                    </div>
                    <label class="col-lg-2 control-label resize">€</label>
                </div>
            </div>
