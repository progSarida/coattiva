...
try{
    ...
    if(!$cls_db->ExecuteQuery($query)){
        ...
        $log->error("Aggiornamento DB fallito, tabella sul db sgravio, pagina coattiva/elimina_pdf.php");
    }

    if($tipoEst == "sgravio") $query = "UPDATE partita_tributi SET Printed_Sgravio = 'no' WHERE ID = ".$Partita_ID;
    else $query = "UPDATE partita_tributi SET Printed_Annull = 'no' WHERE ID = ".$Partita_ID;

    if(!$cls_db->ExecuteQuery($query)){
        $error = 1;
        $msg = "Aggiornamento DB fallito";
        $log->error("Aggiornamento DB fallito, tabella sul db partita_tributi, pagina coattiva/elimina_pdf.php");
    }
}
catch(Exception $ex){
...
echo "<script>window.location.href = '".WEB_ROOT."/coattiva/annulamento_sgravi.php?c={$c}&a={$a}&calling_page={$calling_page}&last_act={$last_act}&partita={$Partita_ID}&p={$p}&msg={$msg}&error={$error}&pageCalled=".$cls_help->getVar("pageCalled")."';</script>";
die;
