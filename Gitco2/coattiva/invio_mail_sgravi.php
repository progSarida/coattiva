...
//Variabile aggiunta da me in phpMailer, (nelle versioni nuove della libreria ci dovrebbe già essere)
        ...
        if ($modInvio == "PEC") {
            ...
            */
            ...
            if($TipoPDF=='ente') {
                ...
                //$PEC = "ufficiale.riscossione@pec.it";
                $cls_mail->addAddress($PEC);
            }
            else if($TipoPDF=='utente' && $Tipo != 1) {
                $PEC = $a_utente["PEC"];
                //$Test = null;
                //$cls_mail->addAddress($Test);
                //$PEC = "ufficiale.riscossione@pec.it";
            ...
            */
            ...
            if($TipoPDF=='ente') {
                ...
                //$mail = "gianluca.virdis8901@gmail.com";
                $cls_mail->addAddress($mail);
            }
            else if($TipoPDF=='utente'  && $Tipo != 1) {
                $mail = $a_utente["Mail"];
                //$mail = "gianluca.virdis8901@gmail.com";
        ...
        else if($TipoPDF=='utente'  && $Tipo != 1) $cls_mail->addAttachment($result["File_1"]);
        else $log->error("Tipo ente/utente non presente");
...
header("Location: annulamento_sgravi.php?c={$c}&a={$a}&calling_page={$calling_page}&last_act={$last_act}&partita={$Partita_ID}&p={$p}&msg={$msg}&error={$error}&pageCalled={$cls_help->getVar('pageCalled')}");