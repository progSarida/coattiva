	...
	switch($invia)
	{
		...
		//$salva->Importo = $importo;
			...
			if($num_rate!=0)
			{
				...
				$dovuto = number_format($cls_math->conv_num($importi_rate[$rata-1]),2);
			}
			else {
				if($rata==null){
		...
		case "Delete":
			...
			$cls_db->End_Transaction();
			header("Location: pagamento.php?c={$c}&a={$a}&error={$error}&msg={$msg}");
