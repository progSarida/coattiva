...
require $_SERVER['DOCUMENT_ROOT'] . explode("/Gitco2", $_SERVER['SCRIPT_NAME'])[0] . "/config/_config.php";
    ...
    include_once(ROOT . "/search_modal/offcanvas/role_offcanvas.php");
...
// Insermento dati da selezione
function initialId(type,val){
    switch (type){
        case 'role_d':
        case 'role_y':
            $('#id_ruolo').val(val['ID']);
            $('#ruolo').val(val['Descrizione']);
            $('#data').val(val['Data_Fornitura']);
            $('#num_ruolo').val(val['Num_Ruolo']);
            $('#tipo_ruolo').val(val['Ruolo']);
            $('#num_rate').val(val['Num_Rate']);
            break;
    }
}
    ...
    if(valorediritorno!=null){
        switch(selectParent){
            ...
            case "ruolo":
                ...
                $('#num_ruolo').val(valorediritorno.Num_Ruolo);
...
<div class="row">
	...
	<div class="col col-lg-2 col-lg-offset-4">
		<div class="form-group">
			...
			<div class="col-lg-12">
				<input type=button id=new_ruolo name=new_ruolo value="Cerca Ruolo" class="form-control resize btn btn-primary" onclick="/*RicercheDaId('ruolo',1);*/openOfcanvas('role',1);">
			</div>
		</div>
	</div>
</div>
		...
		<div class="form-group">
			...
			<div class="col-lg-8">
				<select id=tipo_ruolo name=tipo_ruolo class="form-control resize vld_req">
					<option></option>
					<option id=Ord value=0>Ordinario</option>
					<option id=Coa value=1>Coattivo</option>
				</select>
			</div>
		</div>
	</div>
