...
if($_SESSION['username']==NULL)
{
	...
	if($id_ruolo!=null)
	{
		...
		if(!$cls_db->DbSave($cls_utils->GetObjectQuery((array) $ruolo,"ruolo",array("ID" => $id_ruolo))))
		{
			...
			$storico->insRow('U', "Modificato ruolo ".$ruolo->Descrizione."(".$ruolo->Comune_ID.") per ente ".$ente['Denominazione']."[".$c."]");
	...
	else
	{
		...
		if(!$newID)
		{
			...
			$storico->insRow('I', "Inserito ruolo ".$ruolo->Descrizione."(".$ruolo->Comune_ID.") per ente ".$ente['Denominazione']."[".$c."]");
