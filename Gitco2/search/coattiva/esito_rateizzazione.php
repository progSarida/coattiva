...
$cls_text->a_var = array(
    ...
    "{InstalmentManager}" => $atto->Nominativo_Gestore_Rateizzazione,
    "{InstalmentManagerPosition}" => $atto->Posizione_Gestore_Rateizzazione,
    "{Motivation}" => $atto->Motivazione_Respinta_Rateizzazione,
    "{InstalmentNumber}" => $num_rate,
...
if($stampa_select == "PROVVISORIA")
{
    if($mode == "ajax_pigno"){
        ...
        $pdf->Output( $path."/rateizzazione_esito.pdf" , 'F');
        //var_dump("son qui");
        echo json_encode([
                "type" => "temp",
                "file" => ARCHIVIO_WEB."/temp/rateizzazione_esito.pdf",
                "error" => 0,
