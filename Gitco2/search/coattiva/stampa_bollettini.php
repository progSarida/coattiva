...
$settore = isset($atto->Tipo_Riscossione)?$atto->Tipo_Riscossione:$atto->Tipo;
...
if($stampa_select == "PROVVISORIA")
    ...
    if($stampa_select == "PROVVISORIA")
    {
        if($mode == "ajax_pigno"){
            ...
            $pdf->Output( $path."/rateizzazione_bollettini.pdf" , 'F');
            //var_dump("son qui");
            echo json_encode([
                    "type" => "temp",
                    "file" => ARCHIVIO_WEB."/temp/rateizzazione_bollettini.pdf",
                    "error" => 0,
