	...
	case ("ricUfficio"):
		...
		if( $posted == true )
		{
			$ric_comune = $cls_help->getVar('ric_comune');
      		...
      		$num_comuni = $cls_db->getNumberRow($resultComune);// mysql_num_rows($resultComune);
...
<?php
        while($com_trovato = mysqli_fetch_array($resultComune, MYSQLI_ASSOC))
        {
            ...
            $comune_ente = $cls_db->getObjectLineNull($cls_db->ExecuteQuery($query),"comuni_lista");
        	//$comune_ente = new comune($CC_ente);
        	$nome_ente = $comune_ente->Com_Nome;
        	$nome_ente_temp = addslashes($comune_ente->Com_Nome);
