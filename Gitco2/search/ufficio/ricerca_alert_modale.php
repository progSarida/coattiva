...
case('ricUfficio'):
	...
	if ($num_comuni==0){	echo"<script>alert('Non � stato trovato nessun ufficio relativo alla ricerca effettuata.'); self.close();</script>";}
	else
	{
...
<?php
        while($com_trovato = mysqli_fetch_array($resultComune, MYSQLI_ASSOC))
        {
          ...
          $query = "SELECT * FROM comuni_lista WHERE Com_Codice_Catastale = '".$CC_ente."'";
      		$param = $cls_db->getArrayLineNull($cls_db->ExecuteQuery($query),"comuni_lista");// safe_query($query);
      		//$val = mysql_fetch_array($result);

        	$nome_ente = $param["Com_Nome"];
        	$nome_ente_temp = addslashes($nome_ente);
...
?>
	...
	<tr <?php echo $stile_riga; ?>>
		...
		<input title="Clicca qui per inserire il comune" type=image src="<?= IMMAGINIWEB; ?>/select.png" id=tab_<?php echo $j+1 ?> <?php echo $add_tag; ?> style="width:25px; height:25px; border:0;"
