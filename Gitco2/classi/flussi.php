...
class flussi
{
	...
	public function __construct($cartella = NULL, $fl = NULL, $tipo = NULL, $comune = NULL, $anno = NULL, $numero = NULL, $data = NULL, $ora = NULL, $est = NULL)
	{
        ...
        $this->Filename = $cartella . $fl . "_" . $tipo . "_" . $comune . "_" . $anno . "_" . $numero . "_" . $data . "_" . $ora . "." . $est;

		$this->myFlusso = fopen ($this->Filename, "w+");
	}
...
//    }
	...
	function ChiudiFlusso ($PrinterId=null)
	{
		...
		if($PrinterId<2 && $PrinterId!=null){
            ...
            $flowFileTxt = $fileTxt . "." . $this->myEstensione;
            $flowFileRar = $nomeRar . ".rar";
        ...
        $flowsTable = new flows_table(null);
		switch ($this->myTipo)
		{
			case "verbali": $myFluxTab->Tipo = "V_EST"; $flowsTable->DocumentTypeId=1; break;
			case "avvisi": $myFluxTab->Tipo = "AV_INT"; $flowsTable->DocumentTypeId=4; break;
			case "ingiunzioni": $myFluxTab->Tipo = "ING"; $flowsTable->DocumentTypeId=2; break;
			case "pigno_banca": $myFluxTab->Tipo = "PIGNO_BANCA"; $flowsTable->DocumentTypeId=8; break;
			case "pigno_veicolo": $myFluxTab->Tipo = "PIGNO_VEICOLO"; $flowsTable->DocumentTypeId=6; break;
            case "pigno_lavoro": $myFluxTab->Tipo = "PIGNO_LAVORO"; $flowsTable->DocumentTypeId=7; break;
			default: alert ("il tipo flusso non e' gestito in flussi.php"); $myFluxTab->Tipo = ""; $flowsTable->DocumentTypeId=0; break;
		}
		...
		$myFluxTab->Nome_Flusso = $flowFileTxt;
        $myFluxTab->Nome_Flusso_Rar = $flowFileRar;
		$myFluxTab->Data_Travaso_Verso_Gitco = '0000-00-00';

        $flowsTable->CityId = $this->myComune;
        $flowsTable->PrinterId = $this->PrinterId;
        $flowsTable->Year = $this->myAnno;
        $flowsTable->TaxType = $this->TaxType;
        $flowsTable->Number = $this->myNumero;
        $flowsTable->FileName = $flowFileRar;
        $flowsTable->PrintTypeId = $this->PrintTypeId;
//        $flowsTable->DocumentTypeId;

        $flowsTable->UploadDate = null;
        $flowsTable->ProcessingDate = null;
        $flowsTable->PostagePaymentDate = null;
        $flowsTable->OldExportDate = null;
        $flowsTable->CreationDate = $this->myData;
        if($this->PrinterId==1){
            $flowsTable->UploadDate = $this->myData;
            $flowsTable->ProcessingDate = $this->myData;
        }

        $flowsTable->OriginalRecordsNumber = $this->myRigheTotali;
        $flowsTable->RecordsNumber = $this->myRigheTotali;
        $flowsTable->Zone0Number = $this->myRigheTotali;
        $flowsTable->Zone1Number = 0;
        $flowsTable->Zone2Number = 0;
        $flowsTable->Zone3Number = 0;

        $flowsTable->PrintCost = $this->PrintCost;
        $flowsTable->Zone0Postage = $this->Zone0Postage;
        $flowsTable->Zone1Postage = $this->Zone1Postage;
        $flowsTable->Zone2Postage = $this->Zone2Postage;
        $flowsTable->Zone3Postage = $this->Zone3Postage;
        $flowsTable->Operatore = $_SESSION['username'];

		$myFluxTab->InsertUpdateFlussoTab();
        $flowsTable->InsertUpdateFlussoTab();
	}
}
...
class flows_table
{
    ...
    public $ProcessingDate = null;
    public $SendDate = null;
    public $PostagePaymentDate;
    public $PostagePaymentBankId;
    public $OriginalRecordsNumber;
    ...
    public $PostageInvoiceId;
    public $OldExportDate = null;
    public $Note;
    public $Operatore;
    ...
    public function __construct ($progr)
    {
        ...
        $query = "SELECT * FROM flows WHERE Id = '" . $progr . "' ";
        $result = mysql_query($query);
        $rigaFlussi = mysql_fetch_assoc($result);
    ...
    public function FlussoGiaPresente ()
    {
        $queryCerca = "SELECT Id FROM flows ";
        $queryCerca .= "WHERE DocumentTypeId = " .$this->DocumentTypeId." ";
        $queryCerca .= "AND CityId = '" . $this->CityId . "' ";
        $queryCerca .= "AND Year = " . $this->Year . " ";
        $queryCerca .= "AND Number = " . $this->Number . " ";
        //echo "<br>" . $queryCerca;
        $resCerca = mysql_query($queryCerca);
        $rigaCerca = mysql_fetch_assoc($resCerca);
        return $rigaCerca['Id'];
    ...
    public function CheckDocumentType ()
    {
        ...
        {
            case 2://Ingiunzione
            case 3://Sollecito di pagamento
            case 4://Avviso di intimazione ad adempiere
            case 6://Pignoramento di beni mobili registrati
            case 7://Pignoramento presso datore di lavoro
            case 8://Pignoramento presso banca
            case 11://Sollecito pre ingiunzione
            case 12://Avviso di messa in mora
                break;
    ...
    public function InsertUpdateFlussoTab ($forzoInsertUpdate = NULL)
    {
        if ($this->CheckDocumentType() == false) return;
        ...
        else
        {
            ...
            {
                $queryUpd = "SELECT Id FROM flows WHERE Id = '$this->Id'";
                $resUpd = mysql_query($queryUpd);
                $rigaUpd = mysql_fetch_assoc($resUpd);
                //alert ($queryUpd);
                if ($rigaUpd['Id'] == NULL)
    ...
    public function insert_flusso_locale($fields_to_insert, $values_to_insert)
    {
        ...
        $query = "INSERT INTO flows (" . $clause . ") VALUES (";
        $clause = "";
        for ($i = 0; $i < $dim1; $i++)
        {
            if($fields_to_insert[$i]=="SendDate" || $fields_to_insert[$i]=="UploadDate" || $fields_to_insert[$i]=="OldExportDate"
                || $fields_to_insert[$i]=="ProcessingDate" || $fields_to_insert[$i]=="PostagePaymentDate"){
                if($values_to_insert[$i]==null || $values_to_insert[$i]=="" || $values_to_insert[$i]=="0000-00-00")
                    $clause .= "null";
                else
                    $clause .= "'" . $values_to_insert[$i] . "'";
            }
    ...
    public function update_flusso_locale($key, $fields_to_update, $values_to_update)
    {
        ...
        $myOldFlusso = new flows_table($key);
        ...
        for ($i = 0; $i < $dim1; $i++)
        {
            if ($myOldFlusso->$fields_to_update[$i] != $values_to_update[$i])
            {
                $clause .= $fields_to_update[$i] . "=";
                if($values_to_update[$i]==null)
                    $clause.= "null , ";
                else
                    $clause .= "'" .$values_to_update[$i]. "' , ";
            }
        }
        //alert ($clause);
        if ($clause == "") return TRUE;  // non updata nulla, perch� sono tutti uguali
        ...
        $query = "UPDATE flows SET $clause WHERE Id = '" . $key . "'";
