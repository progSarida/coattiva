...
class PHPMailer
{
    ...
    public function smtpConnect($options = array())
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $this->SMTPOptions)) {
                try {
                    if ($this->Helo) {
                        $hello = $this->Helo;
                    } else {
    ...
    /**
     * Costruisci mail.
     * @param object $par_email Parametri di autenticazione mail (Classe parametri_email)
     * @param string $subject
     * @param string $body
     */
    public function creaMailCompleta($par_email, $subject, $body)
    {
    	//$this->SMTPDebug = 2;
    	$from = $par_email->Indirizzo_Email;
    	$autenticazione = $par_email->Autenticazione_Uscita;
    	$username = $par_email->Nome_Utente_Uscita;
    	$password = decryptit($par_email->Password_Uscita);
    	$from_name = $par_email->Nome_Visualizzato;
    	$sicurezza = $par_email->Sicurezza_Connessione;
    	$host = $par_email->Server_Posta_Uscita;
    	$protocol = $par_email->Protocollo_Uscita;
    	$porta = $par_email->Porta_Uscita;
    	...
    	if(strtolower($protocol)=="smtp")
    	{
    		$this->IsSMTP();  // telling the class to use SMTP
    		$this->SMTPSecure 	= $sicurezza;
    		if($autenticazione=="si")
    		{
    			$this->SMTPAuth = true;
