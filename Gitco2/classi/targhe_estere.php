...
class registro_cronologico_cds
{
	...
	public $Reg_Data_Ristampa_Richiesta = NULL;
	...
	public function CorreggiDatiCambioArticolo ($id_articolo)
	{
		if ($this->Reg_Ente_Per_Richiesta == NULL) $this->Reg_Ente_Per_Richiesta = 1;
		if ($this->Reg_Motivi_Mancata_Contestazione == NULL) $this->Reg_Motivi_Mancata_Contestazione = 1;
		...
		if ($id_articolo != "")
		{
			$articolo = new targhe_estere_articoli($id_articolo);
			$articolo->DiurnoNotturno($this->Reg_Ora_Avviso);
			
			$sommaImpAmministrative = $articolo->Tar_Tariffa_Sanz;
			$sommaImpMaxAmministrative = $articolo->Tar_Tariffa_Sanz_Max;
		...
		$this->Reg_Articoli_Infrazione = $id_articolo;
		$this->Reg_Importi_Separati = conv_num($sommaImpAmministrative);
		$this->Reg_Importo_Amministrativo = $sommaImpAmministrative;
		$this->Reg_Importo_Sanzione_Massima = $sommaImpMaxAmministrative;
	...
	public function ControllaArticoloZtl ($tipo)  //  $tipo = INS per insert/update
	{
		...
		{
			$articoloRilevatoComma9 = $this->SelectArticoloInfrazione7Comma9();
			$articoloRilevatoComma1 = $this->SelectArticoloInfrazione7Comma1();
...
// 			else // per un controllo
			{
				...
				for ($i = 0; $i < count ($this->Coll_Articoli_Infranti); $i++)
				{
					if ($articoloRilevatoComma1 == $this->Coll_Articoli_Infranti[$i]->Tar_Progr)
					{
						//alert ("ok, articolo combaciante: $articoloRilevato ");
						return "OK_ARTICOLO";
					}
					else if ($articoloRilevatoComma9 == $this->Coll_Articoli_Infranti[$i]->Tar_Progr)
					{
						//alert ("ok, articolo combaciante: $articoloRilevato ");
						return "OK_ARTICOLO";
					}
	...
	public function SelectArticoloInfrazione142 (&$sanzioneAccessoria)
	{
		...
		else alert ("non c'è rilevatore in selectarticolo142:".$sanzioneAccessoria);
	...
	function SelectArticoloInfrazione7Comma1 ()
	{
		$query = "SELECT Tar_Progr FROM targhe_estere_articoli ";
		$query .= "WHERE Tar_Anno = '$this->Reg_Anno' AND Tar_Com = '$this->Reg_Comune_Violazione' ";
		$query .= "AND Tar_Articolo = '7' AND Tar_Comma = '1' AND Tar_Genere_Infrazione = 'ZTL'";
		//alert ($query);
		$progressivo = single_answer_query($query);
		//alert ($risp);
		if ($progressivo == NULL)
...
SELECTTIPO;
	...
	public function NewDeterminaArticolo ()
	{
		...
		if (count($explodeArticoli) == 0)
		{
			if (!isset($this->Coll_Articoli_Infranti[0]) && $this->Reg_Articoli_Infrazione!=null)
			{
				$this->Coll_Articoli_Infranti[0] = new targhe_estere_articoli($this->Reg_Articoli_Infrazione);
			}
		}
		else if (count($explodeArticoli) > 1)
		{
		...
		else if ($tipoReg == "ZTL")
		{
			...
			$this->Reg_Dati_Infrazione = "";
			//alert ($this->Reg_Rilevatore_Elettronico);
			$rispDet = $this->ControllaArticoloZtl("");
			//$this->Reg_Tempo_Semaforo = "0**0";
			//$rispDet = "OK_ARTICOLO";
		}
		else if ($tipoReg == "SOSTA")
	...
	public function InsertUpdateCambioArticolo ($forzato = NULL,$id_articolo)  //  forzato è INSERT o UPDATE
	{
		//return "articolo numero $this->Reg_Articoli_Infrazione";
		$this->CorreggiDatiCambioArticolo($id_articolo);
	...
	public function update_richieste_locale($key, $fields_to_update, $values_to_update)
	{
		...
		$clause = "";$control_fields = "";
		for ($i = 0; $i < $dim1; $i++)
		{
			$control_fields.= $fields_to_update[$i]." = ".$values_to_update[$i]."; - ";
	...
	function QueryRichiesteDaStampare ($comune)
	{
		...
		$queryRichiesteDaStampare .= "Reg_Progr_Registro != 0 AND ";
	...
	function ListaPerStampeRichiesteDati ($arrayPost)
	{
		...
		$aggiuntaElenco = "";
		if ($da_n_elenco != NULL) $aggiuntaElenco .= " AND Reg_Progr >= " . $da_n_elenco . " ";
		if ($a_n_elenco != NULL) $aggiuntaElenco .= " AND Reg_Progr <= " . $a_n_elenco . " ";
		
		$aggiuntaDataAvviso = "";
		if ($da_avviso != NULL) $aggiuntaDataAvviso .= " AND Reg_Data_Avviso >= '" . to_mysql_date($da_avviso) . "' ";
		if ($a_avviso != NULL) $aggiuntaDataAvviso .= " AND Reg_Data_Avviso <= '" . to_mysql_date($a_avviso) . "' ";
		
		$aggiuntaDataInserimento = "";
		if ($da_insert != NULL) $aggiuntaDataInserimento .= " AND Reg_Data_Registrazione >= '" . to_mysql_date($da_insert) . "' ";
		if ($a_insert != NULL) $aggiuntaDataInserimento .= " AND Reg_Data_Registrazione <= '" . to_mysql_date($a_insert) . "' ";
		...
		if ($svizzere != "")
		{
			...
			$datediff = date('Y-m-d', strtotime('-6 months'));
			$aggiuntaDataStampa = " AND Reg_Data_Stampa_Richiesta = '0000-00-00' AND (Reg_Data_Ristampa_Richiesta='0000-00-00' OR Reg_Data_Ristampa_Richiesta < '".$datediff."') ";
		...
		$aggiuntaComune = "";
		if ($selectcomune != NULL)  $aggiuntaComune = " AND Reg_Comune_Violazione = '" . $selectcomune . "' ";
		...
		while ($rigacomuni = mysql_fetch_assoc($result)){
			...
			$query .= "AND Reg_Data_Esecuzione_Impossibile = '0000-00-00' AND ";
			...
			echo "NUMERO RICHIESTE --> " . mysql_num_rows($resultRichieste)."<br /><br />";

			$tempquery = "";

			while ($rigaRichieste = mysql_fetch_array($resultRichieste))
			{
				//for ($k = 0; $k < 15; $k++)
