...
$myParametroAtto = new testo_preavviso_fermo(null);
$myId = $myParametroAtto->CercaParametroData($c, date("Y-m-d"));

$myParametroAtto = new testo_preavviso_fermo($myId);
...
$Beni_Strumentali = $myParametroAtto->Beni_Strumentali;
...
function salva_form() 
{
	if (CheckCampiObbligatori () == true)
	{
		if (CheckTuttiCampi () == true)
		{
			...
			if(control_salva)
       			$("#form_testo_preavviso_fermo").submit();
		}
    }
}
...
function annulla() 
{
	...
	stringa = "testo_preavviso_fermo.php?"+stringaPHP;
...
function CheckCampiObbligatori ()
 {
	...
	if (ParolaPresenteInStringa ("beni_strumentali_testo", "{PECGESTORE}") == false) return false;
...
function CheckTuttiCampi ()
{
	...
	if (CheckCaratteri ("beni_strumentali") == false) return false;
...
$(document).ready(function(){
	...
	$('#form_testo_preavviso_fermo').ajaxForm(
...
<table class="table_interna text_center" border="0" cellspacing="10" cellpadding="0">
	<tr>
		<td><font class="titolo font16 under_decor">Testo Preavviso Fermo amministrativo</font></td>
	</tr>
</table>
...
<form name="testo_preavviso_fermo" id="form_testo_preavviso_fermo" action="testo_preavviso_fermo_salva.php" method="post">
...
<table class="table_interna text_center" border="0">
	...
	<tr>
		<td>
			<div id="beni_strumentali">Beni strumentali:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="beni_strumentali" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="2"><?=$Beni_Strumentali?></textarea>
		</td>
		<td>
			<font size=-2>
			</font>
