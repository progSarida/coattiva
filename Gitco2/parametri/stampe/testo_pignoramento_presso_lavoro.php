...
//$tipo_terzo = $cls_help->getVar('tipo_terzo');
$tipo_terzo = "lavoro";
...
$myId = $cls_testi->CercaParametroDataPignoramento($c, date("Y-m-d"));

$myParametroAtto = new testo_pignoramento_presso_lavoro($myId);
...
function salva_form() 
{
	if (CheckCampiObbligatori () == true)
	{
		if (CheckTuttiCampi () == true)
		{
			...
			if(control_salva)
       			$("#form_testo_pignoramento_presso_lavoro").submit();
		}
    }
}
...
function annulla() 
{
	...
	stringa = "testo_pignoramento_presso_lavoro.php?"+stringaPHP;
...
function CheckCampiObbligatori ()
 {
	...
	if (ParolaPresenteInStringa ("ordina_testo", "{TERZI}") == false) return false;
	...
	if (ParolaPresenteInStringa ("intimo_testo", "{TERZI}") == false) return false;
	if (ParolaPresenteInStringa ("intimo_testo", "{UTENTE}") == false) return false;

	if (ParolaPresenteInStringa ("informo_testo_2", "{TERZI}") == false) return false;
...
$(document).ready(function(){
	...
	$('#form_testo_pignoramento_presso_lavoro').ajaxForm(
...
<form name="testo_pignoramento_presso_lavoro" id="form_testo_pignoramento_presso_lavoro" action="testo_pignoramento_presso_lavoro_salva.php" method="post">
...
<table class="table_interna text_center" border="0">
	...
	<tr>
		...
		<td>
			<font size=-2>
			{TERZI}<br>{UTENTE}<br>{UTENTE2}<br>{NUMEROCONTO}<br>{INTESTATARIOCONTO}<br>{IBAN}<br>{CODICEUTENTE}<br>{CRONOLOGICO}<br>{RIFERIMENTO}<br>{ENTE}
			</font>
		</td>
	</tr>
	<tr>
