...
$table = $cls_ruolo->a_docDetails["textTable"];
...
$idParams = isset($a_text['ID'])?$a_text['ID']:null;
...
?>
    ...
    <script>
        ...
        switchMenuImg("F5");
        F5_button = function(){
            stringaPHP = "c=<?php echo $c; ?>&a=<?php echo $a; ?>&docType=<?php echo $docType; ?>";
            stringa = "text_parameters.php?"+stringaPHP;
        ...
        function char_consentiti(fieldNumber, id) {
            if(id==undefined){
            ...
            for ( var i = 0; i < valore.length; i++ ) {
                ...
                if (!valore.charAt(i).match(re)) {
                    ...
                    var messageError = "Hai inserito un carattere non valido ' " + char + " ' nel campo ' " + field.text() + " ' ";
                    alert(messageError);
                    // esco
                    return;
                }
    ...
    <form name="<?php echo $table; ?>" id="form_<?php echo $table; ?>" action="text_parameters_exe.php" method="post">
        ...
        <table class="table_interna text_center" border="0">
            ...
            foreach($cls_text->a_textParams[1]['notification'] as $notifType=>$a_notification) {?>

                <tr><td colspan="4"><hr></td></tr>
                <tr><td colspan="4"><?=$a_notification['title']?></td></tr>

                <?php
                for($i=1;$i<=count($a_notification['params']);$i++){?>

                    <tr >
                        <td>
                            <div id="id_<?=$notifType.$i?>"><?=$a_notification['params'][$i]['field']?>:</div>
                        </td>
                        <td></td>
                        <td>
                            <textarea id="<?=$notifType.$i?>" name="<?=$notifType?>[<?=$i?>]" onkeyup="char_consentiti(0,'<?=$notifType.$i?>');" style="width:95%" rows="4"><?php echo $cls_text->a_text[$notifType.'_'.$a_notification['params'][$i]['id']]; ?></textarea>
                        </td>
                        <td>
                            <span class="font12"><?php echo implode("<br>",$a_notification['params'][$i]['var']); ?></span>
                        </td>
                    </tr>
