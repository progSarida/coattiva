...
$testo = new testo_richiesta_decesso($myId);
...
function salva_form() 
{
	if (CheckCampiObbligatori () == true)
	{
		if (CheckTuttiCampi () == true)
		{
			...
			if(control_salva)
       			$("#form_testo_richiesta_decesso").submit();
		}
    }
}
...
function annulla() 
{
	...
	stringa = "testo_richiesta_decesso.php?"+stringaPHP;
...
function CheckTuttiCampi ()
{
	...
	if (CheckCaratteri ("Richiesta_Certificato") == false) return false;
...
$(document).ready(function(){
	...
	$('#form_testo_richiesta_decesso').ajaxForm(
...
<form name="testo_richiesta_decesso" id="form_testo_richiesta_decesso" action="testo_richiesta_decesso_salva.php" method="get">
...
<table class="table_interna text_center" border="0">
	...
	<tr>
		...
		<td>
			<textarea name="Informazioni" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="3"><?=$Informazioni?></textarea>
		</td>
		<td>
			<font size=-2>
			{INFODESTINATARIO}
	...
	<tr>
		<td>
			<div id="Richiesta_Certificato">Richiesta certificato:</div>
		</td>
		<td>
			<font size=-2>CEN</font>
		</td>
		<td>
			<textarea name="Richiesta_Certificato" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="5"><?=$Richiesta_Certificato?></textarea>
		</td>
		<td>
			<font size=-2>
			</font>
