...
function salva_form() 
{
	if (CheckCampiObbligatori () == true)
	{
		if (CheckTuttiCampi () == true)
		{
			...
			if(control_salva)
       			$("#form_testo_richiesta_matricole").submit();
		}
    }
}
...
function annulla() 
{
	...
	stringa = "testo_richiesta_matricole.php?"+stringaPHP;
...
function CheckCampiObbligatori ()
 {
	if (ParolaPresenteInStringa ("Luogo_Data", "{COMUNEGESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Descrizione", "{GESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Descrizione", "{INDIRIZZOGESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Descrizione", "{ENTRATE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta", "{DOVUTO}") == false) return false;

	if (ParolaPresenteInStringa ("PEC", "{PECGESTORE}") == false) return false;
...
{
	if (CheckCaratteri ("Luogo_Data") == false) return false;
	if (CheckCaratteri ("Oggetto") == false) return false;
	if (CheckCaratteri ("Richiesta") == false) return false;
	if (CheckCaratteri ("Descrizione") == false) return false;
	if (CheckCaratteri ("Legge") == false) return false;
	if (CheckCaratteri ("PEC") == false) return false;
...
$(document).ready(function(){
	...
	$('#form_testo_richiesta_matricole').ajaxForm(
...
<table class="table_interna text_center" border="0" cellspacing="10" cellpadding="0">
	<tr>
		<td><font class="titolo font16 under_decor">Testo richiesta matricole</font></td>
	</tr>
</table>
...
<form name="form_testo_richiesta_matricole" id="form_testo_richiesta_matricole" action="testo_richiesta_matricole_salva.php" method="get">
...
<table class="table_interna text_center" border="0">
	...
	<tr>
		<td class="width20">
			<div id="Luogo_Data">Luogo e data:</div>
		</td>
		<td class="width4">
			<font size=-2>CENT</font>
		</td>
		<td>
			<textarea name="Luogo_Data" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="2"><?=$Luogo_Data?></textarea>
		</td>
		<td>
			<font size=-2>
			{COMUNEGESTORE}<br>{DATASTAMPA}
			</font>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<textarea name="Descrizione" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="5"><?=$Descrizione?></textarea>
		</td>
		<td>
			<font size=-2>
			{DOVUTO}
	...
	<tr>
		...
		<td>
			<font size=-2>
			{GESTORE}&nbsp;<br>{INDIRIZZOGESTORE}&nbsp;<br>{ENTRATE}
			</font>
		</td>
	</tr>
	<tr>
		<td>
			<div id="Legge">Legge:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Legge" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="4"><?=$Legge?></textarea>
		</td>
		<td>
			<font size=-2>
			</font>
	...
	<tr>
		...
		<td>
			<textarea name="PEC" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="6"><?=$PEC?></textarea>
		</td>
		<td>
			<font size=-2>
			{PECGESTORE}
	...
	<tr>
		...
		<td>
			<textarea name="Saluti" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="5"><?=$Saluti?></textarea>
		</td>
		<td>
			<font size=-2>
	...
	<tr>
		<td>
			<div id="Intestazione_Firma">Qualifica firma:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Intestazione_Firma" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="3"><?=$Intestazione_Firma?></textarea>
		</td>
		<td>
			<font size=-2>
			{FUNZIONARIORESPONSABILE}<br>{RESPONSABILEPROCEDIMENTO}
			</font>
		</td>
	</tr>
	<tr>
		...
		<td>
			<textarea name="Firma" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="3"><?=$Firma?></textarea>
		</td>
		<td>
			<font size=-2>
			{FUNZIONARIORESPONSABILE}<br>{RESPONSABILEPROCEDIMENTO}
