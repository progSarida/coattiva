...
$firma_incaricato = stripslashes($testo->Firma_Incaricato);
$layout.= "<script>$('[name=Firma][value=".$firma_incaricato."]').prop('checked',true);</script>";
...
function salva_form() 
{
	if (CheckCampiObbligatori () == true)
	{
		if (CheckTuttiCampi () == true)
		{
			...
			if(control_salva)
       			$("#form_testo_esito_rateizzazione").submit();
		}
    }
}
...
function annulla() 
{
	...
	stringa = "testo_esito_rateizzazione.php?"+stringaPHP;
...
function CheckCampiObbligatori ()
 {
	if (ParolaPresenteInStringa ("Richiesta", "{DATARICHIESTA}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta", "{NOMEUTENTE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta", "{COMUNENASCITA}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta", "{DATANASCITA}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta", "{INDIRIZZOUTENTE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta", "{ATTO}") == false) return false;

	if (ParolaPresenteInStringa ("Richiesta_Negata", "{NOMEGESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta_Negata", "{POSIZIONEGESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta_Negata", "{MOTIVAZIONE}") == false) return false;
	
	if (ParolaPresenteInStringa ("Richiesta_Accolta", "{NOMEGESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta_Accolta", "{POSIZIONEGESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Richiesta_Accolta", "{NUMERORATE}") == false) return false;
...
{
	if (CheckCaratteri ("Richiesta") == false) return false;
	if (CheckCaratteri ("Richiesta_Negata") == false) return false;
	if (CheckCaratteri ("Richiesta_Accolta") == false) return false;
	if (CheckCaratteri ("Testo_Accolta") == false) return false;
...
$(document).ready(function(){
	...
	$('#form_testo_esito_rateizzazione').ajaxForm(
...
<table class="table_interna text_center" border="0" cellspacing="10" cellpadding="0">
	<tr>
		<td><font class="titolo font16 under_decor">Testo esito rateizzazione</font></td>
	</tr>
</table>
...
<form name="testo_esito_rateizzazione" id="form_testo_esito_rateizzazione" action="testo_esito_rateizzazione_salva.php" method="get">
...
<table class="table_interna text_center" border="0">
	...
	<tr>
		<td class="width20">
			<div id="Richiesta">Richiesta:</div>
		</td>
		<td class="width4">
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Richiesta" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="5"><?=$richiesta?></textarea>
		</td>
		<td>
			<font size=-2>
			{DATARICHIESTA}&nbsp;<br>{NOMEUTENTE}&nbsp;<br>{COMUNENASCITA}&nbsp;<br>{DATANASCITA}&nbsp;<br>{INDIRIZZOUTENTE}&nbsp;<br>{ATTO}
			</font>
		</td>
	</tr>
	<tr>
		<td>
			<div id="Richiesta_Negata">Richiesta negata:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Richiesta_Negata" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="4"><?=$richiesta_negata?></textarea>
		</td>
		<td>
			<font size=-2>
			{NOMEGESTORE}&nbsp;<br>{POSIZIONEGESTORE}&nbsp;<br>{MOTIVAZIONE}
			</font>
		</td>
	</tr>
	<tr>
		<td>
			<div id="Richiesta_Accolta">Richiesta accolta:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Richiesta_Accolta" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="6"><?=$richiesta_accolta?></textarea>
		</td>
		<td>
			<font size=-2>
			{NOMEGESTORE}&nbsp;<br>{POSIZIONEGESTORE}&nbsp;<br>{NUMERORATE}
			</font>
		</td>
	</tr>
	<tr>
		<td>
			<div id="Testo_Accolta">Testo accolta:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Testo_Accolta" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="6"><?=$testo_accolta?></textarea>
		</td>
		<td>
			<font size=-2>
	...
	<tr>
		...
		<td class="text_left"><input type="radio" name=Firma value=""  checked> Nessuna
			<br>
			<input type="radio" name=Firma value="incaricato" > Incaricato rateizzazione
		<td>
			<font size=-2>
			</font>
		</td>
