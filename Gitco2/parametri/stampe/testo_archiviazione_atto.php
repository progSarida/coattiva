...
//PARAMETRI TESTO
$para_testo = new testo_archivizione_atto(NULL);
$myId = $para_testo->CercaParametroData($c, date("Y-m-d"));
$testo = new testo_archivizione_atto($myId);

$Oggetto = stripslashes($testo->Oggetto);
$Sottotitolo_Oggetto = stripslashes($testo->Sottotitolo_Oggetto);
$Scrivente = stripslashes($testo->Scrivente);
$Premesso = stripslashes($testo->Premesso);
$Premesso_Testo = stripslashes($testo->Premesso_Testo);
$Risultanze_Ufficio = stripslashes($testo->Risultanze_Ufficio);
$Comunica = stripslashes($testo->Comunica);
$Comunica_Testo = stripslashes($testo->Comunica_Testo);
$Informazioni = stripslashes($testo->Informazioni);
$Informazioni_Testo = stripslashes($testo->Informazioni_Testo);
...
function salva_form() 
{
	if (CheckCampiObbligatori () == true)
	{
		if (CheckTuttiCampi () == true)
		{
			...
			if(control_salva)
       			$("#form_testo_archiviazione_atto").submit();
		}
    }
}
...
function annulla() 
{
	...
	stringa = "testo_archiviazione_atto.php?"+stringaPHP;
...
function CheckCampiObbligatori ()
 {
	...
	if (ParolaPresenteInStringa ("Oggetto", "{DOCUMENTO}") == false) return false;
	if (ParolaPresenteInStringa ("Oggetto", "{IDCRONOLOGICO}") == false) return false;
	if (ParolaPresenteInStringa ("Oggetto", "{ANNOCRONOLOGICO}") == false) return false;
	if (ParolaPresenteInStringa ("Oggetto", "{RIFERIMENTO}") == false) return false;
	
	if (ParolaPresenteInStringa ("Sottotitolo_Oggetto", "{TIPORISCOSSIONE}") == false) return false;
	if (ParolaPresenteInStringa ("Sottotitolo_Oggetto", "{ENTEGESTITO}") == false) return false;

	if (ParolaPresenteInStringa ("Scrivente", "{GESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Premesso_Testo", "{UTENTE}") == false) return false;
	
	if (ParolaPresenteInStringa ("Risultanze_Ufficio", "{GESTORE}") == false) return false;
	if (ParolaPresenteInStringa ("Comunica_Testo", "{UTENTE}") == false) return false;
...
function CheckTuttiCampi ()
{
	...
	if (CheckCaratteri ("Scrivente") == false) return false;
	...
	if (CheckCaratteri ("Risultanze_Ufficio") == false) return false;
...
$(document).ready(function(){
	...
	$('#form_testo_richiesta_rateizzazione').ajaxForm(
...
<table class="table_interna text_center" border="0" cellspacing="10" cellpadding="0">
	<tr>
		<td><font class="titolo font16 under_decor">Testo archiviazione atto</font></td>
	</tr>
</table>
...
<form name="testo_richiesta_rateizzazione" id="form_testo_richiesta_rateizzazione" action="testo_richiesta_rateizzazione_salva.php" method="get">
...
<table class="table_interna text_center" border="0">
	<tr>
		...
		<td class="width4">
			<font size=-2 color="red">All.</font>
		</td>
		<td>
			<font size=-2 color="red">Testo Blocchi</font>
		</td>
		<td class="width20">
			<font size=-2 color="red">Campi OBBLIGATORI:</font>
		</td>
	</tr>
	<tr>
		<td colspan=4><br></td>
	...
	<tr>
		...
		<td>
			<font size=-2>
			{DOCUMENTO}<br>{IDCRONOLOGICO}<br>{ANNOCRONOLOGICO}<br>{RIFERIMENTO}
			</font>
		</td>
	</tr>
	<tr>
		...
		<td>
			<textarea name="Sottotitolo_Oggetto" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="3"><?=$Sottotitolo_Oggetto?></textarea>
		</td>
		<td>
			<font size=-2>
	...
	<tr>
		<td>
			<div id="Scrivente">Scrivente:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Scrivente" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="3"><?=$Scrivente?></textarea>
		</td>
		<td>
			<font size=-2>
			{GESTORE}
	...
	<tr>
		...
		<td>
			<textarea name="Premesso" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="2"><?=$Premesso?></textarea>
		</td>
		<td>
			<font size=-2>
	...
	<tr>
		...
		<td>
			<textarea name="Premesso_Testo" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="4"><?=$Premesso_Testo?></textarea>
		</td>
		<td>
			<font size=-2>
			{UTENTE}
	...
	<tr>
		<td>
			<div id="Dettaglio_Atto">Dettaglio Atto:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea class="sfondo_grigio text_center" name="Dettaglio_Atto" readonly style="width:95%" rows="1">NON EDITABILE</textarea>
		</td>
		<td>
			<font size=-2>
	...
	<tr>
		<td>
			<div id="Risultanze_Ufficio">Risultanze Ufficio:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea name="Risultanze_Ufficio" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="4"><?=$Risultanze_Ufficio?></textarea>
		</td>
		<td>
			<font size=-2>
			{GESTORE}
	...
	<tr>
		...
		<td>
			<textarea name="Comunica_Testo" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="4"><?=$Comunica_Testo?></textarea>
		</td>
		<td>
			<font size=-2>
			{UTENTE}
	...
	<tr>
		<td>
			<div id="Dettaglio_Archiviazione">Dettaglio archiviazione:</div>
		</td>
		<td>
			<font size=-2>GIUS</font>
		</td>
		<td>
			<textarea class="sfondo_grigio text_center" name="Dettaglio_Archiviazione" readonly style="width:95%" rows="1">NON EDITABILE</textarea>
		</td>
		<td>
			<font size=-2>
	...
	<tr>
		...
		<td>
			<textarea name="Informazioni" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="2"><?=$Informazioni?></textarea>
		</td>
		<td>
			<font size=-2>
			</font>
	...
	<tr>
		...
		<td>
			<textarea name="Informazioni_Testo" onblur="CheckCaratteri(this);" onchange="CheckCampiObbligatori();" style="width:95%" rows="2"><?=$Informazioni_Testo?></textarea>
		</td>
		<td>
			<font size=-2>
			</font>
