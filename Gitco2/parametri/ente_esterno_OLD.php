...
$QUERY = $cls_param->Get_Query_EnteEs($c,$tipo_ente);

$a_param = $cls_db->getArrayLine($cls_db->ExecuteQuery($QUERY["EnteE"]));
...
$QUERY = $cls_param->Get_Query_EnteEs($c,$tipo_ente, $ID_ente);
$array_doc = array();
...
function carica_doc(value)
{
	...
	openWindowSearch(stringa,{width:800, height:600, left:(($(window).width()/2)-400), top:(($(window).height()/2)-300)});
...
<div class="row">
	<div class="col col-lg-4 col-lg-offset-1">
		<div class="form-group">
			...
			<div class="col-lg-8">
				<input class="form-control resize validateCustom vld_Custom_r" style="background-color: rgb(153, 204, 255); border: 2px solid black;" readonly tabindex=3 name=comune id=comune_id value="<?php echo $a_param["Comune"]; ?>" onclick="cerca_comune();">
			</div>
		</div>
	</div>
	...
	<div class="col col-lg-2">
		<div class="form-group">
			...
			<div class="col-lg-8">
				<input class="form-control resize validateCustom vld_Custom_r" style="background-color: #97CFDD; border: 2px solid black;" readonly tabindex=5 id=cap_id name=cap size=4 value="<?php echo $a_param["Cap"]; ?>">
			</div>
		</div>
	</div>
</div>
