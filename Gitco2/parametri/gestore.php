...
$a_img = array(
    "flag"=>true,
    "filename"=>$a_enteAdmin["Gestore_File_Firma"],
    "path"=>SUPER_ROOT."/archivio/firme/".$c."/".$a_enteAdmin["Gestore_File_Firma"],
    "webpath"=>SUPER_WEB_ROOT."/archivio/firme/".$c."/".$a_enteAdmin["Gestore_File_Firma"]
);
if(is_file($a_img['path'])){
    $imagick = new Imagick($a_img['path']);
    $a_img['dim'] = $imagick->getImageGeometry();
}
else
    $a_img = $a_defaultImg;
...
F11_button = function(){
    ...
    $("#frameHelp").attr("src","<?= SUPER_WEB_ROOT."/archivio/help/Gestore.pdf"; ?>");
    $("#helpModalLabel").empty().append("<b>Help Gestore</b>");
...
<form class="form-horizontal validate" name=form_gestore id=form_gestore method=post action="gestore_salva.php" enctype="multipart/form-data">
...
<div id=gestore style="margin-top: 1%;">
	...
	<div class="row" style="margin-top: 1%;">
		<div class="col col-lg-3 col-lg-offset-1" >
			<div class="form-group">
				...
				<div class="col-lg-8">
					 <input style="width: 100%; background-color: rgb(153, 204, 255); border: 2px solid black;" class=" text_left form-control resize" readonly name=comune id=comune_id value="<?php echo $a_enteAdmin["Gestore_Comune"]; ?>" size=15 ondblclick="/*cerca_comune();*/openOfcanvas('citySearchModal',0);">
	      </div>
			</div>
		</div>
	...
	<div class="row" style="margin-top: 1%;">
		...
		<div class="col col-lg-4">
			<div class="form-group">
				...
				<div class="col-lg-8">
					 <input style="width: 85%;" class="text_left form-control vld_CFPI resize" id=CF_id name=CF size=20 value="<?php echo $a_enteAdmin["Gestore_CF"]; ?>" >
				</div>
			</div>
		</div>
	</div>
	...
	<div class="col col-lg-10 col-lg-offset-1">
			<div class="form-group">
				<label class="col-lg-1 control-label resize" style="text-align: left;">Abilitazione</label>
				<div class="col-lg-11">
					<textarea style="max-width: 100%;" class="text_left form-control resize" id=abilitazione_id name=abilitazione rows="6" value="<?php echo $a_enteAdmin["Gestore_Abilitazione"]; ?>" ><?php echo $a_enteAdmin["Gestore_Abilitazione"]; ?></textarea>
			  </div>
			</div>
		</div>
	</div>
	...
	<div class="row justify-content-md-center ">
	<div class="col col-md-auto text_center">
			<p class="titolo font16 under_decor">Dati Rimborso Spese ex art.17, c. 3 D.lgs. 13 aprile 1999, n.112 e s.m.i.</p>
	</div>
	</div>
	<div class="row">
		<div class="col col-lg-3 col-lg-offset-1">
			<div class="form-group">
				...
				<div class="col-lg-8">
					 <input class="text_left form-control resize" id=Intestatario_id name=Intestatario size=18 value="<?php echo isset($a_enteAdmin["Gestore_Intestatario"])?$a_enteAdmin["Gestore_Intestatario"]:null; ?>" >
			  </div>
			</div>
		</div>
		<div class="col col-lg-3">
			<div class="form-group">
				<label class="col-lg-4 control-label resize" style="text-align: left;">Conto Corrente</label>
				<div class="col-lg-8">
					 <input class="text_left form-control resize" id=Conto_Corrente_id name=Conto_Corrente value="<?php echo isset($a_enteAdmin["Gestore_Conto_Corrente"])?$a_enteAdmin["Gestore_Conto_Corrente"]:null; ?>" >
				</div>
			</div>
		</div>
		<div class="col col-lg-4">
			<div class="form-group">
				<label class="col-lg-4 control-label resize">IBAN</label>
				<div class="col-lg-8">
					 <input class="text_left form-control vld_IBAN" id=IBAN_id name=IBAN size=16 value="<?php echo isset($a_enteAdmin["Gestore_IBAN"])?$a_enteAdmin["Gestore_IBAN"]:null; ?>" >
				</div>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col col-lg-3 col-lg-offset-1" >
			<div class="form-group">
				<label class="col-lg-4 control-label resize" style="text-align: left;">Scad. pag. Giorno</label>
				<div class="col-lg-8">
					 <input type="text" id=Scadenza_Giorno class="text_right form-control resize vld_int"  name="Scadenza_Giorno" 	value="<?php echo isset($a_enteAdmin["Gestore_Scadenza_Giorno"])?$AddZero($a_enteAdmin["Gestore_Scadenza_Giorno"]):null; ?>" >
				</div>
			</div>
		</div>
		<div class="col col-lg-3">
			<div class="form-group">
				<label class="col-lg-4 control-label resize" style="text-align: left;">Mese</label>
				<div class="col-lg-8">
					 <input type="text" id=Scadenza_Mese class="text_right form-control resize vld_int"  name="Scadenza_Mese" value="<?php echo isset($a_enteAdmin["Gestore_Scadenza_Mese"])?$AddZero($a_enteAdmin["Gestore_Scadenza_Mese"]):null; ?>" >
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="row" style="display: flex;margin-top: 1%;">
        <div class="col col-lg-6 col-lg-offset-1" style="margin: auto;">
            <div class="form-group" >
                <label class="col-lg-4 control-label resize" style="text-align: left;">Immagine firma</label>
                <div class="col-lg-8" >
                    <input class="form-control resize" type="file" name="File_Firma" value="Carica immagine" style="width: 100%; background-color: rgb(153, 204, 255);">
                </div>
            </div>
        </div>
        <div class="col-lg-2">
            <div id=mostra_immagine class="image-magnify4 resize" title="Clicca per allargare immagine" onclick="window.open('<?php echo $a_img['webpath']; ?>')">
                <div class="thumbnail4 text_center">
                    <img id="thumbnail_image4" src="<?php echo $a_img['webpath']; ?>" border="1>">
                    <div class="popup4"></div>
                </div>
            </div>
        </div>
        <div class="col-lg-2 resize" style="margin: auto;">
            <?php if($a_img['flag']) : ?>
                <button type="button" class="btn btn-danger px-3" id="DeleteFileID"  onclick="DeleteFile('<?= $a_img['path'] ?>');" ><i class="fa fa-times-circle" aria-hidden="true"></i> Elimina</button>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function DeleteFile(path){
        $.ajax({
            type: "POST",
            async: true,
            url: "ajax/ajax_delete_file_gest.php",
            data: {
                path: path,
            ...
            success: function(value) {
                ...
                if(value == "OK") {
                    $("#DeleteFileID").hide();
                    $("#thumbnail_image4").attr("src", "<?= $a_defaultImg['webpath'] ?>");
                    $("#mostra_immagine").attr("onclick", "window.open('<?= $a_defaultImg['webpath'] ?>')");

                    dimensiona_magnify("4", "<?php echo $a_defaultImg['dim']['width']; ?>", "<?php echo $a_defaultImg['dim']['height']; ?>", 150, 150);
                }
            }
        });
    }
	...
	$( window ).load(function() {
        ...
        if("<?php echo $a_img['dim']['width']; ?>">0)
            dimensiona_magnify("4", "<?php echo $a_img['dim']['width']; ?>" , "<?php echo $a_img['dim']['height']; ?>" , 150, 150 );
