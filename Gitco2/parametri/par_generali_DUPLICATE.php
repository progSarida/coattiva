    ...
    function updateInputs(spese,testo_spese,SMA,intestatario_SMA,numero_SMA){
        ...
        if(spese=="y"){
            $('#testo_spese').prop('readonly',false).toggleClass("readonly").val(testo_spese);
						//$('#testo_spese').addClass("vld_req");
        }//else $('#testo_spese').removeClass("vld_req");
        ...
        if(SMA=="y"){
            ...
            $('#numero_SMA').prop('readonly',false).toggleClass("readonly").val(numero_SMA);
						//$('#intestatario_SMA').addClass("vld_req");
						//$('#numero_SMA').addClass("vld_req");
        }else{
					//$('#intestatario_SMA').removeClass("vld_req");
					//$('#numero_SMA').removeClass("vld_req");
				}

				//$('.validate').bootstrapValidator('revalidateField', $("#testo_spese"));
				//$('.validate').bootstrapValidator('revalidateField', $("#intestatario_SMA"));
				//$('.validate').bootstrapValidator('revalidateField', $("#numero_SMA"));
    }
    ...
    function changeSMA(){
        ...
        if(SMA=="y"){
            ...
            $('#numero_SMA').prop('readonly',false).removeClass("readonly");
						//$('#intestatario_SMA').addClass("vld_req");
						//$('#numero_SMA').addClass("vld_req");
						var classList = document.getElementById('intestatario_SMA').className.split(/\s+/);
						var allClass = "";
						for (var i = 0; i < classList.length; i++) {
						    allClass += " " + classList[i];
							}
							alert(allClass);
							//$('.validate').bootstrapValidator('enableFieldValidators','ClassRequired', true, 'notEmpty');
        }
        else{
            ...
            $('#numero_SMA').prop('readonly',true).addClass("readonly").val("");
						...
						for (var i = 0; i < classList.length; i++) {
							...
							//$('.validate').bootstrapValidator('enableFieldValidators','ClassRequired', false, 'notEmpty');



							//$('.validate').data('bootstrapValidator').resetField($("#intestatario_SMA"));
							//$('.validate').data('bootstrapValidator').resetField($("#numero_SMA"));
							//$('.validate').bootstrapValidator('updateStatus', $("#intestatario_SMA"), 'NOT_VALIDATED', 'lessThan');
							//$("#intestatario_SMA").closest('.validate').bootstrapValidator('revalidateField', $("#intestatario_SMA"));
        }
				//$('.validate').bootstrapValidator('revalidateField', $("#intestatario_SMA"));
				//$(".validate").data('bootstrapValidator').revalidateField($("#intestatario_SMA"));

				//$("#intestatario_SMA").closest('.validate').bootstrapValidator('revalidateField', "ClassRequired");
				//$('.validate').bootstrapValidator('revalidateField', $("#numero_SMA"));
			//	$('.validate').bootstrapValidator('updateStatus', 'ClassRequired', 'NOT_VALIDATED')
       //.bootstrapValidator('validateField', 'ClassRequired');
			 //var fv = $('.validate').data('formValidation');
  	 		//fv.resetField('#intestatario_SMA', true);
    }
    ...
    function changeSpese(){
        ...
        if(spese_anticipate=="y"){
            ...
            $('#testo_spese').prop('readonly',true).addClass("readonly").val("");
						//$('#testo_spese').removeClass("vld_req");
        }
...
<div class="row">
	...
	<div class="col-lg-2">
		<div class="form-group ">
			<input class="readonly width80 form-control vld_req resize" name="intestatario_SMA" id="intestatario_SMA" readonly>
		</div>
	</div>
	...
	<div class="col-lg-4">
		<div class="form-group ">
		  <input class="readonly width80 form-control vld_req resize" name="numero_SMA" id="numero_SMA" readonly>
		</div>
	</div>
</div>
...
<div class="row">
	...
	<div class="col-lg-7">
		<div class="form-group ">
			<textarea class="readonly form-control vld_req resize" name="testo_spese" id="testo_spese" style="max-width: 100%;" readonly></textarea>
		</div>
	</div>
</div>
<div class="row">
	<div class="col-lg-10 col-lg-offset-1 text_left vld_req resize"><span class="titolo">RESTITUZIONE MOD.23L - RACCOMANDATA A.G.</span></div>
</div>
