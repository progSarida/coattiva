...
$a_para_Resp = $cls_db->getArrayLineNull($cls_db->ExecuteQuery($cls_param->Get_Query_Resp($c,$tipo_riscossione)),"parametri_responsabili");

$par_id = $a_para_Resp["ID"];
if($par_id==null) $par_id = 0;

$a_width = array(0,0,0,0,0);
$a_height = array(0,0,0,0,0);
$a_border = array(0,0,0,0,0);
...
$firma_funz_risc = $cls_param->firmaSingola("Funzionario_Riscossione",$a_para_Resp);
...
if($par_id != null)
{
	...
	if($firma_resp_proc['path_firma']!="" && $a_para_Resp["Responsabile_Testo"]!="si" && is_file($firma_resp_proc['path_firma']))
	{
		...
		$d = $img_responsabile->getImageGeometry();
		$a_width[1] = $d['width'];
		$a_height[1] = $d['height'];
		$a_border[1] = 1;
	}
	...
	if($firma_uff_risc['path_firma']!="" && $a_para_Resp["Ufficiale_Testo"]!="si" && is_file($firma_uff_risc['path_firma']))
	{
		...
		$d = $img_ufficiale->getImageGeometry();
		$a_width[2] = $d['width'];
		$a_height[2] = $d['height'];
		$a_border[2] = 1;
	}
	...
	if($firma_resp_rich['path_firma']!="" && $a_para_Resp["Responsabile_Richieste_Testo"]!="si" && is_file($firma_resp_rich['path_firma']))
	{
		...
		$d = $img_resp->getImageGeometry();
		$a_width[3] = $d['width'];
		$a_height[3] = $d['height'];
		$a_border[3] = 1;
	}

	if($firma_funz_risc['path_firma']!="" && $a_para_Resp["Funzionario_Riscossione_Testo"]!="si" && is_file($firma_resp_rich['path_firma']))
	{
		$img_resp = new Imagick($firma_resp_rich['path_firma']);
		$d = $img_resp->getImageGeometry();
		$a_width[4] = $d['width'];
...
class Signature {
    constructor(LG, RP, UR, RR, FR) {
        this.legaleRappresentante = LG;
        this.responsabileProcedimento = RP;
        this.ufficialeRiscossione = UR;
        this.responsabileRichieste = RR;
		this.funzionarioRiscossione = FR;
    }
}

var PathFile = new Signature('<?php echo $firma_funz_resp['firma']; ?>','<?php echo $firma_resp_proc['firma']; ?>','<?php echo $firma_uff_risc['firma']; ?>','<?php echo $firma_resp_rich['firma']; ?>','<?php echo $firma_funz_risc['firma']; ?>');

$(document).ready(function(){

	dimensiona_magnify("0", "<?php echo $a_width[0]; ?>" , "<?php echo $a_height[0]; ?>" , 150, 100 );
	dimensiona_magnify("1", "<?php echo $a_width[1]; ?>" , "<?php echo $a_height[1]; ?>" , 150, 100 );
	dimensiona_magnify("2", "<?php echo $a_width[2]; ?>" , "<?php echo $a_height[2]; ?>" , 150, 100 );
	dimensiona_magnify("3", "<?php echo $a_width[3]; ?>" , "<?php echo $a_height[3]; ?>" , 150, 100 );
	dimensiona_magnify("4", "<?php echo $a_width[4]; ?>" , "<?php echo $a_height[4]; ?>" , 150, 100 );
...
function cambia_tipo(){
	...
	if($('#scelta_firma_funzionario').val()=="firma"){
		...
		else removeClass("funzionario_firma","vld_Custom_r");//$("#funzionario_firma").removeClass("validateCustom vld_Custom_r");
	}
	else{
		$('#manuale_funzionario').hide();
		removeClass("funzionario_firma","vld_Custom_r");
	...
	if($('#scelta_firma_responsabile').val()=="firma"){
		...
		else removeClass("responsabile_firma","vld_Custom_r");//$("#responsabile_firma").removeClass("validateCustom vld_Custom_r");
	}
	else{
		$('#manuale_responsabile').hide();
		removeClass("responsabile_firma","vld_Custom_r");
	...
	if($('#scelta_firma_ufficiale').val()=="firma"){
		...
		else removeClass("ufficiale_firma","vld_Custom_r");//$("#ufficiale_firma").removeClass("validateCustom vld_Custom_r");
	}
	else{
		$('#manuale_ufficiale').hide();
		removeClass("ufficiale_firma","vld_Custom_r");
	...
	if($('#scelta_firma_responsabile_richieste').val()=="firma"){
		...
		else removeClass("richieste_firma","vld_Custom_r");//$("#richieste_firma").removeClass("validateCustom vld_Custom_r");
	}
	else{
		$('#manuale_responsabile_richieste').hide();
		removeClass("richieste_firma","vld_Custom_r");
		//$("#richieste_firma").removeClass("validateCustom vld_Custom_r");
	}

	if($('#scelta_firma_funzionario_riscossione').val()=="firma"){
		$('#manuale_funzionario_riscossione').show();
		if($("#funz_risc").val()!="" && !fileExists(PathFile.funzionarioRiscossione)) $("#funz_risc_firma").addClass("validateCustom vld_Custom_r");
		else removeClass("funz_risc_firma","vld_Custom_r");//$("#funz_risc_firma").removeClass("validateCustom vld_Custom_r");
	}
	else{
		$('#manuale_funzionario_riscossione').hide();
		removeClass("funz_risc_firma","vld_Custom_r");
		//$("#funz_risc_firma").removeClass("validateCustom vld_Custom_r");
	}
}
</script>
...
<input type=hidden name=titolo_riscossione value=<?php echo $titolo_riscossione; ?> />
...
<input type=hidden name=funz_risc_firma value=<?php echo $a_para_Resp["Funzionario_Riscossione_Firma"]; ?> />
...
<div class="row" style="margin-top: 2%;">
	<div class="col col-lg-5 col-lg-offset-1">
		<div class="form-group">
			<label class="col-lg-5 control-label resize" style="text-align: left;"><span id="testo_funz_legale">Funzionario responsabile</span>/Amministratore Unico</label>
...
<div class="row" id="manuale_funzionario">
	<div class="col col-lg-10 col-lg-offset-1">
		<div class="form-group">
			...
			<div class="col-lg-4">
				<div id=mostra_immagine0 class="image-magnify0" title="Clicca per allargare immagine" onclick="window.open('<?php echo $firma_funz_resp['firma']; ?>')">
					<div class="thumbnail0 text_center">
						<img id="thumbnail_image0" src="<?php echo $firma_funz_resp['firma']; ?>" border="<?php echo $a_border[0]; ?>">
...
<div class="row" style="margin-top: 2%;">
	<div class="col col-lg-5 col-lg-offset-1">
		<div class="form-group">
			<label class="col-lg-5 control-label resize" style="text-align: left;">Responsabile del procedimento/Responsabile Polizia-Area Fininziaria - ...</label>
			<div class="col-lg-6 col-lg-offset-1">
...
<div class="row" id="manuale_responsabile">
	<div class="col col-lg-10 col-lg-offset-1">
		<div class="form-group">
			...
			<div class="col-lg-4">
				<div id=mostra_immagine1 class="image-magnify1" title="Clicca per allargare immagine" onclick="window.open('<?php echo $firma_resp_proc['firma']; ?>')">
					<div class="thumbnail1 text_center">
						<img id="thumbnail_image1" src="<?php echo $firma_resp_proc['firma']; ?>" border="<?php echo $a_border[1]; ?>">
...
<div class="row" style="margin-top: 2%;">
	<div class="col col-lg-5 col-lg-offset-1">
		<div class="form-group">
			<label class="col-lg-5 control-label resize" style="text-align: left;">Ufficiale della Riscossione/Il Messo Notificatore</label>
			<div class="col-lg-6 col-lg-offset-1">
...
<div class="row" id="manuale_ufficiale">
	<div class="col col-lg-10 col-lg-offset-1">
		<div class="form-group">
			...
			<div class="col-lg-4">
				<div id=mostra_immagine2 class="image-magnify2" title="Clicca per allargare immagine" onclick="window.open('<?php echo $firma_uff_risc['firma']; ?>')">
					<div class="thumbnail2 text_center">
						<img id="thumbnail_image2" src="<?php echo $firma_uff_risc['firma']; ?>" border="<?php echo $a_border[2]; ?>">
...
<div class="row" id="manuale_responsabile_richieste">
	<div class="col col-lg-10 col-lg-offset-1">
		<div class="form-group">
			...
			<div class="col-lg-4">
				<div id=mostra_immagine3 class="image-magnify3" title="Clicca per allargare immagine" onclick="window.open('<?php echo $firma_resp_rich['firma']; ?>')">
					<div class="thumbnail3 text_center">
						<img id="thumbnail_image3" src="<?php echo $firma_resp_rich['firma']; ?>" border="<?php echo $a_border[3]; ?>">
...
<div class="row" style="margin-top: 2%;">
	<div class="col col-lg-5 col-lg-offset-1">
		<div class="form-group">
			<label class="col-lg-5 control-label resize" style="text-align: left;">Funzionario della riscossione/Funzionario Responsabile della Riscossione</label>
			<div class="col-lg-6 col-lg-offset-1">
					<input class="form-control resize" name=funz_risc id=funz_risc value="<?php echo $a_para_Resp["Funzionario_Riscossione"]; ?>" >
			</div>
		</div>
	</div>
	<div class="col col-lg-4 col-lg-offset-1">
		<div class="form-group">
			<label  class="col-lg-2 control-label resize">Telefono</label>
			<div class="col-lg-9 col-lg-offset-1">
					<input name=tel_funz_risc id=tel_funz_risc style="width: 80%;" class="form-control resize text_right vld_tel int" value="<?php echo $a_para_Resp["Funzionario_Riscossione_Telefono"]; ?>" size=15>
			</div>
		</div>
	</div>
</div>
<div class="row">
	<div class="col col-lg-5 col-lg-offset-1">
		<div class="form-group">
			<label class="col-lg-5 control-label resize" style="text-align: left;">Tipo di firma</label>
			<div class="col-lg-6 col-lg-offset-1">
				<select id="scelta_firma_funzionario_riscossione" name="scelta_firma_funzionario_riscossione" class="form-control req resize" onchange="cambia_tipo();">
					<option value="firma">Firma digitale</option>
					<option value="testo">Testo sostitutivo</option>
				</select>
			</div>
...
<div class="row" id="manuale_funzionario_riscossione">
	<div class="col col-lg-10 col-lg-offset-1">
		<div class="form-group">
			<label class="col-lg-3 control-label resize" style="text-align: left;">FIRMA DIGITALE</label>
			<div class="col-lg-5">
				<input class="form-control resize" style="width: 100%; background-color: rgb(153, 204, 255);" type="file" name="funz_risc_firma" id="funz_risc_firma" value="Carica immagine">
			</div>
			<div class="col-lg-4">
				<div id=mostra_immagine4 class="image-magnify4" title="Clicca per allargare immagine" onclick="window.open('<?php echo $firma_funz_risc['firma']; ?>')">
					<div class="thumbnail4 text_center">
						<img id="thumbnail_image4" src="<?php echo $firma_funz_risc['firma']; ?>" border="<?php echo $a_border[4]; ?>">
...
<script type="text/javascript">
	...
	$( window ).load(function() {
		...
		if("<?php echo $a_para_Resp["Funzionario_Riscossione_Testo"]; ?>"!="si") $('#scelta_firma_funzionario_riscossione').val('firma');
		else $('#scelta_firma_funzionario_riscossione').val('testo');
