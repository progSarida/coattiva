...
$rs = new cls_db();
$row = $rs->getArrayLine($rs->SelectQuery("SELECT * FROM text_parameters where Form_Type_ID=".$form_type." AND CC = '$c'"));
$cityText = $adminCityName;
$submit = "Update";
if($row['Form_Type_ID']==null){
    $row = $rs->getArrayLine($rs->SelectQuery("SELECT * FROM text_parameters where Form_Type_ID=".$form_type." AND CC = '*****'"));
