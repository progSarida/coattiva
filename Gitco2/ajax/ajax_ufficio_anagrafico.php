...
switch($ajax)
{
	...
	if($ComuneQuery=="" && $TipoQuery == "" && $ID > 0) $query = "SELECT * FROM ufficio_comune WHERE ID = {$ID} ";
	else if($ComuneQuery != "" && $TipoQuery != "") $query = "SELECT * FROM ufficio_comune WHERE CC = '{$ComuneQuery}' AND Tipo = '{$TipoQuery}' ORDER BY Comune";
	else { echo null; die;}
