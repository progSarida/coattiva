...
for($i = 0;$i<$count;$i++){
    ...
    $tableElem[$i]['Tipo_Banca'] = 'Sede';
    $tableElem[$i]["action_row"] = 'initialId("bank_headq",'.str_replace("'","&apos;",json_encode($result[$i])).');$(".offcanvas").modal("hide");';
    $tableElem[$i]["select"] = "<i onclick='initialId(\"bank_headq\",".str_replace("'","&apos;",json_encode($result[$i]))."); $(\".offcanvas\").modal(\"hide\");' style='cursor: pointer;color: #0c63e4;' class='fas fa-arrow-right'></i>";
}
