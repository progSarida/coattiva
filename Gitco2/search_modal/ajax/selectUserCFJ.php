...
$ric_CDF = $cls_help->getVar('user_cf');                        // input
...
//if($lock == 'N'){
    ...
    $query.= "WHERE Genere != 'D' AND (Codice_Fiscale like '".$ric_CDF."%' and CC_Comune='".$admin."')) ";
    $query.= "UNION ";
    ...
    $query.= "WHERE Genere = 'D' AND (Partita_Iva like '".$ric_CDF."%' AND CC_Comune='".$admin."')) ";
/*}
...
for($i = 0;$i<$count;$i++){
    ...
    $tableElem[$i]["action_row"] = 'initialId("cf",'.str_replace("'","&apos;",json_encode($result[$i])).');$(".offcanvas").modal("hide");';
    $tableElem[$i]["select"] = "<i onclick='initialId(\"cf\",".str_replace("'","&apos;",json_encode($result[$i]))."); $(\".offcanvas\").modal(\"hide\");' style='cursor: pointer;color: #0c63e4;' class='fas fa-arrow-right'></i>";
}
