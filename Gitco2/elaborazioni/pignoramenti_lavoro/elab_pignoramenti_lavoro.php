...
$updatePartitaQuery = "UPDATE partita_tributi AS PT LEFT JOIN terzo_pvt as TP ON TP.Utente_ID = PT.Utente_ID ".
"SET PT.Elaboration_Id=null, PT.Position_Status_Id=null, PT.flag_elaboration=null ".
"WHERE PT.Elaboration_Id=".$last_el_id." AND TP.Utente_ID is null";
...
and (select count(TP.Utente_ID) from terzo_pvt as TP where TP.Utente_ID = U.ID) >0
GROUP BY P.ID";
...
if (count($pignoramenti) > 0) {
     ...
     */
        ...
        try {
            ...
            foreach($pignoramenti as $key=>$row)
            {
                 ...
                 */
                    ...
                    // }
                    // else {
                    //     $BasePagamento = $a_data['TOTALE_PAGAMENTI']-$a_data['Atto_Diritto_Riscossione'];
                ...
                */
                ...
                $pignoPressoTerzi = new PignoramentoPressoTerzi($cls_db);
            ...
            $attinotifica = new cls_PignoramentoNotificatoTerzo($cls_db);
        ...
        $storico->insRow('E', "Elaborato '".$a_elaboration['Description']."': Pignoramenti presso datore di lavoro ".$ente['Denominazione']."[".$a_elaboration['CC']."]. Stato 'Richiesta INIPEC'");
