...
$redirectPage = ELAB_PIGNORAMENTI_LAVORO_WEB."/mgmt_pignoramenti_lavoro.php?c=".$c."&a=".$a."&el=".$a_elab_list['Elaboration_Id'];
?>
...
$query_v_pignoramenti = "SELECT * FROM v_pignoramento_terzi_stampa WHERE Elaboration_List_Id = ".$filter['el_list_id']." AND CC = '" . $filter['city'] . "' ";
...
$prendiTerzi = function($a_results_terzo) use ($cls_db)
{
    $ret = "";
    foreach($a_results_terzo as $item)
    {
        $ret.=$item["Cognome_Ditta"].", ";
    }
...
$prendiTerzoHeader = function ($a_results_terzo,&$lista_terzi_fatti,&$Terzo) use ($cls_db,$cls_registry)
{
    ...
    {
        if(!in_array($terzo["Utente_ID"], $lista_terzi_fatti)) 
        {
            $id = $terzo["Utente_ID"];
            $Terzo = $terzo['Cognome_Ditta'];
    ...
    $return =  $cls_registry->printHeaderTerzo($result[0]);
    return $return;
};


include_once "crea_relata_originale.php";
//end funzioni
...
try{
    for ($i = 0; $i < count($a_results); $i++) {
        ...
        if ($isDebitore($a_results,$i))
        {
            $sono_terzo = false;
            $a_results[$i]['Terzi']  =  $prendiTerzi($a_results_terzo);
            $a_recipientHeader = $cls_registry->printHeader($a_results[$i]);
            if($originale==0)
            {
                $a_results[$i]['PrintType'] = "ORIGINALE";
                $suffisso_file_pdf = "Originale";
                // crea anche la relata

                $RelataOriginale = new RelataOriginale($cls_db);
                $RelataOriginale->partita_id = $a_results[$i]["Partita_ID"];
                $RelataOriginale->tipo = $filter['printType'];
                if ($filter['printType']=="final") {
                    $finalFileRelataOriginale = $crea_file_name($a_results[$i],"Relata_originale");
                    $RelataOriginale->finalFileRelataOriginale = $finalFileRelataOriginale;
                }
                
                $testo_relata_originale = $RelataOriginale->GeneraRelata();
        ...
        {   
            ...
            $a_results[$i]['Terzi'] = $Terzo;
            $a_results[$i]['PrintType'] = "COPIA TERZO";
            $suffisso_file_pdf = "Copia_terzo";
            $suffisso_file_relata_pdf = "Relata_terzo";
        }
        ...
        // if($type_id==1) continue; //se è diretta non fare relata
        // if($a_elab_list['PrintTypeId']==1) continue; // se lo stampatore è 1 non fare la relata

        if(($type_id==1) || ($a_elab_list['PrintTypeId']==1))
        {
            if (!$sono_terzo)
                if($originale==0) 
                {
        ...
        if (($filter['printType'] == "final"))
        {
            ...
            if($originale==0)
                ...
                if ($testo_relata_originale!= "") $pdf->writeHTML($testo_relata_originale);
            else
...
if (count($a_results) == 0)
    ...
    if($filter['printType'] == "temp"){
        ...
        $storico->insRow('E', "Stampa definitiva elemento ".$cls_help->getVar('el_list_id')." elaborazione ".$elabs['Description'].": Pignoramenti presso datore di lavoro ".$ente['Denominazione']."[".$elabs['CC']."]");
