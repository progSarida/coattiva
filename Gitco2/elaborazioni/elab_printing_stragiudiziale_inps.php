...
$nato_da = $help->getVar("born_from");
$nato_a = $help->getVar("born_to");
...
$query = 'SHOW FUNCTION STATUS where Name = "SPLIT_STRING";';
$function = $db->getResults($db->ExecuteQuery($query));


if(count($function) == 0){
    $query = 'SET GLOBAL log_bin_trust_function_creators = 1;';
    $db->ExecuteQuery($query);
    $query = 'CREATE FUNCTION SPLIT_STRING(str VARCHAR(2000), delim VARCHAR(12), pos INT) RETURNS VARCHAR(255) RETURN REPLACE(SUBSTRING(SUBSTRING_INDEX(str, delim, pos), CHAR_LENGTH(SUBSTRING_INDEX(str, delim, pos-1)) + 1), delim, "");';
    $db->ExecuteQuery($query);
}
else{
    ...
    $query = 'SET GLOBAL log_bin_trust_function_creators = 1;';
    $db->ExecuteQuery($query);
    $query = 'DROP FUNCTION SPLIT_STRING;';
    $db->ExecuteQuery($query);
    $query = 'CREATE FUNCTION SPLIT_STRING(str VARCHAR(2000), delim VARCHAR(12), pos INT) RETURNS VARCHAR(255) RETURN REPLACE(SUBSTRING(SUBSTRING_INDEX(str, delim, pos), CHAR_LENGTH(SUBSTRING_INDEX(str, delim, pos-1)) + 1), delim, "");';
    $db->ExecuteQuery($query);
}
...
$query = 'SELECT PT.CC, PT.ID AS Partita_ID 
    ...
    LEFT JOIN appeal AS APP ON APP.Act_ID = A.ID
    LEFT JOIN enti_gestiti AS EG ON EG.CC = PT.CC
    LEFT JOIN partite_check_stra AS UCS ON UCS.CC = PT.CC AND UCS.Partita_ID = PT.ID AND UCS.flag_check = 1 AND UCS.type_stra_id = 2
    ...
    where (IF(APP.Start_Date is NULL,"noRicorso",IF(COALESCE(APP.End_Date,"1900-01-01") < NOW(),"noRicorso","siRicorso")) = "noRicorso" ) 
    AND NOT EXISTS (SELECT * FROM previdenziale_utente AS b_u WHERE  b_u.Utente_ID = PT.Utente_ID )
    AND PT.Is_Discharged = 0 AND (PT.Is_Expired <> 1 OR PT.Is_Expired IS NULL) AND A.DocumentTypeId in (2, 4) AND PT.CC = "'.$c.'"
    AND (
               CASE
                    ...
                    ELSE "Non valida" COLLATE utf8mb4_unicode_ci
               END
           ) = "Attiva" COLLATE utf8mb4_unicode_ci
           AND IF(APP.Start_Date is NULL,"noRicorso",IF(COALESCE(APP.End_Date,"1900-01-01") < NOW(),"noRicorso","siRicorso")) = "noRicorso"
           AND IF(A.Data_Richiesta_Rate IS NULL, "noRate",IF(SPLIT_STRING(A.Scadenze_Rate, "*", A.Rate_Previste) < NOW(),"noRate","siRate")) = "noRate"
    ';

if($nato_da != null && $nato_a != null){
    $query .= " AND U.Data_Nascita >= '".$date->GetDateDB($nato_da,"IT")."' AND U.Data_Nascita <= '".$date->GetDateDB($nato_a,"IT")."' ";
}
else if($nato_da != null){
    $query .= " AND U.Data_Nascita >= '".$date->GetDateDB($nato_da,"IT")."' ";
}
else if($nato_a != null){
    $query .= " AND U.Data_Nascita <= '".$date->GetDateDB($nato_a,"IT")."' ";
}
...
$a_enteAdmin = $db->getArrayLine( $db->SelectQuery("SELECT Denominazione FROM enti_gestiti WHERE CC = '".$c."'") );
...
//var_dump($banca);die;
            ...
            $cls_text = new cls_textParameters();
            $a_subtext = $db->getResults($db->SelectQuery($cls_text->getSubParametersQuery($c,39)));
            $cls_text->html_body = isset($a_text['Content']) ? $a_text['Content'] : null;
            $cls_text->html_replaced_body = $cls_text->html_body;
