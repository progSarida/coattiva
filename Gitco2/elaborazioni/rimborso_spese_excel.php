...
<script>
    ...
    switchMenuImg("F5");
    F5_button = function(){
        location.href="rimborso_spese_excel.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>";
    }
    ...
    function recallNewYear(el){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                //validateForm(document.getElementById("data_stampa"))
            },
    ...
    function ajaxCall() {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if(resp.error == 0)
                    showFileOnModal(resp.path,"Excel dettaglio rimborso spese Art. 17",resp.path.split('.').pop());
            },
            error:function(resp)
            {
                spinner.closeSpinner();
...
<form id="f_rimborso_spese" name="rimborso_spese_form" action="print_rimborso_spese_excel.php" method="post" >
    <input type=hidden name="c" value="<?php echo $c ?>" />
    <input type=hidden name="a" value="<?php echo $a ?>" />
    <input type=hidden id="data_stampa_rif" name="data_stampa_rif" value="" />

    <div class="row justify-content-md-center " style="margin: 2%;">
        <div class="col col-md-auto text_center">
            <span class="titolo font16 under_decor">Stampa rimborso spese (Excel di dettaglio)</span>
        </div>
    </div>
