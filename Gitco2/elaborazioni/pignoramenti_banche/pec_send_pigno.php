...
{
    $query = "SELECT PG.ID as PignoID,
            NA.ID, NA.Tipo_Notifica, PG.Partita_ID, PG.ID_Cronologico, PG.Anno_Cronologico, PG.CC, NA.SignedPdfFlag, PG.DocumentTypeId, PG.Elaboration_Id,
            DT.Description AS DocumentType, DT.PrefixName, DT.FolderName, U.PEC, E.Sending_Date AS Email_Sending_Date
            FROM notifica_atto as NA
            JOIN pignoramento_generale as PG on NA.Atto_Notificato_ID=PG.ID
            JOIN partita_tributi P ON P.ID=PG.Partita_ID
...
foreach ($a_notifiche_atti as $a_notifica_atto) {
    ...
    $crea_file_name=function($a_results,$suffix="Copia") use($c){
        if($a_results["Tipo_Notifica"]=="debitore")
            $type="debitore";
        else
            $type="banca";
        ...
        $path=$pignoId."/";
        $filename=$prefix."_".$cc."_".$anno."_".$id."_".$notifica_id."_".$suffix."_".$type.".pdf";
        $filename_Relata=$prefix."_".$cc."_".$anno."_".$id."_".$notifica_id."_"."Relata_".$type.".pdf";
        $filename_Relata_Signed=$prefix."_".$cc."_".$anno."_".$id."_".$notifica_id."_"."Relata_".$type."_signed".".pdf";
    ...
    if(!is_file($file_copia)){
        $msg = "INVIO INCOMPLETO! File pignoramento non trovato (NOTIFICA ".$a_results["ID"].")";
        $error = 1;
        if(!is_file($file)){
            $msg.= " - Relata pignoramento non trovata (NOTIFICA ".$a_results["ID"].")";
        }
        $checkFailedSending = 1;
        break;
    }
    if(!is_file($file)){
        $msg = "INVIO INCOMPLETO! Relata pignoramento non trovata (NOTIFICA ".$a_results["ID"].")";
        $error = 1;
        $checkFailedSending = 1;
        break;
    }
