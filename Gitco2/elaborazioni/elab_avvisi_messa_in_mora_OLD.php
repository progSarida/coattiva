...
//                        echo var_dump($ultimo_atto_valido->Atto);
                ...
                if($partita->Flag_Blocco_Maggiorazioni!="si" && $parametri->Maggiorazione_Ingiunzione!="no")
                {
                    if($importoInteressi>0){
                        if($partita->Tipo == "CDS")
                        {
                            $interessi = $cls_elab->calcola_interessi( $data_interessi, $data_calcolo, $importoInteressi );
                        }
                        else{

                            //$interessi_tributi = new interessi_tributi($c);
                            $query = "SELECT * FROM interessi_tributi WHERE CC = '".$c."' ORDER BY Data_Inizio ASC";
                            $array_classe = $cls_db->getResultsNull($cls_db->ExecuteQuery($query),"interessi_tributi");
                            $interessi_tributi = new stdClass();

                            for($i=0;$i<count($array_classe);$i++)
                            {
                                $interessi_tributi->ID[$i] = $array_classe[$i]['ID'];
                                $interessi_tributi->CC[$i] = $array_classe[$i]['CC'];
                                $interessi_tributi->Data_Inizio[$i] = $array_classe[$i]['Data_Inizio'];
                                $interessi_tributi->Data_Fine[$i] = $array_classe[$i]['Data_Fine'];
                                $interessi_tributi->Tasso_Interessi[$i] = $array_classe[$i]['Tasso_Interessi'];
                            }

                            $interessi_array = $cls_elab->calcola_interessi_tributi( $data_interessi, $data_calcolo, $importoInteressi, $interessi_tributi );
                            $interessi = $cls_elab->totale_interessi_tributi($interessi_array);
