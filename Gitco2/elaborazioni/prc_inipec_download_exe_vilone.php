...
function callInipecDownload($rs,$utils,$username,$password,$idRichiesta,$messageObject,$c,$a){
    $glob=new CkGlobal();
...
//        $_SESSION['Message'] = $messageObject->getMessagesString();
        header("location: prc_inipec.php?c=".$c."&a=".$a."&msg=Impossibile scaricare la richiesta, provare più tardi!&error=2");
        die;
    }
    ...
    if ($zip->open($pathFILE."/response_{$username}_{$idRichiesta}.zip", ZipArchive::CREATE)) {
        ...
        if(!$rs->ExecuteQuery($query)){
            $rs->Rollback();
            header("location: prc_inipec.php?c=".$c."&a=".$a."&msg=Errore, impossibile aggiornare esito sulla tabella ini_pec_request!&error=1");
        }
...
if($passwordExpiration!=null && strtotime(date('Y-m-d'))>strtotime($passwordExpiration)) {
    echo "<script>location.href = 'prc_inipec.php?c=".$c."&a=".$a."&msg=Password scaduta, per modificarla premere il lucchetto verde&error=2' </script>";
...
for($i=0; $i < $countRes; $i++){
    if($table_row[$i]["IdRichiesta"]!=null){
        $csv = callInipecDownload($cls_db,$cls_utils,$username,$password,$table_row[$i]["IdRichiesta"],$log,$c,$a);
        foreach($csv as $row){
            ...
            if (!empty($pec))
            {
                ...
                foreach($a_utenti as $a_utente){
                    if(!empty($a_utente['PEC']) && $a_utente['PEC']!=$pec){
                        ...
                        if (!$cls_db->ExecuteQuery($query)) {
                            $cls_db->Rollback();
                            header("location: prc_inipec.php?c=" . $c . "&a=" . $a . "&msg=Errore, impossibile aggiornare lo storico!&error=1");
                        }
                    }
                    ...
                    if (!$cls_db->ExecuteQuery($query)) {
                        $cls_db->Rollback();
                        header("location: prc_inipec.php?c=" . $c . "&a=" . $a . "&msg=Errore, impossibile salvare la PEC sulla tabella utente!&error=1");
                    }
                ...
                if(count($a_utenti)>0){
                    ...
                    if (!$cls_db->ExecuteQuery($query)) {
                        $cls_db->Rollback();
                        header("location: prc_inipec.php?c=" . $c . "&a=" . $a . "&msg=Errore, impossibile salvare la PEC sulla tabella ini_pec_request_pec!&error=1");
                    }
                    ...
                    if(!empty($pec)){
                        ...
                        $query.= "SET A.PrinterId=1, A.PrintTypeId=4, A.Modalita_Stampa='pec', A.Tipo_Ufficiale='procedimento' ";
...
header("location: prc_inipec.php?c=".$c."&a=".$a."&msg=Scaricamento dati completato correttamente!&error=0");