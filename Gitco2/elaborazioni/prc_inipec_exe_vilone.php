...
$query.= "AND P.CC IN (".$whereCC.") AND E.Elaboration_Status_Id<=2 ";
...
$query.= "GROUP BY P.CF_PI ORDER BY P.CF_PI ";
$a_inipec = $cls_db->getResults($cls_db->ExecuteQuery($query));
if(count($a_inipec)==0){
    echo "<script>location.href = 'prc_inipec.php?c=".$c."&a=".$a."&msg=Nessun utente valido trovato per la richiesta a IniPec. Contattare la assistenza per la verifica dei dati&error=2' </script>";
    die;
}
...
if ($zip->open($pathFILE . "/" . $nomeFile . '.zip', ZipArchive::CREATE)) {
    ...
    if(!$idRichiesta){
        echo "<script>location.href = 'prc_inipec.php?c=".$c."&a=".$a."&msg=Richiesta errata!&error=2' </script>";
        die;
    }
    ...
    foreach ($a_inipec as $key=>$a_cf){
        ...
        if($cls_db->DbSave($cls_utils->GetObjectQuery($a_saveRequest,"ini_pec_request_pec")) === false){
            ...
            echo "<script>location.href = 'prc_inipec.php?c=".$c."&a=".$a."&msg=Salvataggio CF ".$a_saveRequest["CodiceFiscale"]." fallito!&error=2' </script>";
            die;
        }
...
header("location: prc_inipec.php?c=".$c."&a=".$a."&msg=Richiesta dati completata correttamente!&error=0");