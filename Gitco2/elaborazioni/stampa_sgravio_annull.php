...
$sgravio_annull = $help->getVar("sgravio_annull");
...
if($sgravio_annull == "sgravio"){
    $formTypeId = 15;
    $query .= " AND SG.Tipo = 1 ";
    $filtriDescrizione = "Stampa discarichi: ";
}
else if($sgravio_annull=="annull") {
    $formTypeId = 38;
    $query .= " AND SG.Tipo = 2 ";
    $filtriDescrizione = "Stampa annullamenti: ";
}
...
if($ruolo_da != ""){
    $query .= " AND YEAR(PT.Data_Fornitura) >= '".$ruolo_da."' ";
    $filtriDescrizione .= " - Da anno fornitura ".$ruolo_da;
}
if($ruolo_a != ""){
    $query .= " AND YEAR(PT.Data_Fornitura) <= '".$ruolo_a."' ";
...
for($i=0; $i < $count; $i++){
    ...
    if($sgravio_annull == "annull"){
        $file = array();

        $cls_text->a_var = array(
            "{Document}" => strtoupper($result[$i]['TIPO_ATTO']),
            "{CronoYear}" => $result[$i]['ANNO_CRONOLOGICO'],
            "{CronoID}" => $result[$i]['ID_CRONOLOGICO'],
            "{Reference}" => $result[$i]['Comune_ID']."/".$result[$i]['Anno_Riferimento'],
            ...
            "{Notes}" => $result[$i]['Note_Blocco'],
            "{ActsNotified}" => $cls_st->tutti_gli_atti_notificati($result[$i]['Partita_ID']),
        ...
        for ($x = 1; $x <=2; $x++) {
            ...
            $pdf->setRecipientHeaderAnnul($a_recipientHeader_annull, $x, $sgravio_annull);
            ...
            if ($x == 1)
                $identificativo_file = "Annullamento_Copia_Utente_" . $result[$i]["Sgravio_ID"];
            else
                $identificativo_file = "Annullamento_Copia_Ente_" . $result[$i]["Sgravio_ID"];
            $nome_file = $identificativo_file . ".pdf";

            $pdf->Output($path . "/" . $nome_file, 'F');

            $file[] = $nome_file;
            $allPDFFile[] = $path . "/" . $nome_file;
        }

        if($help->getVar('printType') == "final") {
            $query = "UPDATE partita_tributi SET Printed_Annull = 'si', print_annull_date = '" . $data_stampa . "' WHERE ID = " . $result[$i]["Partita_ID"];
    ...
    else if($sgravio_annull == "sgravio"){
...
//        $pdf->ln(10);
        ...
        //$msg = "Selezionare sgravio o annullamento";
        //echo "<script>location.href = 'elenco_sgravi_annull.php?c=".$c."&a=".$a."&msg=".$msg."&error=1';</script>";
        echo json_encode([
            "error" => 1,
            "msg" => "Selezionare sgravio o annullamento"
        ]);
    ...
    $tipo = 0;
    if($sgravio_annull == "sgravio") $tipo = 1;
    else if($sgravio_annull == "annull") $tipo = 2;
...
if(count($allPDFFile) > 0) {
    ...
    if($help->getVar('printType') == "final") {
        ...
        if($sgravio_annull == "sgravio"){
            ...
            $pdfNameFile = "Stampa_Annullamenti_".$procedure_id."_" . $c . "_" . date("d-m-Y_H-i-s") . ".pdf";
        }
    }
    else{
        ...
        if($sgravio_annull == "sgravio")
            ...
            $pdfNameFile = "Stampa_Annullamenti_" . $c . "_" . date("d-m-Y_H-i-s") . ".pdf";
    }
