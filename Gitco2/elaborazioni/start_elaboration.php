...
if($auth>1)
    $whereRuolo = "WHERE CC='".$c."'";
else
    $whereRuolo = "";
$query = "SELECT * FROM ruolo ".$whereRuolo." ORDER by Data_Inserimento DESC";
...
?>
    ...
    <script>
        ...
        function pagina_menu (value)
        {
            ...
            if(value=='suc')
            {
                link = "start_elaboration.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>&tipo_atto="+atto_next;
            }
            else if(value=='prev')
            {
                link = "start_elaboration.php?c=<?php echo $c; ?>&a=<?php echo $a; ?>&tipo_atto="+atto_prev;
            }
