...
if (!((!empty($documentTypeId) && is_numeric($documentTypeId) && !empty($cod_catastale) && !empty($description) && !empty($data_elab) && !empty($data_calc_int))
    ...
    echo "<div class='alert alert-danger' role='alert'>
                È OBBLIGATORIO COMPILARE IL CAMPO TIPO ATTO	
                </div>";
...
$query_partita = " SELECT * FROM v_check_partite WHERE Elaboration_Id is null ";
...
if (count($partite) > 0) {
    ...
    flush();	ob_flush();
    echo "<script>endBar('".$c."','".$a."',".$last_el_id.",".$documentTypeId.",'".$cod_catastale."');
    </script>";
    flush();	ob_flush();		flush();	ob_flush();
    /** PHP PROGRESS BAR    END  */
