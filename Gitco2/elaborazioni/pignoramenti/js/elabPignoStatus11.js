        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function ( data, type, row, meta ) {
                        ...
                        filesubstringRelata = row.v_manage_acts_pignoramenti.PignoID + "/" +filesubstring + "_Relata";
                        filesubstringCopia = row.v_manage_acts_pignoramenti.PignoID + "/" +filesubstring+"_Copia";
                        ...
                        if(fileExists(relatasignedFilePath)){
                            returnHtml += '<i title="PDF Relata firmata '+crono+'" class="fa-solid fa-file-signature fa-lg" style="color:darkred; cursor:pointer;" aria-hidden="true" data-toggle="modal" data-target="#act-pdf" onclick="openPdf(\''+relatasignedFilePath+'\');"></i>';
                            ...
                            returnHtml += '<i title="PDF Relata '+crono+'" class="fas fa-file-pdf fa-lg" style=" color:darkred; cursor:pointer;" aria-hidden="true" data-toggle="modal" data-target="#act-pdf" onclick="openPdf(\''+relatafilePath+'\');"></i>';
                        }
    ...
    function printButton(el_list_Id) {
        ...
        $.ajax({
            ...
            success: function(response) {
                var response = JSON.parse(response);
                if (response.esito == "OK") {
                    location.href = web_root+"/elaborazioni/pignoramenti/elab_printing_pignoramento.php?c="+c+"&a="+a+"&el_list_id="+el_list_Id;
                } else {
                    console.log(response);
                    swal({
                        title: "ERROR!",
    ...
    function flowCreation(el_list_Id) {
        if (authFlag == 0) {
            ...
            location.href = web_root+"/elaborazioni/pignoramenti/elab_pigno_flows.php?c="+c+"&a="+a+"&el_list_id="+el_list_Id;
        }
    }
    ...
    function sendPec(el_list_Id) {
        if (authFlag == 0) {
            ...
            location.href = web_root+"/elaborazioni/pignoramenti/pec_send_pigno.php?c="+c+"&a="+a+"&Elaboration_Id="+elab_id+"&Elaboration_List_Id="+el_list_Id;
        }
    }
    ...
    function checkPec(el_list_Id) {
        if (authFlag == 0) {
            ...
            location.href = web_root+"/elaborazioni/pignoramenti/pec_download_receipts_pigno.php?c="+c+"&a="+a+"&Elaboration_Id="+elab_id+"&Elaboration_List_Id="+el_list_Id;
        }
    }
    ...
    function closeFlow(el_list_Id) {
        ...
        $.ajax({
            url: web_root+"/elaborazioni/pignoramenti/elab_closing_flow_pignoramento.php?c="+c+"&a="+a,
            type: 'POST',
            data: {
                'el_list_Id': el_list_Id,
            },
