...
if (strtolower($_SESSION['username']) == "mirkop" || strtolower($_SESSION['username']) == "michele" || strtolower($_SESSION['username']) == "emanuela" || strtolower($_SESSION['username']) == "robertop" || strtolower($_SESSION['username']) == "fabrizio")
    $authFlag = 1;
else
...
$act_flow = FLUSSI_WEB;
$actionSignedFiles = ELAB_PIGNORAMENTI_WEB."/upload_signed_files.php";
...
switch($a_elab['Elaboration_Status_Id']){
    ...
    <script>
        ...
        F3_button = function() {
            if (submit_buttons('Elabora')) {
                location.href ='elab_acts_pigno.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$a_elab['Id'];?>';
            }
        }
    ...
    <script src="<?= ELAB_PIGNORAMENTI_JS ?>/elabPignoStatus1.js"></script>

<?php
        break;
    case ElaborationStatus::ACQUISIZIONE_ACI:
        
        ?>

    <script>
        function ScaricaVisureACI()
        {
            location.href='elaborazione_visura_pignoramento.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$a_elab['Id'];?>&cod_cat=<?=$a_elab['CC'];?>&tipo_atto=<?=$a_elab['Document_Type_Id'];?>';
        }
        switchMenuImg("F3");
        F3_button = function() {
           if (submit_buttons('Elabora')) {

                location.href = 'elab_pignoramenti.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$a_elab['Id'];?>';
            }
        }
    ...
    <script src="<?= ELAB_PIGNORAMENTI_JS ?>/elabPignoStatus8.js"></script>

<?php
        
        break;

        case ElaborationStatus::SELEZIONE_VEICOLI:
            
 ?>

    <script>
        var CheckTribunali = 0;
        var CheckIVG=0;
        var documentTypeId = <?=$a_elab["DocumentTypeId"]?>;
        switchMenuImg("F3");
        F3_button = function() {
            if ((documentTypeId==6) && ((CheckTribunali+CheckIVG)>0))
            {
                alert("Mancano "+CheckTribunali+" Tribunali  e "+CheckIVG+" Istituti di Vendita Giudiziaria a degli elementi!");
            }
            else
             if (submit_buttons('Elabora')) {

                location.href = 'elab_pignoramenti_tariffe.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$a_elab['Id'];?>';
                alert("Fase successiva!");
            }
        }
    ...
    <script src="<?= ELAB_PIGNORAMENTI_JS ?>/elabPignoStatus9.js"></script>
    <?php
            break;
        ...
        <script>
            ...
            F3_button = function() {
                if(callPec==0){
                    if (submit_buttons('Elabora')) {
                        location.href ='elab_crono_assignment_notifica_atto.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$a_elab['Id'];?>&cod_cat=<?=$a_elab['CC'];?>&tipo_atto=<?=$a_elab['Document_Type_Id'];?>';
                    }
                }
                else
        ...
        <script src="<?= ELAB_PIGNORAMENTI_JS ?>/elabPignoStatus10.js"></script>
        <?php
                break;
                                    ...
                                    if($a_elab_list['PecReceiptsFlag']==1){
                                        ...
                                        if($a_count['MissingReceiptNumber']==0){
                                            ...
                                            <script>
                                                ...
                                                location.href ="<?= ELAB_PIGNORAMENTI_WEB ?>/mgmt_pignoramenti.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$a_elab['Id'];?>";
                                            </script>
                                            <?php
                                        }
                                        else{
            ...
            <script src="<?= ELAB_PIGNORAMENTI_JS ?>/elabPignoStatus11.js"></script>
