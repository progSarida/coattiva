...
$estrazione_elaborazione = $cls_help->getvar('estrazione_elaborazione');
if(is_null($estrazione_elaborazione)) $estrazione_elaborazione ='no';
...
<script>
     var estrazione_elaborazione = '<?= $estrazione_elaborazione?>';
        
    function link(c,a,el)
    {
        if (estrazione_elaborazione=='si')
            return  "<?= ELAB_PIGNORAMENTI_WEB ?>/mgmt_estrazioni.php?c="+c+"&a="+a+"&el="+el;
        return "<?= ELAB_PIGNORAMENTI_WEB ?>/mgmt_pignoramenti.php?c="+c+"&a="+a+"&el="+el;
    }
    function startBar(){
...
$query_partita = " SELECT * FROM v_check_pignoramenti WHERE Elaboration_Id is null ";
...
if (count($partite) > 0) {
    ...
    try {
        ...
        $elab_status_id = $estrazione_elaborazione == 'si' ? 9999 : 1;
        $documentTypeId = $estrazione_elaborazione == 'si' ? 22 : $documentTypeId; //per far i controlli della classe cls_elaborationPignoramento.
        
        $a_dbParams_pre_elab = array(
            'table' => 'elaborations',
            'fields'=> array(
                array(  'name' => 'Elaboration_Status_Id',      'type' => 'int', 'value' => $elab_status_id) ,
        ...
        foreach ($partite as $partita) {
            ...
            if ($estrazione_elaborazione=='si')
            {
                //creo nella tabella pvt il riferimento alla partita e alla elaborazione attuale
                if ($a_getPositionStatus['flag_elaboration']==1)
                {
                    $ins_pvt_estrazione = array(
                        'table' => 'estrazione_pvt',
                        'fields'=> array(
                            array(  'name' => 'Partita_ID',      'type' => 'int', 'value' => $partita['Partita_ID']) ,
                            array(  'name' => 'Elaboration_Id',                  'type' => 'int','value' => $last_el_id),
                            array(  'name' => 'Position_Status_Id',                  'type' => 'int','value' => $a_getPositionStatus['position_status']),
                       
                        )
                    );
            
                    $cls_db->DbInsert($ins_pvt_estrazione);
                }
        ...
        if ($estrazione_elaborazione=='si')
        {
            ...
            //aggiorno il document_type di modo che prenda la giusta descrizione
            $q = "update elaborations set Document_Type_Id = 43 where Id =$last_el_id ";
            $cls_db->ExecuteQuery($q);
        }
    ...
    if($estrazione_elaborazione == 'si'){
        $atto_ = "Estrazione posizioni pignorabili";
        $stato_ = "Estrazione per comuni";
    } else {
        $atto_ = "Preavvisi fermi amministrativi";
            $stato_ = "Pre-Elaborato";
    }

    $storico->insRow('E', "Elaborazione '".$description."': ".$atto_." ".$ente['Denominazione']."[".$c."]. Stato '".$stato_."'");
