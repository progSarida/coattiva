    ...
    switch ($elab_list['PrinterId']) {
        case 1:
            $a_dbParams_elab_list = array(
                ...
                'fields'=> array(
                    array(  'name' => 'Elaboration_Status_Id',  'type' => 'int', 'value' => 7),
                    array(  'name' => 'FlowDate',   'type' => 'date', 'value' => date('Y-m-d')),
                )
            );
            $cls_db->DbSave($a_dbParams_elab_list);

            $contList = $cls_db->getNumberRow($cls_db->ExecuteQuery("SELECT Id FROM elaboration_lists WHERE Elaboration_Id=".$elab_list['Elaboration_Id']." AND Elaboration_Status_Id = 7"));
            ...
            if((int)$elabs['ListNumber']==(int)$contList)
                $a_dbParams_elab['fields'][] = array(  'name' => 'Elaboration_Status_Id', 'type' => 'int', 'value' => 7);
...
//            $cls_mail->SMTPDebug = 2;
            ...
            $file = ATTI . "/" . $elab_list["CityId"] . "/" .$elab_list["FolderName"] . "/FLUSSI/" . $elab_list['FileName'];
...
//        $flag = true;
    ...
    $storico->insRow('E', "Inviato flusso numero ".$elab_list['FlowNumber']." elemento ".$cls_help->getVar("el_list_Id")." elaborazione '".$elab['Description']."' (".$tipo_doc['Description'].") ente ".$nome_ente."[".$elab['CC']."]");
}
catch(Exception $e)
{
   /*  if($error == 0) $error = 1;
