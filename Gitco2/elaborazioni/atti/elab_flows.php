...
<script>
    ...
    function endBar(value) {
        ...
        $("#barlabel").text(value);
        sleep(1000);
        window.opener.location.href ="<?= ELAB_ATTI_WEB ?>/mgmt_elaboration.php?c=<?=$c;?>&a=<?=$a;?>&el=<?=$filter['last_el_id'];?>";
...
if(count($a_results) == 0)
    echo "<script>noResultsBar();</script>";
      ...
      //  if($filter['printStatus']=="toPrint"){
            ...
            $storico->insRow('E', "Creato flusso numero ".$cls_flow->flowNumber." elemento ".$cls_help->getVar('el_list_id')." elaborazione '".$elab['Description']."': ".$atto_." ".$nome_ente."[".$elab['CC']."]");
