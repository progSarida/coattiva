...
$Is_Expired = $cls_help->getVar('Is_Expired');
...
$documentTypeId = $cls_help->getVar('tipo_atto');
...
if (!((!empty($documentTypeId) && is_numeric($documentTypeId) && !empty($cod_catastale) && !empty($description) && !empty($data_elab) && !empty($data_calc_int))
    ...
    echo "<div class='alert alert-danger' style='text-align: center;' role='alert'>
                È OBBLIGATORIO COMPILARE IL CAMPO DESCRIZIONE
                </div>";
...
$query_partita = " SELECT * FROM v_check_partite WHERE Elaboration_Id is null AND archived IS NULL ";

if(!empty($ruolo))
    $query_partita .= " AND Ruolo_ID = ".$ruolo;
if($Is_Expired==1)
    $query_partita .= " AND  Is_Expired = 0";
...
if (count($partite) > 0) {
    ...
    try {
        ...
        $a_dbParams_pre_elab = array(
            ...
            'fields'=> array(
                ...
                array(  'name' => 'DefaultPecTipoUfficiale',            'type' => 'string', 'value' => $cls_help->getVar("DefaultTipoUfficialePEC")),
                array(  'name' => 'DefaultPecTipoStampa',               'type' => 'int', 'value' => $cls_help->getVar("DefaultTipoStampaPEC")),
                array(  'name' => 'DefaultRaccomandataTipoUfficiale',   'type' => 'string', 'value' => $cls_help->getVar("DefaultTipoUfficialeRaccomandata")),
                array(  'name' => 'DefaultRaccomandataTipoStampa',      'type' => 'int', 'value' => $cls_help->getVar("DefaultTipoStampaRaccomandata"))
        ...
        $cls_elaboration = new cls_elaboration($a_params);
        $i = 0;
    ...
    switch ($documentTypeId){
        case 2:
            ...
            break;
        case 3:
            $atto = "Solleciti di pagamento";
            break;
        case 4:
            $atto = "Avvisi d'intimazione";
            break;
        case 11:
            $atto = "Solleciti pre ingiunzione";
            break;
        case 12:
            $atto = "Avvisi di messa in mora";
            break;
    }

    $storico->insRow('E', "Elaborazione '".$description."': ".$atto." ".$nome_ente."[".$c."]. Stato 'Pre-Elaborato'");
