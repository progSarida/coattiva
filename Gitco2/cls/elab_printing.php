...
$queryInteressiTributi = "SELECT DATE_FORMAT(Data_Inizio, '%d/%m/%Y') as Data_Inizio, DATE_FORMAT(Data_Fine, '%d/%m/%Y') as Data_Fine, CONCAT(Tasso_Interessi,' &#8364;') as Tasso_Interessi FROM interessi_tributi WHERE CC = '****' ORDER BY Data_Inizio ASC;";
...
for ($i = 0; $i < count($a_results); $i++) {
    ...
    $pdf->setManagerHeaderLAB($cls_ente->a_header);
