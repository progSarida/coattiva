...
// 				break;
  ...
  public function getDataPartita_1($p,$c)
  {
    $query = "SELECT ID FROM partita_tributi WHERE Utente_ID = '".$p."' AND CC = '".$c."'";
  	$results = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));// mysql_array($query);
      ...
      for( $k=0; $k<count($pignoramento_id); $k++)
        ...
        $query = "SELECT * FROM pignoramento_generale WHERE ID = ".$pignoramento_id[$k]['ID']." AND CC = '".$c."'";
  			$partita[$i]["Pignoramento"][$k] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"pignoramento_generale");//new pignoramento( $pignoramento_id[$k]['ID'] , $c );
        ...
        $where_notifica_debitore = "CC = '".$c."' AND Atto_Notificato_ID = '".$pignoramento_id[$k]['ID']."' AND Tipo_Atto_Notificato = 'pignoramento' AND Tipo_Notifica = 'debitore'";
    		$query = "SELECT ID FROM notifica_atto WHERE ".$where_notifica_debitore;//$notifica_debitore_id = select_mysql_array("ID", "notifica_atto" , $where_notifica_debitore);
        $notifica_debitore_id = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));
           ...
           $query = "SELECT * FROM notifica_atto WHERE ID = '".$notifica_debitore_id[$m]['ID']."' AND CC = '".$c."'";
				//echo $query;
    			 $partita[$i]["Pignoramento"][$k]["Notifiche_Debitore"][$m] =$this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"notifica_atto");//new notifica_atto( $notifica_debitore_id[$m]['ID'] , $c );
    		}
...
//print_r($atto_id);
      for( $x=0; $x<count($atto_id); $x++)
      {
        ...
        $partita[$i]["Atto"][$x] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"atto");//new atto( $atto_id[$i]['ID'] , $c );
      //  echo "count ".$x."<br>";
        ...
        $pagamento_id = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));//select_mysql_array("ID", "pagamento","Atto_ID = '".$this->ID."' AND Partita_ID = '".$this->Partita_ID."' AND Tipo_Atto NOT LIKE 'Pignoramento%'", "Rata");

    		for( $z=0; $z<count($pagamento_id); $z++)
    		{
        	 $query = "SELECT * FROM pagamento WHERE ID = '".$pagamento_id[$z]['ID']."' AND CC = '".$c."'";
			     $partita[$i]["Atto"][$x]["Pagamento"][$z] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"pagamento");//new pagamento( $pagamento_id[$z]['ID'] , $c );
    		}
      }
  	}
  ...
  {
    $query = "SELECT * FROM partita_tributi WHERE ID = '".$p."' AND CC = '".$c."'";
...
//echo "<h1>n1 --> ".$duenovanta["Record_N1"]."</h1>";
  ...
  public function GetDataPigno($ID,$c)
  {
	  //echo "<h1>aaa".$ID."</h1>";
    $ID = $ID==""?'null':$ID;

    $query = "SELECT * FROM pignoramento_generale WHERE ID = ".$ID." AND CC = '".$c."'";
    ...
    $result["Notifiche_Debitore"] = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));

    if(isset($result["Notifiche_Debitore"][0]))
    {
      //$query = "SELECT * FROM notifica_atto WHERE ID = '".$notifica_debitore_id[0]['ID']."' AND CC = '".$c."'";
			$result["Notifica_Debitore"] = $result["Notifiche_Debitore"][0];// $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
    }
    ...
    $result["Notifica_Sollecito"] = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));
    /*for( $i=0; $i<count($result["Notifica_Sollecito"]); $i++ )
    {
      $query = "SELECT * FROM notifica_atto WHERE ID = '".$notifica_sollecito_id[$i]['ID']."' AND CC = '".$c."'";
      $result["Notifica_Sollecito"][$i] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
    }*/
//var_dump($result);
    //if($result["Tipo"]!=null)
    //{
    //echo "<h1>aaa".$result["Tipo"]."</h1>";
      switch($result["Tipo"])
  		{
  			case 'terzi':
          ...
          $terzi_id = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));
//echo "<h1>aaa".count($terzi_id)."</h1>";
  				for( $i=0; $i<count($terzi_id); $i++ )
  				{
					$query = "SELECT * FROM pignoramento_presso_terzi WHERE ID = '".$terzi_id[$i]['ID']."' AND CC = '".$c."'";
							$result["Presso_Terzi"][$i] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"pignoramento_presso_terzi"); //new pignoramento_presso_terzi( $terzi_id[$i]['ID'] , $c );
					...
					$query = "SELECT ID FROM notifica_atto WHERE ".$where_notifica_terzo." ORDER BY ID ASC";
					  ...
					  $query = "SELECT * FROM notifica_atto WHERE ID = '".$notifica_terzo_id[$y]['ID']."' AND CC = '".$c."'";
						...
						//print_r($result["Presso_Terzi"][$i]["Notifiche_Terzo"]);
          ...
          //echo "<h1>".$query." - ".count($preav_fermo_id)."</h1>";
            ...
            $query = "SELECT * FROM pignoramento_veicolo WHERE ID = '".$preav_fermo_id[$i]['ID']."' AND CC = '".$c."'";
  			...
  			case 'fermo':
				  ...
				  //echo "<h1>".$query."</h1>";
				 $result["Fermo"] = $this->cls_db->getResultsNull($this->cls_db->ExecuteQuery($query),"pignoramento_veicolo");//select_mysql_array("ID", "pignoramento_veicolo" , "Pignoramento_ID = '".$progr."' AND CC = '".$c."'");

  				/*for( $i=0; $i<count($fermo_id); $i++ )
  				{
  					//$this->Fermo[$i] = new pignoramento_veicolo( $fermo_id[$i]['ID'] , $c );
            $query = "SELECT * FROM pignoramento_veicolo WHERE ID = '".$fermo_id[$i]['ID']."' AND CC = '".$c."'";
  					$result["Fermo"][$i] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"pignoramento_veicolo");
  				}*/
			  ...
			  if(count($notifica_istituto_id)==0)
			  {
				...
				for( $i=0; $i<count($notifica_istituto_id); $i++ )
				{
					...
					$result["Notifica_Istituto"][$i] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"notifica_atto");//new notifica_atto( $notifica_istituto_id[$i]['ID'] , $c );
					...
					$array_email = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));//select_mysql_array("ID", "email_inviate", "CC='".$c."' AND Table_Collegata ='notifica_atto' AND ID_Collegato = $this->ID");
					if(count($array_email)>=1)
					{
						$query = "SELECT * FROM email_inviate WHERE ID = '".$result["Notifica_Istituto"][$i]["ID"]."'";
						//include_once CLASSI . "/classe_email.php";
						$result["Notifica_Istituto"][$i]["Email_Object"] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"email_inviate");//new email_inviate($array_email[0]['ID']);
					}
      ...
      for( $i=0; $i<count($pagamento_id); $i++)
      {
        ...
        $result["Pagamento"][$i] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"pagamento");//new pagamento( $pagamento_id[$i]['ID'] , $c );
      }
    //}
  ...
  public function righe_indirizzo($utente)
  {
    ...
    if($utente["Recapito"]["Toponimo"]!=null)
      $indirizzo = $utente["Recapito"];
    else if($utente["Domicilio"]["Toponimo"]!=null)
  ...
  public function GetNavigation($p,$c,$a)
  {
    ...
    if ($p==0)
		...
		{
			$query = "SELECT ID FROM partita_tributi	WHERE ( (ID>'$p') AND (CC='$c') AND Anno_Riferimento = '$a') ORDER BY ID ASC LIMIT 1";
			/*$result = safe_query($query);
			$array_result = mysql_fetch_array($result);
			$this->next = $array_result['ID'];*/
      $nav["next"] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"partita_tributi")["ID"];

			$query = "SELECT ID FROM partita_tributi	WHERE ( (ID<'$p') AND (CC='$c') AND Anno_Riferimento = '$a') ORDER BY ID DESC LIMIT 1";
			/*$result = safe_query($query);
			$array_result = mysql_fetch_array($result);
  ...
  public function totali_spese($data)
	{
		...
		$totale["Totali_Array"][1] = $this->cls_math->conv_num(number_format($totale['totale_1'],2));
		$totale["Totali_Array"][2] = $this->cls_math->conv_num(number_format($totale['totale_1']+$totale['totale_2'],2));
		$totale["Totali_Array"][3] = $this->cls_math->conv_num(number_format($totale['totale_1']+$totale['totale_2']+$totale['totale_3'],2));
