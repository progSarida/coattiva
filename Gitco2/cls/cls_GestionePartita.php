...
//echo "<h1>CountPag: ".count($pagamento_id)."</h1>";
    ...
    public function pagamenti_precedenti($ID, $Partita_ID, $data_check_pagamento=null)
    {
        if(empty($data_check_pagamento))
            $query = "SELECT SUM(Importo) AS TOTALE_PAGAMENTI 
            FROM pagamento WHERE Atto_ID < " . $ID . " AND Partita_ID = " . $Partita_ID . " 
            AND Tipo_Atto NOT LIKE 'Pignoramento%' AND Tipo_Atto NOT LIKE 'Precedenti%' GROUP BY Partita_ID";
        else
            $query = "SELECT SUM(Importo) AS TOTALE_PAGAMENTI 
            FROM pagamento
            WHERE Partita_ID = " . $Partita_ID . " AND Data_Pagamento < '".$data_check_pagamento."'
            AND Tipo_Atto NOT LIKE 'Precedenti%' GROUP BY Partita_ID";
