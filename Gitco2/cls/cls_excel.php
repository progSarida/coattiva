    ...
    public function getArrayFromFile($file){
        ...
        foreach ($obj_excel->getWorksheetIterator() as $worksheet) {
            ...
            for ($row = 1; $row <= $worksheet->getHighestRow(); ++ $row) {
                ...
                $checkValue = $worksheet->getCellByColumnAndRow(0, $row)->getValue();//getCalculatedValue()
                if(trim($checkValue)!=""){
                    ...
                    for ($col = 0; $col < $lastColumn; ++ $col) {
                        ...
                        if($row==1)	{
                            $a_header[$col] = $cell->getValue();//getCalculatedValue()
                        }
                        else if($a_header[$col]!="") {

                            $valore = $cell->getValue();//getCalculatedValue()
    ...
    public function addRow($a_value, $row){
        for($i=0;$i<count($a_value);$i++){
            if(substr($a_value[$i],0,1=="0") && strlen($a_value[$i])==11){
                ...
                $this->sheet->setCellValueExplicit($colString.$row, $a_value[$i], PHPExcel_Cell_DataType::TYPE_STRING);
            }
            else
