...
class cls_GP
{
  ...
  public function getDataScorporo($p,$c,$a)
  {
    ...
    for( $i=0; $i<count($tributo_id); $i++)
    {
        ...
        $array_tributo = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));

        $query = "SELECT Tipo_Codice FROM codice_tributo WHERE Codice_Tributo = '".$array_tributo['Codice_Tributo']."'";
        $array_codTributo = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
  ...
  //  echo "<h1>CountP: ".count($pignoramento_id)."</h1>";
    ...
    for( $y=0; $y<count($pignoramento_id); $y++)
    {
      ...
      $val["Pignoramento"][$y] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query)); //new pignoramento( $pignoramento_id[$y]['ID'] , $c );
...
//echo "<h1>CountPag: ".count($pagamento_id)."</h1>";
      for( $x=0; $x<count($pagamento_id); $x++)
      {
        ...
        $val["Pignoramento"][$y]["Pagamento"][$x] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));

      }

      $query = "SELECT * FROM pignoramento_spese WHERE Pignoramento_ID = ".$pignoramento_id[$y]["ID"]." AND CC = '".$c."'";
      $data["Pignoramento"][$y]["Spese_Pignoramento"] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
    ...
    for( $i=0; $i<count($atto_id); $i++)
    {
      ...
      $val["Atto"][$i] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
  ...
  //  echo "<h1>Num Pag : ".count($pagamento_id)."</h1>";
      ...
      {
        $query = "SELECT * FROM pagamento WHERE ID = '".$pagamento_id[$x]['ID']."' AND CC = '".$c."'";
        $val["Atto"][$i]["Pagamento"][$x] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
  ...
  public function getTotalAmountDuePigno($pigno)
  {
      ...
      if($pigno["Rate_Previste"]>0){
          ...
          if($pagamento_pigno == $this->cls_math->conv_num($result["Totali_Array"][3]))
          {
              $a_amount['tot'] = $this->cls_math->conv_num($result["Totali_Array"][3]);
              $a_amount['spese_accessorie'] = $result["Parziali_Spese_Accessorie"][3];
          }
          else if($pagamento_pigno == $this->cls_math->conv_num($result["Totali_Array"][2]))
          {
              $a_amount['tot'] = $this->cls_math->conv_num($result["Totali_Array"][2]);
              $a_amount['spese_accessorie'] = $result["Parziali_Spese_Accessorie"][2];
          }
          else
          {
              $a_amount['tot'] = $this->cls_math->conv_num($result["Totali_Array"][1]);
  ...
  public function gestione_totali($data)
	{
		...
		$dataArray["Totali_Array"][1] = $this->cls_math->conv_num(number_format($totale_1,2));
	  $dataArray["Totali_Array"][2] = $this->cls_math->conv_num(number_format($totale_2,2));
		$dataArray["Totali_Array"][3] = $this->cls_math->conv_num(number_format($totale_3,2));
  ...
  public function pagamenti_completi($ID,$Partita_ID){
      ...
      return $results['TOTALE_PAGAMENTI'];
  }
  ...
  public function getDataPartita( $progr = 0 , $c , $a = null )
    ...
    //print_r($val);
		...
		$val['CC'] = isset($val['CC'])?$val['CC']:"";
		$val['Comune_ID'] = isset($val['Comune_ID'])?$val['Comune_ID']:"";
		$val['Ruolo_ID'] = isset($val['Ruolo_ID'])?$val['Ruolo_ID']:"";
		$val['Anno_Riferimento'] = isset($val['Anno_Riferimento'])?$val['Anno_Riferimento']:"";
		$val['Tipo'] = isset($val['Tipo'])?$val['Tipo']:"";
		$val['Sottotipo'] = isset($val['Sottotipo'])?$val['Sottotipo']:"";
		$val['Utente_ID'] = isset($val['Utente_ID'])?$val['Utente_ID']:"";

		$val['File_1'] = isset($val['File_1'])?$val['File_1']:"";
		$val['File_2'] = isset($val['File_2'])?$val['File_2']:"";

		//$this->Utente = new utente($val['Utente_ID'], $c);

		$val['Flag_Blocco_Coazione'] = isset($val['Flag_Blocco_Coazione'])?$val['Flag_Blocco_Coazione']:"";
		$val['Flag_Blocco_Maggiorazioni'] = isset($val['Flag_Blocco_Maggiorazioni'])?$val['Flag_Blocco_Maggiorazioni']:"";
		$val['Flag_Blocco_Diritto_Riscossione'] = isset($val['Flag_Blocco_Diritto_Riscossione'])?$val['Flag_Blocco_Diritto_Riscossione']:"";

		$val['Motivo_Blocco'] = isset($val['Motivo_Blocco'])?$val['Motivo_Blocco']:"";
		$val['Note_Blocco'] = isset($val['Note_Blocco'])?$val['Note_Blocco']:"";
		$val['Cancellazione'] = isset($val['Cancellazione'])?$val['Cancellazione']:"";
		$val['Coo_Tipo'] = isset($val['Coo_Tipo'])?$val['Coo_Tipo']:"";
    $val['Split_Parameters_ID'] = isset($val['Split_Parameters_ID'])?$val['Split_Parameters_ID']:"";
		...
		//var_dump($val['ID']);
    $query = "SELECT TR.*, CT.Descrizione as Tipo_Tributo, CT.Tipo_Codice, CT.Testo_Codice, CT.Codice_Scorporo FROM tributo AS TR JOIN codice_tributo as CT ON CT.Codice_Tributo=TR.Codice_Tributo WHERE TR.Partita_ID=".$val['ID'];
    if($val['ID']=="") $a_tributi = null;
    else $a_tributi = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));

     //var_dump($a_tributi);
  //  $query = "SELECT ID FROM tributo WHERE Partita_ID = '".$val['ID']."' ORDER BY Codice_Tributo";
  	//$tributo_id = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));
    ...
    $query = "SELECT * FROM pagamento WHERE Partita_ID = '".$val['ID']."' AND Tipo_Atto != 'Precedenti'";
    ...
    $query = "SELECT ID FROM ricorso_generale WHERE Partita_ID = '".$val['ID']."'";
  	$ricorso_id = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));
    ...
    $query = "SELECT ID FROM pignoramento_generale WHERE Partita_ID = '".$val['ID']."'";
  	$pignoramento_id = $this->cls_db->getResults($this->cls_db->ExecuteQuery($query));
        ...
        $val["Data_Inizio_Interessi"] = "";
        //echo "<h1>count tributi --> ".count($a_tributi)."</h1>";

        /*for($x=0; $x<count($pagamento_id);$x++)
        {
          $query = "SELECT * FROM pagamento WHERE ID = ".$pagamento_id[$x]["ID"]." AND CC = '".$c."'";
          $val["Pagamento"][$x] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
        }*/
    ...
    if($a_tributi != null && $a_tributi != array()) {
        for ($i = 0; $i < count($a_tributi); $i++) {
            ...
            /*$query = "SELECT * FROM tributo WHERE ID = '".$tributo_id[$i]['ID']."' AND CC = '".$c."'";
            $val["Tributo"][$i] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));
              //$this->Tributo[$i] = new tributo($tributo_id[$i]['ID'], $c);

              $val["Tributo"][$i]["Tipo_Tributo"] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery("SELECT Descrizione FROM codice_tributo WHERE Codice_Tributo = '".$val['Tributo'][$i]['Codice_Tributo']."'"));
                  $val["Tributo"][$i]["Tipo_Codice"] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery("SELECT Tipo_Codice FROM codice_tributo WHERE Codice_Tributo = '".$val['Tributo'][$i]['Codice_Tributo']."'"));
              $val["Tributo"][$i]["Testo_Codice"] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery("SELECT Testo_Codice FROM codice_tributo WHERE Codice_Tributo = '".$val['Tributo'][$i]['Codice_Tributo']."'"));
              $val["Tributo"][$i]["Codice_Scorporo"] = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery("SELECT Codice_Scorporo FROM codice_tributo WHERE Codice_Tributo = '".$val['Tributo'][$i]['Codice_Tributo']."'"));*/
            ...
            if ($val["Tributo"][$i]["Tipo_Codice"] == "IMPORTO" && $this->cls_date->Get_DateNewFormat($val["Tributo"][$i]["Data_Decorrenza_Interessi"], "DB") != "") {
                ...
                $data_decorrenza = $val["Tributo"][$i]["Data_Decorrenza_Interessi"];
            } else $val["Data_Inizio_Interessi"] = null;
        ...
        $testoSemestri = "";
        $countGiri = 0;
        $dataInizioInteressi = isset($val["Data_Inizio_Interessi"])?$val["Data_Inizio_Interessi"]:"0000-00-00";
        $val["Somma_Spese_Notifica"] = 0;
		for( $i=0; $i<count($atto_id); $i++)
		{
      $query = "SELECT * FROM atto WHERE ID = ".$atto_id[$i]['ID']." AND CC = '".$c."'";
			$val["Atto"][$i] = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"atto"); //new atto( $atto_id[$i]['ID'] , $c );


      $data_preav = explode("**", $val["Atto"][$i]["Date_Stampe_Preavvisi_Ing"] );
