...
class cls_pdf extends TCPDF
{
    ...
    public function setManagerHeader($a_header){
...
//        $this->Cell (116, 5, $a_header['placeDate'] ,0, 1, "L");
    ...
    public function setRecipientHeaderAnnul($a_header,$priority){
...
//        $this->Cell (116, 5, $a_header['placeDate'] ,0, 1, "L");
        ...
        switch($priority)
        {
            case 1:
                ...
                //die;
                $this->Ln(10);
                $this->SetMargins(123.0, $this->GetY()+10);	$this->Ln(0);
                $this->Cell (90, 5, $a_header["ente"]["Info_Denominazione"], 0, 1, "L");//Nome Ente
                /*$this->SetMargins(7.0, 10.0,7.0);*/	$this->Ln(0);

                $this->Cell(90, 5, $a_header['ente']["Info_Via"].", ".$a_header['ente']["Info_Civico"]."/".$a_header['ente']["Info_Interno"], 0, 1, "L");
                $this->Cell(90, 5, $a_header['ente']["Info_Cap"]." ".$a_header['ente']["Info_Comune"]." ".$a_header['ente']["Info_Provincia"], 0, 1, "L");
                break;
            ...
            case 2:
                ...
                //DESTINATARIO 1
                $this->SetMargins(123.0, $this->GetY());	$this->Ln(15);
                $this->Cell (90, 5, $a_header['denomination'][0], 0, 1, "L");//Nome Destinatario
                $this->SetMargins(7.0, 10.0,7.0);	$this->Ln(0);
                ...
                //DESTINATARIO 2
                $this->SetMargins(123.0, $this->GetY()+1);	$this->ln(0);
                for($x_utente = 1;$x_utente<count($a_header['denomination']);$x_utente++) {
                    $this->Cell(90, 5, $a_header['denomination'][$x_utente], 0, 1, "L");//Righe aggiuntive
                    //echo $a_header['denomination'][$x_utente];
                }
...
//        $this->Cell (116, 5, $a_header['placeDate'] ,0, 1, "L");
    ...
    public function Footer() {
        ...
        $this->SetY(-10);
        $this->Cell(0, 5, "Pag. ". ($this->getPage() + 1) ." - ".date("d/m/Y H\hi:s"), 0, false, 'R', 0, '', 0, false, 'T', 'M');

    }

    public function setHeaderPage($fontSize=10){
        ...
        for($i=0;$i<count($this->a_totalsHeader);$i++){
            for($y=0;$y<count($this->a_totalsHeader[$i]);$y++){
                if($this->a_totalsHeader[$i][$y]=="{".$cont."}"){
                    $a_totals[$i][$y] = number_format($totals[$cont],2,",",".");
                    $cont++;
                }
    ...
    public function setRowPage($a_value, $fontSize=8, $fontHeader=10, $limit=40 ){
    ...
    public function setMainPage($a_filters, $recap){
