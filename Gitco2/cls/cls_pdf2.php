...
//        $this->Cell (116, 5, $a_header['placeDate'] ,0, 1, "L");
        ...
        $this->Cell (90, 5, $a_header['placeDate'], 0, 1, "L");//Data e luogo

        $this->SetXY( 7 , 57 );
        $this->Cell ( 95 , 5, $a_header['references'][1], 0, 0, "L");

        $this->SetXY( 7 , 62 );
        if($a_header['references'][2]!="")
        {
        ...
        foreach($this->a_totalsVar as $row=>$a_rowValues){
            foreach($a_rowValues as $keyValue=>$value){
                if(isset($this->a_partialTotals[$cont]))
                    $this->a_partialTotals[$cont]+= str_replace(",",".",$a_value[$row][$value]);
                else
                    $this->a_partialTotals[$cont] = str_replace(",",".",$a_value[$row][$value]);

                if(isset($this->a_totals[$cont]))
                    $this->a_totals[$cont]+= str_replace(",",".",$a_value[$row][$value]);
                else
                    $this->a_totals[$cont] = str_replace(",",".",$a_value[$row][$value]);
    ...
    public function setRowPage($a_value, $limit=40){
