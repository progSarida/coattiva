...
class cls_html
{
    ...
    public function getOptions (array $a_records, array $a_selection )
    {
        ...
        foreach($a_records as $i=>$a_record){

            $option .= "<option value='".$a_record[$a_selection['value']]."'";
            if($a_selection['selected'] == $a_record[$a_selection['value']])
                $option.= " selected";

            $color = "";
            if(isset($a_selection['color'])) {
                $color = " style='color: " . $a_record[$a_selection['color']] . "; font-weight: bold;'";
            }
            ...
            if(is_array($a_selection['text'])){
                ...
                for($j=0;$j<count($a_selection['text']);$j++){
                    if(substr($a_selection['text'][$j],0,1)=="[" && substr($a_selection['text'][$j],strlen($a_selection['text'][$j])-1,1)=="]"){
                        $text.= $a_record[substr($a_selection['text'][$j],1, strlen($a_selection['text'][$j])-2)];
                    }
                    else
                ...
                $option.= $a_record[$a_selection['value']];
            }
