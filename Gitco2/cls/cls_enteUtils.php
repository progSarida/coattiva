...
class cls_enteUtils
{
    ...
    if($ID == null || $ID == "")
    {
      ...
      $a_paramsEnti = array(
    			...
    			'fields'=> array(
    					...
    					array(  'name' => 'A_Mani_Cautelari',               'type' => 'int', 'value' => $val['A_Mani_Cautelari']),
    					array(  'name' => 'Spese_Notifica_Cautelari',       'type' => 'int', 'value' => $val['Spese_Notifica_Cautelari'])
    			)
    	);
  ...
  public function controlloParametriRicorso( $c ,$ID)
  {
    if($ID ==	null)
    {
      ...
      $save = new stdClass();
      $save->CC = $c;
      $save->Termini_Corte_Giustizia_Tributaria = utf8_decode($res['Termini_Corte_Giustizia_Tributaria']);
  ...
  public function crea_tariffe_base($c)
  {
    ...
    for ($i=0; $i < count($result); $i++) {
      ...
      $save->CC = $c;
      $save->Data_Inizio = $result[$i]['Data_Inizio'];
      $save->Tipo = $result[$i]['Tipo'];
      $save->Descrizione = $result[$i]['Descrizione'];
      $save->Importo = $result[$i]['Importo'];
      $save->Note = $result[$i]['Note'];
      $save->Deposito_Portata = $result[$i]['Deposito_Portata'];
      $save->Importo_Fisso = $result[$i]['Importo_Fisso'];
      $save->Km_Giorni_Importo_Fisso = $result[$i]['Km_Giorni_Importo_Fisso'];
      $save->Coefficiente = $result[$i]['Coefficiente'];
      $save->Pignoramenti = $result[$i]['Pignoramenti'];
