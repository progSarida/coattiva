...
class cls_290
{
    ...
    public $a_countCheck = array("errori"=>0, "importato" =>0, "scarti"=>0, "omonimie" => 0, "toImport"=>0);
...
//            echo "(CC ".$this->a_model[$rowNumber]['CODICE_CATASTALE_DESTINATARIO']." - COMUNE ".$this->a_model[$rowNumber]['COMUNE_DESTINATARIO'].")";
    ...
    public function checkUtenteModel($rowNumber){
        if($this->a_model[$rowNumber]['TIPO_SOGGETTO']!="DITTA"){
            ...
            if (strlen($this->a_model[$rowNumber]['PARTITA_IVA'])!= 11
                || preg_match("/[^0-9]/",$this->a_model[$rowNumber]['PARTITA_IVA'])
    ...
    public function checkCodiciTributo($rowNumber){
        ...
        switch($this->a_model[$rowNumber]['TIPO_RISCOSSIONE']){
            case "CDS / AMMINISTRATIVA":
                $this->a_model[$rowNumber]['TIPO_PARTITA'] = "CDS";  break;
            case "IMU":
            case "TASI":
            case "ICI":
        ...
        if(count($this->a_model[$rowNumber]['CODICI_TRIBUTO'])>0){
            $query = "SELECT COUNT(*) as countCodes FROM codice_tributo WHERE Codice_Tributo IN (".$whereIn.") ";
            $query.= "AND (";
    ...
    public function readN4($line){
        ...
        if(!$imponibInt>0)  $imponibInt = 0;
        if(!$imponibDec>0)  $imponibDec = 0;
        if(!$impostaInt>0)  $impostaInt = 0;
        if(!$impostaDec>0)  $impostaDec = 0;
    ...
    public function getErrorHtml($html){
        return '<div class="col-lg-2 RowLabelError">ERRORE '.$this->a_countCheck['errori'].'</div>
            <div class="col-lg-10 RowInput"><b>'.$html.'</b></div>
            <div class="clean_row HSpace1"></div>';
    }

    public function getNoticeHtml($html, $text){
        return '<div class="col-lg-2 RowLabel">'.$text.' '.$this->a_countCheck['omonimie'].'</div>
    ...
    public function check290(){
        $a_position = array(
            "N1" => null, "N2" => null, "N3" => null, "N4" => null, "Extra" => null
        );
        if(isset($this->a_290["N0"]))
...
//                        }
        ...
        foreach ($this->a_checkUtente[$recordType] as $checkType=>$a_checks){
            foreach($a_checks as $key=>$a_check){
                ...
                if($recordType=="N2")
                    ...
                    $utenteN3 = $this->a_290['N3'][$a_check['position']['N1']][$a_check['position']['N2']][$a_check['position']['N3']]['Utente']['Cognome']['value'].
                    $this->a_290['N2'][$a_check['position']['N1']][$a_check['position']['N2']]['Utente'][$a_check['position']['N3']]['Ditta']['value']." ".
                    $this->a_290['N2'][$a_check['position']['N1']][$a_check['position']['N2']]['Utente'][$a_check['position']['N3']]['Nome']['value'];

                    $html .= " - [ N2 " . ($a_check['position']['N2']+1) . " ] " . $utenteN3 . " - ". $a_check['InformazioniCartella'];
                }
                ...
                if($checkType=="DB"){
                    ...
                    $N2error .= $this->getNoticeHtml($html, 'UTENTE');
                }
                else{
                    $this->a_countCheck['scarti']++;
    ...
    public function checkUtente($recordType, $a_utente, $a_position){
        if($a_utente['Utente']['NaturaGiuridica']['value']==1){
            ...
            if (strlen($a_utente['Utente']['PI']['value'])!= 11
                || preg_match("/[^0-9]/",$a_utente['Utente']['PI']['value'])
