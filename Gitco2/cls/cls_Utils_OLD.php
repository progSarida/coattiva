  ...
  public function GetObjectQuery($arrayField,$table,$arrayWhere = null)
  {
    ...
    //print_r($allType);
      ...
      for($i=0; $i<count($allType); $i++)
      {
        ...
        {
          /*$typeInsert = "";
          if(is_string($val)) $typeInsert = "string";
          else if(is_numeric($val)){
              if(is_int($val)) $typeInsert = "integer";
              else $typeInsert = "double";
          }else if(is_bool($val)) $typeInsert = "boolean";*/


          $Type = $this->GetTypeObject($allType[$i]["DATA_TYPE"]);

            //echo $key.": ".$Type." --- ".$typeInsert." --> ".$val."<br>";
          /*if($Type == "int" && $typeInsert != "integer" && $typeInsert != "double" && $typeInsert != "boolean") {
              if($typeInsert == "string" && !is_numeric($val))
                return false;
          }
          else if(($Type == "string" || $Type == "date") && $typeInsert != "string") return false;*/
