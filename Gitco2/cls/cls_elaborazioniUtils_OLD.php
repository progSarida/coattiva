...
//        $array_result = mysql_fetch_row($result);
    ...
    public function getDataPartita($ID,$c,$AnnoRif)
    {
        ...
        $query = "SELECT * FROM partita_tributi WHERE ID = '".$ID."' AND CC = '".$c."' AND Anno_Riferimento = '".$AnnoRif."'";
...
//        }
    ...
    public function TipiPagamento ($selected, $uscita, $attorif = null)  //  uscita pu� essere:
        // SELECT
        // TIPODASCRITTA
        // SCRITTADATIPO
    {
...
//				"Sollecito di avviso di intimazione ad adempiere",
    ...
    function GetDataUtente($utente)
    {
        ...
        $utente->Residenza = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));// new indirizzo( $progr , 'res' , $c );
        ...
        if($utente->Residenza != null)
            if($utente->Residenza->Via_ID!=1)
            {
                ...
                $utente->Residenza->Toponimo = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"toponimo");//new toponimo( $utente->Residenza->Via_ID , $utente->CC_Comune );
            }
            else if($utente->Residenza->Via_Cap_ID!=1)
            {
                $query = "SELECT * FROM toponimi_cappati WHERE ID = '".$utente->Residenza->Via_Cap_ID."'";
                $utente->Residenza->Toponimo = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"toponimi_cappati");//new toponimo_cap( $utente->Residenza->Via_Cap_ID );
            }
            else
                $utente->Residenza->Toponimo = null;

        $query = "SELECT * FROM indirizzo WHERE Utente_ID = '".$utente->ID."' AND Tipo = 'dom'";
        $utente->Domicilio = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));//new indirizzo( $progr , 'dom' , $c );
        ...
        if($utente->Domicilio != null)
            if($utente->Domicilio->Via_ID!=1)
            {
                ...
                $utente->Domicilio->Toponimo = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"toponimo");//new toponimo( $utente->Residenza->Via_ID , $utente->CC_Comune );
            }
            else if($utente->Domicilio->Via_Cap_ID!=1)
            {
                $query = "SELECT * FROM toponimi_cappati WHERE ID = '".$utente->Domicilio->Via_Cap_ID."'";
                $utente->Domicilio->Toponimo = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"toponimi_cappati");//new toponimo_cap( $utente->Residenza->Via_Cap_ID );
            }
            else
                $utente->Domicilio->Toponimo = null;

        $query = "SELECT * FROM indirizzo WHERE Utente_ID = '".$utente->ID."' AND Tipo = 'rec'";
        $utente->Recapito = $this->cls_db->getArrayLine($this->cls_db->ExecuteQuery($query));//new indirizzo( $progr , 'rec' , $c );
        ...
        if($utente->Recapito != null)
            if($utente->Recapito->Via_ID!=1)
            {
                ...
                $utente->Recapito->Toponimo = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"toponimo");//new toponimo( $utente->Residenza->Via_ID , $utente->CC_Comune );
            }
            else if($utente->Recapito->Via_Cap_ID!=1)
            {
                $query = "SELECT * FROM toponimi_cappati WHERE ID = '".$utente->Recapito->Via_Cap_ID."'";
                $utente->Recapito->Toponimo = $this->cls_db->getArrayLineNull($this->cls_db->ExecuteQuery($query),"toponimi_cappati");//new toponimo_cap( $utente->Residenza->Via_Cap_ID );
            }
            else
