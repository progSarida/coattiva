...
//		$this->log = new LOG();
  ...
  public function checkFieldType($field){
	    if(is_numeric($field['value']) && $field['type']!='string')
            $value = $field['value'];
        else if(is_null($field['value'])||trim($field['value']," ")=="")
            $value = 'null';
	...
	function connect(){
        //echo "<script>alert('".DB_HOST."' - '".DB_USERNAME."' - '".DB_PASSWORD."' - '".DB_NAME."');</script>";
		try{
...
//                $this->log->error("Alla riga ".$e->getLine().".\nCodice: ".$e->getCode().".\nErrore: ".$e->getMessage());
    ...
    function getObjectLineNull($results,$table=null){
        ...
        {
            if(isset(self::$emptyObjectCache[$table])){
                return clone self::$emptyObjectCache[$table];
            }
            ...
            self::$emptyObjectCache[$table] = clone $val;
        }else return null;
        return $val;

    }

    function getResults($results, $resType = "array", $setKey = false){
        $a_res = array();
        if($resType=="array"){
...
//        var_dump($a_values);
    ...
    // All'interno della tua classe cls_db
    public function escapeString($data) {
        // 1. Assicurati che $this->conn sia l'oggetto di connessione mysqli valido
        if ($this->conn instanceof mysqli) {
            return $this->conn->real_escape_string($data);
        }
        
        // Fallback meno sicuro per ambienti molto vecchi (NON raccomandato)
        ...
        switch($type){
            ...
            case "date":
                if($value=="0000-00-00" || $value=="" || !isset($value) || is_null($value))
                    ...
                    if (strpos($value, '/') !== false) {
                        $value = $this->help->toDbDate($value);
                    }
                    return "'".addslashes($value)."'";
                break;
            case "int":
...
//        echo "**".$value." ".gettype($value)."**";
    ...
    function getColumnDataTypes($table){
        $results = $this->ExecuteQuery("SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_DEFAULT FROM information_schema.columns WHERE table_name='".$table."'");
        $a_columns = $this->getResults($results);
        $a_return = array();
        for($i=0;$i<count($a_columns);$i++){
            $a_return[$a_columns[$i]['COLUMN_NAME']] = $this->GetObjectType($a_columns[$i]['DATA_TYPE']);
        }
        return $a_return;
    }

    public function GetObjectQuery($table, $a_values, $a_dataTypes = null, $a_where = null)
    {
        $a_params = array( 'table' => $table, 'fields'=> array() );
        if(is_null($a_dataTypes))
            $a_dataTypes = $this->getColumnDataTypes($table);
        foreach($a_values as $field=>$value)
            $a_params['fields'][] = array('name' => $field, 'type' => $a_dataTypes[$field], 'value' =>$value);

        if(is_array($a_where)){
            $a_params['updateField'] = array();
            foreach($a_where as $field=>$value)
                $a_params['updateField'][] = array('name' => $field, 'type' => $a_dataTypes[$field], 'value' =>$value);
        }

        return $a_params;
    }

    private function GetObjectType($Type)
    {
