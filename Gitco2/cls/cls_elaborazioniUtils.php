...
class cls_elaborazioniUtils
{
    ...
    function getFiltersDescription(array $filter){
        ...
        if(!empty($filter['Court_Level'])){
            if($filter['Court_Level']!=null && $filter['Court_Level']!=""){
                $a_return[$i]['label'] = "GRADO DI GIUDIZIO";
                $grado = "";
                switch($filter['Court_Level']){
                    case 1: $grado = "1° Grado"; break;
                    case 2: $grado = "2° Grado"; break;
                    case 3: $grado = "3° Grado"; break;
                    default: $grado = "NESUNO"; break;
                }
                $a_return[$i]['value'] = $grado;
                $i++;
            }
        }

        if(!empty($filter['autority_type'])){
            if($filter['autority_type']!=null && $filter['autority_type']!=""){
                $a_return[$i]['label'] = "AUTORITA'";
                $a_return[$i]['value'] = $filter['autority_type'];
                $i++;
            }
        }

        if(!empty($filter['data_udienza_da']) && !empty($filter["data_udienza_da"])){

            $a=$i;
            if($filter['data_udienza_da']!=null && $filter['data_udienza_da']!=""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";
                $a_return[$i]['label'] = "DATA UDIENZA";
                $a_return[$i]['value'] .= "Dal ". $this->cls_help->toItalianDate($filter['data_udienza_da']);
                $a=$i+1;
            }
            if($filter['data_udienza_a']!=null && $filter['data_udienza_a']!=""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";

                $a_return[$i]['label'] = "DATA UDIENZA";
                $a_return[$i]['value'] .= " al ". $this->cls_help->toItalianDate($filter['data_udienza_a']);
                $a=$i+1;
            }

            $i=$a;
        }
        else if(!empty($filter['data_udienza_da'])){
            if($filter['data_udienza_da']!=null && $filter['data_udienza_da']!=""){
                $a_return[$i]['label'] = "DATA UDIENZA";
                $a_return[$i]['value'] = "Dal ". $this->cls_help->toItalianDate($filter['data_udienza_da']);
                $i++;
        ...
        else if(!empty($filter['data_udienza_a'])){
            if($filter['data_udienza_a']!=null && $filter['data_udienza_a']!=""){
                $a_return[$i]['label'] = "DATA UDIENZA";
                $a_return[$i]['value'] = "Fino al ". $this->cls_help->toItalianDate($filter['data_udienza_a']);
                $i++;
            }
        }

        if(!empty($filter['id_cronologico_da']) && !empty($filter["id_cronologico_a"])){

            $a = $i;
            if($filter['id_cronologico_da']!=null && $filter['id_cronologico_da']!=""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";
                $a_return[$i]['label'] = "ID CRONOLOGICO";
                $a_return[$i]['value'] .= "Da ". $filter['id_cronologico_da'];
                $a=$i+1;
            }
            if($filter['id_cronologico_a']!=null && $filter['id_cronologico_a']!=""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";

                $a_return[$i]['label'] = "ID CRONOLOGICO";
                $a_return[$i]['value'] .= " a ". $filter['id_cronologico_a'];
                $a=$i+1;
            }

            $i=$a;
        }
        else if(!empty($filter['id_cronologico_da'])){
            if($filter['id_cronologico_da']!=null && $filter['id_cronologico_da']!=""){
                $a_return[$i]['label'] = "ID CRONOLOGICO";
                $a_return[$i]['value'] = "Da ". $filter['id_cronologico_da'];
                $i++;
        ...
        else if(!empty($filter['id_cronologico_a'])){
            if($filter['id_cronologico_a']!=null && $filter['id_cronologico_a']!=""){
                $a_return[$i]['label'] = "ID CRONOLOGICO";
                $a_return[$i]['value'] = "Fino a ". $filter['id_cronologico_a'];
                $i++;
            }
        }

        if(!empty($filter['anno_cronologico_da']) && !empty($filter["anno_cronologico_a"])){

            $a=$i;
            if($filter['anno_cronologico_da']!=null && $filter['anno_cronologico_da']!=""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";
                $a_return[$i]['label'] = "ANNO CRONOLOGICO";
                $a_return[$i]['value'] .= "Dal ". $filter['anno_cronologico_da'];
                $a=$i+1;
            }
            if($filter['anno_cronologico_a']!=null && $filter['anno_cronologico_a']!=""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";

                $a_return[$i]['label'] = "ANNO CRONOLOGICO";
                $a_return[$i]['value'] .= " al ". $filter['anno_cronologico_a'];
                $a=$i+1;
            }
            $i=$a;
        }
        else if(!empty($filter['anno_cronologico_da'])){
            if($filter['anno_cronologico_da']!=null && $filter['anno_cronologico_da']!=""){
                $a_return[$i]['label'] = "ANNO CRONOLOGICO";
                $a_return[$i]['value'] = "Dal ". $filter['anno_cronologico_da'];
                $i++;
        ...
        else if(!empty($filter['anno_cronologico_a'])){
            if($filter['anno_cronologico_a']!=null && $filter['anno_cronologico_a']!=""){
                $a_return[$i]['label'] = "ANNO CRONOLOGICO";
                $a_return[$i]['value'] = "Fino al ". $filter['anno_cronologico_a'];
                $i++;
            }
        }
        ...
        else if(isset($filter['descrizione_a'])){
            if($filter['descrizione_a']!=null && $filter['descrizione_a']!=""){
                $a_return[$i]['label'] = "DESCRIZIONE";
        ...
        if(isset($filter['data_richiesta_da']) && isset($filter["data_richiesta_a"])){
            if (!isset($a_return[$i]['value']))
                $a_return[$i]['value'] = "";

            if($filter['data_richiesta_da'] != "" && $filter['data_richiesta_da'] != null ) {

                $a_return[$i]['label'] = "DATA RICHIESTA";
                $a_return[$i]['value'] .= "Da " . $filter['data_richiesta_da'];
            }
            if($filter["data_richiesta_a"] != "" && $filter["data_richiesta_a"] != null){
                $a_return[$i]['label'] = "DATA RICHIESTA";
                $a_return[$i]['value'] .= " A " . $filter['data_richiesta_a'];
            }

            if(($filter['data_richiesta_da'] != "" && $filter['data_richiesta_da'] != null) || ($filter["data_richiesta_a"] != "" && $filter["data_richiesta_a"] != null))
                $i++;
        }
        else if(isset($filter['data_richiesta_da'])){
            if($filter['data_richiesta_da'] != "" && $filter['data_richiesta_da'] != null){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";
        ...
        else if(isset($filter['data_richiesta_a'])){
            if($filter['data_richiesta_a'] != null && $filter['data_richiesta_a'] != ""){
                if(!isset($a_return[$i]['value']))
                    $a_return[$i]['value'] = "";
        ...
        if(isset($filter['comune'])){
            if(!isset($a_return[$i]['value']))
                $a_return[$i]['value'] = "";

            if($filter['comune'] != null && $filter['comune'] != "") {
                $a_return[$i]['label'] = "COMUNE";
                $a_return[$i]['value'] = $filter['comune'];
        ...
        else if(isset($filter['importo_a'])){
            if($filter['importo_a']!=null && $filter['importo_a']!=""){
                $a_return[$i]['label'] = "IMPORTO";
        ...
        if(isset($filter['ambito'])){
            if($filter['ambito']!=null && $filter['ambito']!=""){

                $a_return[$i]['label'] = "AMBITO";
                $a_return[$i]['value'] = $filter['ambito'];

                $i++;
            }
        }

        if(isset($filter['anno'])){
            if($filter['anno']!=null && $filter['anno']!=""){

                $a_return[$i]['label'] = "ANNO";
                $a_return[$i]['value'] = $filter['anno'];

                $i++;
            }
        }

        if(isset($filter['azione'])){
            if($filter['azione']!=null && $filter['azione']!=""){

                $a_return[$i]['label'] = "AZIONE";
                $a_return[$i]['value'] = $filter['azione'];

                $i++;
            }
        }

        if(isset($filter['utente'])){
            if($filter['utente']!=null && $filter['utente']!=""){

                $a_return[$i]['label'] = "UTENTE";
                $a_return[$i]['value'] = $filter['utente'];

                $i++;
            }
        }

        if(isset($filter['da_data'])){
            if($filter['da_data']!=null && $filter['da_data']!=""){

                $a_return[$i]['label'] = "DA DATA";
                $a_return[$i]['value'] = $filter['da_data'];

                $i++;
            }
        }

        if(isset($filter['a_data'])){
            if($filter['a_data']!=null && $filter['a_data']!=""){

                $a_return[$i]['label'] = "A DATA";
                $a_return[$i]['value'] = $filter['a_data'];

                $i++;
            }
        }

        if(isset($filter['banca'])){
            if($filter['banca']!=null && $filter['banca']!=""){

                $a_return[$i]['label'] = "BANCA";
                $a_return[$i]['value'] = $filter['banca'];
