...
class Imap{
     ...
     *
     * @return void
     * @param $par_email parametri mail
     * @param $mailbox imap_open string
     * @param $username
     * @param $password
     * @param $protocol imap or pop3
     * @param $porta 
     * @param $encryption ssl, tls, notls
     * @param $validazione true or false
     */
    public function __construct($par_email, $validazione = true, $mailbox = '', $username = '', $password = '', $protocol = '', $porta = '', $encryption = '') {
        
    	if($par_email!=null)
    	{
    	    $cls_cript = new cls_crypt();

    		$username = $par_email->Nome_Utente;
    		$password = $cls_cript->decryptIt($par_email->Password);
    		$mailbox = $par_email->Server_Posta_Arrivo;
    		$porta = $par_email->Porta_Arrivo;
    ...
    /**
     * ritorna email che rispettano il criterio inserito per la ricerca
     *
     * @return array messages
     * @param $criterio tipo di ricerca
     ...
     * @param $criterio_string valore della ricerca
     * @param $withbody without body
     */
    public function getCriteriaMessages($criterio, $criterio_string, $withbody=true){
//     	$result = imap_sort($this->imap, SORTARRIVAL, 0, null, $criterio);

    	$result = imap_search($this->imap, $criterio.' "'.$criterio_string.'"');
    	foreach($result as $k=>$i){
    		$emails[]= $this->formatMessage($i, $withbody);
    	}
    ...
    /**
     * ritorna email che rispettano il criterio inserito per la ricerca con un ordinamento preciso
     *
     * @return array messages
     * @param $criterio_ordinamento tipo di ricerca
     ...
     * @param $ordinamento 0 ascendente o 1 discendente
     * @param $criterio tipo di ricerca
     * @param $criterio_string valore della ricerca
     * @param $withbody without body
     */
    public function getOrderedMessages($criterio_ordinamento, $ordinamento, $criterio, $criterio_string, $withbody=true){
    	
    	$result = imap_sort($this->imap, $criterio, $ordinamento, null, $criterio.' "'.$criterio_string.'"');
    	foreach($result as $k=>$i){
    		$emails[]= $this->formatMessage($i, $withbody);
    	}
    ...
    public function getAttachments($mailNum, $part, $partNum) {
        ...
        if (isset($part->parts)) {
            foreach ($part->parts as $key => $subpart) {
                ...
                $result = $this->getAttachments($this->imap, $mailNum, $subpart,
                    $newPartNum);
                if (count($result) != 0) {
    ...
    public function getPart($messageNumber, $partNumber, $encoding) {
    	...
    	$data = imap_fetchbody($this->imap, $messageNumber, $partNumber);
    	switch($encoding) {
    ...
    public function getEmailParts($mail_id, $parti)
    {
    	$flattenedParts = $this->flattenParts($parti);
    	$email = array();
    	...
    	foreach($flattenedParts as $partNumber => $part) {
    		switch($part->type) {
    			...
    			case 0:
    				...
    				$email['Mail'][] = $this->getPart($mail_id, $partNumber, $part->encoding);
    			...
    			case 1:
    				...
    				break;
    			case 2:// attached message headers
    			case 3: // application
    			case 4: // audio
    			case 5: // image
    			case 6: // video
    			case 7: // other
    				...
    				if($filename) {
    					// it's an attachment
    					$email['Allegati']['filename'][] = $filename;
    					$email['Allegati']['data'][] = $this->getPart($mail_id, $partNumber, $part->encoding);
    ...
    public function headerString($header)
    {
    	$string_header = "";
	    foreach ($header as $key => $value) {
	    
	    	$string_header.= strtoupper($key).": ";
	    	 
	    	if(is_array($value))
	    	{
	    		for($v=0;$v<count($value);$v++)
	    		{
	    		if(is_object($value[$v]))
	    		foreach ($value[$v] as $key_int => $value_int)
	    			$string_header.= "[".$key_int." = ".$value_int."]; ";
	    					else
	    			$string_header.= $value[$v]."; ";
	    		}
	    		}
	    		else
	    			$string_header.= $value;
