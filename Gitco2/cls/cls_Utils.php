...
class cls_Utils{
   ...
   * $prefisso testuale, la prima parte del nome, e alla fine c'� data
    ...
    function delete_files_after_X_days ($path, $prefisso, $giorni)
    {
        ...
        while (($file = readdir($handle)) != false)
        {
            ...
            if (substr($file, 0, $lungFissa) == $prefisso)
            {
                ...
                if ($dataOk == false)
                {
                    $dataFile = $esplodoTrattini[$totPezzi-1];  //  l'ultimo � la data?
                    $dataOk = false;
                    $esplodoMeno = explode ("-", $dataFile);
                    if (count($esplodoMeno) == 3)
                    {
  ...
  function crea_dir( $path )
  {
    ...
    public function contains($str,$strCheck){
        $flag = false;
  ...
  private static $columnTypeCache = array();
  ...
  public function GetObjectQuery($arrayField,$table,$arrayWhere = null)
  {
    ...
    if (isset(self::$columnTypeCache[$table])) {
        $allType = self::$columnTypeCache[$table];
    } else {
        ...
        self::$columnTypeCache[$table] = $allType;
    }
    ...
    if($arrayWhere!=null)
    {
      ...
      foreach($arrayWhere as $key => $val) {
        ...
        if(gettype($val)!="object") if($Type==""&&$exsist) $Type = is_numeric($val)?"int":"string";
        else if($Type==""&&$exsist) $Type = is_numeric($val->value)?"int":"string";
  		  //if($val=='null'){$Type = "int"; $val="";}
  		  //echo $Type.": $key = $val<br>";
