...
class cls_print{
    ...
    public function setPrintParams($printType, $type){
        switch($printType){
                ...
                switch($type){
                    ...
                    case "positions":
                        ...
                        $this->a_filters = array("city","fileType","positionNotificationLimit","expiredPosition","PrinterId","chron", "paymentStatus",
                            ...
                            "taxStopFlag","dischargeFlag","dischargeDate","importFromTo","fornitureFromTo","sort");
                        $this->tax_firstOpt = "<option value=''>Tutte</option>";
                        $this->year_blank = "y";
                        break;

                    case "last_acts":
                        $this->a_type = array("title"=>"Elenco ultimi atti","action"=>"list_last_acts.php");
                        $this->a_filters = array("city","fileType",
                            "PrinterId", "paymentActStatus", "cronoYear",
                            "fornitureFromTo","elaborationDate","printDate",
                            "taxStopFlag","sort");
                        $this->year_blank = "y";
                            break;
                    ...
                    case "SOLL_PRE":
                        ...
                        $this->a_filters = array("city","cronoYear","fileType","PrinterId","printStatus","PrintTypeOrdinaria","officialType","elaborationDate",
                            "printDate","notificationDate","flowDate","flowNumber","blockSingleAct","taxStopFlag","dischargeFlag","dischargeDate","sort");
                        break;
                    case "AV_MORA":
                        $this->a_type = array("title"=>"Elenco Avvisi di messa in mora","action"=>"list_atto.php");
                        $this->a_filters = array("city","cronoYear","fileType","PrinterId","printStatus","PrintTypeId","officialType","elaborationDate",
                            "printDate","notificationDate","flowDate","flowNumber","blockSingleAct","taxStopFlag","dischargeFlag","dischargeDate","sort");
                        break;
                    case "ING":
                        $this->a_type = array("title"=>"Elenco Ingiunzioni","action"=>"list_atto.php");
                        $this->a_filters = array(
                            "city", "cronoYear", "fileType", "PrinterId", "printStatus", "TrafficLaw", "PrintTypeId", "officialType",
                            "elaborationDate", "printDate", "notificationDate",
                            "notificationMode", "notificationStock", "notificationAnomaly",
                            "flowDate", "flowNumber",
                            "instalmentAtto","paymentActStatus",
                            "blockSingleAct","taxStopFlag","dischargeFlag","dischargeDate","sort"
                        );
                        break;
                    ...
                    case "AV_INT":
                        ...
                        $this->a_filters = array(
                            "city","cronoYear","fileType", "PrinterId", "printStatus", "PrintTypeId", "officialType",
                            "elaborationDate", "printDate", "notificationDate",
                            "notificationMode", "notificationStock", "notificationAnomaly",
                            "flowDate", "flowNumber",
                            "instalmentAtto","paymentActStatus",
            ...
            case "elab":
                switch($type){
                    case "discharge":
                        $this->a_type = array("title"=>"Elaborazione discarico",
                            //"action"=>"elab_discharge.php"
                        );
                        $this->a_filters = array("elabType","finalElabDate","dischargeLimitDate", "fileType","sort");
                        $this->a_messages = array(array("title"=>"PROCEDURA DI DISCARICO BLOCCATA!"));
                        break;

                    case "extraction":
                        $this->a_type = array("title"=>"Elaborazione estrazione discarico",
                            //"action"=>"elab_extraction.php"
                        );
                        $this->a_filters = array("elabType","finalElabDate","sort");
                        $this->a_messages = array(array("title"=>"PROCEDURA DI ESTRAZIONE DISCARICO BLOCCATA!"));
                        break;
    ...
    public function setFilter($filter){
        ...
        switch($filter){
            ...
            case "importFromTo":
                ...
                $a_filterParams['title'] = "Importo preso in carico";
                $a_filterParams['input'] = "Da <input name=\"from_import\" class=\"text_center width30\" onblur='this.value = this.value.replace(\",\",\".\");' >";
                $a_filterParams['input'].= "&nbsp;&nbsp;a <input name=\"to_import\" class=\"text_center width30\" onblur='this.value = this.value.replace(\",\",\".\");'>";

                break;

            case "fornitureFromTo":

                $a_filterParams['title'] = "Data preso in carico";
                $a_filterParams['input'] = "Dal <input name=\"from_forniture\" class=\"text_center width30 picker\">";
                $a_filterParams['input'].= "&nbsp;&nbsp;al <input name=\"to_forniture\" class=\"text_center width30 picker\">";

                break;

            case "cronoYear":

                    $a_filterParams['title'] = "Anno cronologico";
                    $a_filterParams['input'] = "Dal <input name=\"from_cronoYear\" class=\"text_right width15\">";
                    $a_filterParams['input'].= "&nbsp;&nbsp;al <input name=\"to_cronoYear\" class=\"text_right width15\">";
            ...
            case "paymentActStatus":
                $a_filterParams['title'] = "Pagamenti";
                $a_filterParams['input'] = "<select name=\"paymentActStatus\" class=\"width95\">
                                                <option></option>                                                                                                                     
                                                <option value='incompleted'>Incompleti ( Nessuno + Parziali )</option>
                                                <option value='no'>Nessuno</option>
                                                <option value='partial'>Parziali</option>
            ...
            case "blockSingleAct":
                $a_filterParams['title'] = "Blocco atto singolo";
                $a_filterParams['input'] = "<select name=\"blockSingleAct\" class=\"width95\">
                                                <option value=\"no\">No</option>
                                                <option value=\"si\">Si</option>
                                                <option value=\"\">Entrambi</option>
                                            </select>";
    ...
    public function getMessages(){
        $html = "";
        foreach($this->a_messages as $a_message){
            $html.= $this->addMessageDiv($a_message);
        }

        return $html;
    }

    public function addMessageDiv($a_params){
        $div = "<div class=\"width90 text_center\" style=\"clear:both;height:23px;\">";
        $div.= "    <div style=\"float:left;\" class=\"width100\"><span class=\"font_bold\" style='color: darkred'>".$a_params['title']."</span></div>";
        $div.= "</div>";
        return $div;
    }
    ...
    function getWhereFromFilters($filter, $table=null, $tipo = "atto")
    {
        ...
        //ANNO CRONOLOGICO
        if (isset($filter['from_cronoYear'])) {
            if($filter['from_cronoYear']>0){
                if ($where != "")
                    $where .= "AND ";

                $where .= "( Anno_Cronologico >= " . $filter['from_cronoYear'] . " ";
                if ($filter['to_cronoYear'] > 0)
                    $where .= "AND Anno_Cronologico <= " . $filter['to_cronoYear'] . " ";
                $where .= ") ";
            }
        }
        ...
        if (isset($filter['notificationStock'])) {
            if($filter['notificationStock']!="") {
                ...
                if($filter['notificationStock']=="y")
                    ...
                    $where .= " Stato_Notifica=".$filter['notificationStock']." ";
            }
        }
        ...
        if (isset($filter['pignoNotificationStock'])) {
            if($filter['pignoNotificationStock']!="") {
                ...
                if($filter['pignoNotificationStock']=="y")
                    ...
                    $where .= " Stato_Notifica_Pignoramento=".$filter['pignoNotificationStock']." ";
            }
        }
        ...
        //IMPORTO PRESO IN CARICO
        if (isset($filter['from_import'])) {
            if($filter['from_import']!="" && $filter['from_import']!=null) {
                if ($where != "")
                    $where .= "AND ";

                $where .= " ( SELECT SUM(TRIB.Imposta) FROM `tributo` as TRIB where TRIB.Partita_ID = v_positions.Partita_ID ) > ".$filter['from_import']." ";

                if(isset($filter['to_import']))
                    if($filter['to_import']!="" && $filter['to_import']!=null)
                        $where .= " AND ( SELECT SUM(TRIB.Imposta) FROM `tributo` as TRIB where TRIB.Partita_ID = v_positions.Partita_ID ) < ".$filter['to_import']." ";
            }
            else{
                if(isset($filter['to_import']))
                    if($filter['to_import']!="" && $filter['to_import']!=null)
...
//        }
        ...
        //Stato Pagamenti
        if (isset($filter['paymentActStatus'])) {
            if ($filter['paymentActStatus'] !="") {
...
//                $where.= " AND Data_Notifica_Pignoramento is null ) OR ";
                ...
                $where .= " (DocumentTypeId IN (2,12,23) AND Data_Notifica IS NOT NULL AND ( CASE ";
                //$where .= " WHEN DocumentTypeId = 2 THEN IF(Data_Notifica + INTERVAL 1 YEAR > '".date('Y-m-d')."', 'KO' , 'OK' ) ";
                //$where .= " WHEN DocumentTypeId = 12 THEN IF(Data_Notifica + INTERVAL 6 MONTH > '".date('Y-m-d')."', 'KO' , 'OK' ) ";
                //$where .= " WHEN DocumentTypeId = 23 THEN IF(Data_Notifica + INTERVAL 1 YEAR > '".date('Y-m-d')."', 'KO' , 'OK' ) ";
                $where .= " WHEN DocumentTypeId = 2 THEN IF(Data_Notifica > '".date('Y-m-d')."' - INTERVAL 11 MONTH, 'OK' , 'KO' ) ";
                $where .= " WHEN DocumentTypeId = 12 THEN IF(Data_Notifica > '".date('Y-m-d')."'  - INTERVAL 5 MONTH, 'OK' , 'KO' ) ";
                $where .= " WHEN DocumentTypeId = 23 THEN IF(Data_Notifica > '".date('Y-m-d')."' - INTERVAL 11 MONTH, 'OK' , 'KO' ) ";
                $where .= " ELSE 'KO' END ) = 'KO' )";
            }
            else if($filter['positionNotificationLimit']=="y"){//attiva
                if ($where != "")
                    $where .= "AND ";
        ...
        //BLOCCO ATTO SINGOLO
        if (isset($filter['blockSingleAct'])) {
            if($filter['blockSingleAct']!=""){
                if ($where != "")
                    $where .= "AND ";

                if($filter['blockSingleAct']=="si")
        ...
        //DATA FORNITURA
        if (isset($filter['from_forniture'])) {
            if($this->cls_help->toDbDate($filter['from_forniture'])!=null){
                if ($where != "")
                    $where .= "AND ";

                $where .= "( Data_Fornitura >= '" . $this->cls_help->toDbDate($filter['from_forniture']) . "' ";

                if(isset($filter['to_forniture']))
                    if ($this->cls_help->toDbDate($filter['to_forniture']) != null)
                        $where .= "AND Data_Fornitura <= '" . $this->cls_help->toDbDate($filter['to_forniture']) . "' ";
                $where .= ") ";
            }
            else{
                if(isset($filter['to_forniture']))
        ...
        if (isset($filter['from_courtHearingDate'])) {
            if($this->cls_help->toDbDate($filter['from_courtHearingDate'])!=null){
                if ($where != "")
                    ...
                    if(isset($filter['no_courtHearingDate'])){
    ...
    public function getFiltersDescription($filter){
        ...
        if(isset($filter['from_cronoYear'])){
            if($filter['from_cronoYear']!=""){
                $a_return[$i]['label'] = "ANNO CRONOLOGICO";
                $a_return[$i]['value'] = "Dal ".$filter['from_cronoYear'];
                if(isset($filter['to_cronoYear']))
                    if(!empty($filter['to_cronoYear']))
                        $a_return[$i]['value'].= " al ".$filter['to_cronoYear'];
