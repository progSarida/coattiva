...
class cls_literalNumber{

    function getPostalFormat($amount)
    {
        list($int, $decimal) = explode('.', $amount);

        $ritorno = $this->integerToWord($int);

        if( $decimal > 0) {
            $ritorno .= '/';
            $ritorno .= $decimal;
            if(strlen($decimal)==1) $ritorno .= '0';
        }
        else
            $ritorno .= '/00';
    ...
    function euroToword($euro_val)
    {
        list($euro, $centesimi) = explode('.', $euro_val);
        $centesimi = intval(substr((string) $centesimi, 0, 2));

        $money = $this->integerToWord($euro);
        $money .= ' euro';
        if( $centesimi > 0)
        {
            $money .= ' e ';
            $money .= $this->integerToWord($centesimi);
            $money .= ' centesimi';
        }
    ...
    function integerToWord($num, $separatore = 0){
        if( !is_string($num) ){
            $num .= "";
        }
    ...
    private function centinaia($num, $separa = 1){
        $num    = (int) $num;
